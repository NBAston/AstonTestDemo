 ...
 */
@ccclass
export default class VXPQZNNAniNode extends cc.Component {
    /**赢家框 */
    ...
    start() {
        ...
        */
        ...
        this._gameStartAni.setCompleteListener((event) => {
            if (!self._isRest) {
                AppGame.ins.xpqznnModel.emit(UQZNNHelper.QZNN_SELF_EVENT.QZNN_START_ANI_COMPLETE);
            }
            self.setGameStartAniActive(false);
        });
        this.setGameStartAniActive(false);
    ...
    private addEvent() {
        // this._eventTongsha = AppGame.ins.xpqznnModel.on(UQZNNHelper.QZNN_SELF_EVENT.QZNN_TONGSHA_EVENT, this.onTongSha, this);
        // this._eventTongpei = AppGame.ins.xpqznnModel.on(UQZNNHelper.QZNN_SELF_EVENT.QZNN_TONGPEI_EVENT, this.onTongPei, this);

        AppGame.ins.xpqznnModel.on(UQZNNHelper.QZNN_SELF_EVENT.QZNN_SUB_S_GAME_START, this.showGameStart, this);
        ...
        AppGame.ins.xpqznnModel.on(UQZNNHelper.QZNN_SELF_EVENT.QZNN_TOTALSCORE_EVENT, this.onTotalScore, this);//分数滚动完事件
        AppGame.ins.xpqznnModel.on(UQZNNHelper.QZNN_SELF_EVENT.QZNN_RESET_SCENE, this.onResetScene, this);//重置场景事件
    ...
    private removeEvent() {
        AppGame.ins.xpqznnModel.off(UQZNNHelper.QZNN_SELF_EVENT.QZNN_SUB_S_GAME_START, this.showGameStart, this);
    ...
    private onGameEnd(data: XPQznn.NN_CMD_S_GameEnd,pl:any) {
        ...
        if (dLWScore != null) {
            ...
            var banker = AppGame.ins.xpqznnModel.gBankerIndex;
            var bankerLocalPos = AppGame.ins.xpqznnModel.getUISeatId(banker);
            ...
            if (bankerLocalPos != null)//有庄家
            {
                if (cbEndType == 1) {//通杀
                    ...
                    for (let j = 0; j < dLWScore.length; j++) {
                        let player = AppGame.ins.xpqznnModel.getbattleplayerbyChairId(j,pl);
                        if (player != null && player.playStatus == 1 && player.seatId >= 0 && player.seatId < 4) {

                            let index1 = AppGame.ins.xpqznnModel.getUISeatId(j);
                    ...
                    for (let j = 0; j < dLWScore.length; j++) {
                        ...
                        if (player != null && player.playStatus == 1 && player.seatId >= 0 && player.seatId < 4) {
                            ...
                            let index2 = AppGame.ins.xpqznnModel.getUISeatId(j);
                ...
                for (let i = 0; i < dLWScore.length; i++) {
                    ...
                    let player = AppGame.ins.xpqznnModel.getbattleplayerbyChairId(i,pl);
                    if (player != null && player.playStatus == 1 && player.seatId >= 0 && player.seatId < 4) {
    ...
    sendFapaiCompleteEvent(index: number) {
        ...
        AppGame.ins.xpqznnModel.emit(UQZNNHelper.QZNN_SELF_EVENT.QZNN_FAPAI_COMPLETE, index);
    ...
    private setFaPaiAni(index: number, b?: boolean) {
        ...
        if (b) {
            for (let j = 0; j < 5; j++) {
                ...
                if(j==4){
                    this._cardsFly[index][j].opacity = 0
                }
    ...
    setGameStartAniActive(b: boolean) {
        ...
        if (b == true) {
            ...
            //UAudioManager.ins.playSound("audio_qznn_ksyx");
