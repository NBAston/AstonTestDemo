 ...
 */
@ccclass
export default class VXPQZNNMenuNode extends VBaseUI {//cc.Component {
    // onLoad () {}
    ...
    start() {
        ...
        if (this.btn_exit != null) {
            ...
            // handler.component = this.getComponent("VQZNNMenuNode_xp");
    ...
    onBtnExitClick(event) {
        ...
        AppGame.ins.xpqznnModel.exitGame();
    ...
    onBtnRecordClick(event) {
        AppGame.ins.showUI(ECommonUI.QZNN_Record, AppGame.ins.roomModel.requestGameRecords(EGameType.XPQZNN));
    }
    /**牌型按钮点击事件 */
    onBtnPXClick(event) {
