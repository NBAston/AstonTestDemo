 ...
 */
@ccclass
export default class VXPQZNNXiaZhuNode extends cc.Component {
    ...
    updateXiazhuBtns(zhus: number[], length?: number) {
        ...
        for (let i = 0; i < length; i++) {
            ...
            if(zhus[i] > 0) {
                ...
                UEventHandler.addClick(item, this.node, "VQZNNXiaZhuNode_xp", "sendXiaZhuInfo", i);
    ...
    private sendXiaZhuInfo(event, customEventData) {
        ...
        AppGame.ins.xpqznnModel.sendAddScore(customEventData);
    }
    ...
    private onAddScoreResult(data: any) {
        ...
        // let chairId = AppGame.ins.xpqznnModel.changeToLocalPos(addJettonUser);
    ...
    private onGameSceneScore(data: any) {
        ...
        //         callback:"xz"
        //     }
        //     AppGame.ins.xpqznnModel.emit(UQZNNHelper.QZNN_SELF_EVENT.QZNN_DJS_EVENT, djsData);
        // }

        // AppGame.ins.xpqznnModel.emit(UQZNNHelper.QZNN_SELF_EVENT.QZNN_DJS_EVENT)
    }

    private onXuanZhuangEnd(data:any){
        var dwBankerUser = AppGame.ins.xpqznnModel.gBankerIndex;
