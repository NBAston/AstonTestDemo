...
import UQZNNHelper, { MAX_COUNT, OX_GOLDEN_BULL, OX_BOMB, OX_VALUE0, OX_FIVE_SMALL, OX_SILVERY_BULL, EQZNNState, QZNNBattlePlayerInfo } from "../UQZNNHelper_xp";
import UHandler from "../../../common/utility/UHandler";
import { EGameType, ECommonUI, ELeftType, ELevelType } from "../../../common/base/UAllenum";
import UMsgCenter from "../../../common/net/UMsgCenter";
import { Game, XPQznn, KPQznn, GameServer } from "../../../common/cmd/proto";
 ...
 * 作用:qznn 主要逻辑
 */
export default class MXPQZNNModel extends Model {
    // //大致流程  not use
    // group = {
    ...
    get CopyBattlePlayer(): { [key: number]: QZNNBattlePlayerInfo } {
        let data = {};
    ...
    //玩家是否在打牌
    isGameState(userId: number) {
        let player = this._battleplayer[userId]
        if (player) {
    ...
    /**改变玩家的状态 */
    changePlayerState(playStatus: boolean[]) {

        for (const key in this._battleplayer) {
            if (this._battleplayer.hasOwnProperty(key)) {
                const element = this._battleplayer[key];
                if (element) {
                    let chairId = element.chairId
                    element.playStatus = playStatus[chairId] ? 1 : 0
                }
            }
        }
    ...
    exit(): void {
        ...
        this.unregesterMsg(XPQznn.SUBID.SC_GAMESCENE_FREE, new UHandler(this.onGameSceneFree, this));
        this.unregesterMsg(XPQznn.SUBID.SC_GAMESCENE_CALL, new UHandler(this.onGameSceneCall, this));
        this.unregesterMsg(XPQznn.SUBID.SC_GAMESCENE_SCORE, new UHandler(this.onGameSceneScore, this));
        this.unregesterMsg(XPQznn.SUBID.SC_GAMESCENE_SELCARD, new UHandler(this.onGameSceneSelect, this));

        this.unregesterMsg(XPQznn.SUBID.SC_GAMESCENE_OPEN, new UHandler(this.onGameSceneOpen, this));
        //this.regesterMsg(XPQznn.SUBID.SC_GAMESCENE_OPEN, new UHandler(this.onGameSceneOpen, this));
        // this.regesterMsg(XPQznn.SUBID.SC_GAMESCENE_END, new UHandler(this.onGameSceneEnd, this));

        this.unregesterMsg(XPQznn.SUBID.NN_SUB_S_ADD_SCORE_RESULT, new UHandler(this.onAddScoreResult, this));
        this.unregesterMsg(XPQznn.SUBID.NN_SUB_S_CALL_BANKER, new UHandler(this.onCallBanker, this));
        this.unregesterMsg(XPQznn.SUBID.NN_SUB_S_CALL_BANKER_RESULT, new UHandler(this.onCallBankerResult, this));
        this.unregesterMsg(XPQznn.SUBID.NN_SUB_S_GAME_END, new UHandler(this.onGameEnd, this));
        this.unregesterMsg(XPQznn.SUBID.NN_SUB_S_GAME_START, new UHandler(this.onGameStart, this));
        this.unregesterMsg(XPQznn.SUBID.NN_SUB_S_OPEN_CARD_RESULT, new UHandler(this.onOpenCardResult, this));
        this.unregesterMsg(XPQznn.SUBID.NN_SUB_S_SEND_CARD, new UHandler(this.onSendCard, this));


        //发送选牌
        this.unregesterMsg(XPQznn.SUBID.NN_SUB_S_SEND_SELECT_CARDS, new UHandler(this.onSelectCard, this));

        //确认选牌
        this.unregesterMsg(XPQznn.SUBID.NN_SUB_S_SEL_CARD_RESULT, new UHandler(this.onSureSelectCard, this));
        ...
        this.unregesterMsg(XPQznn.SUBID.NN_SUB_C_ROUND_END_EXIT_RESULT, new UHandler(this.onEndExiitResult, this));
        this._roomInfo = null;
        this._battleplayer = {};
    ...
    qznnRegest() {
        ...
        this.regesterMsg(XPQznn.SUBID.SC_GAMESCENE_FREE, new UHandler(this.onGameSceneFree, this));
        this.regesterMsg(XPQznn.SUBID.SC_GAMESCENE_CALL, new UHandler(this.onGameSceneCall, this));
        this.regesterMsg(XPQznn.SUBID.SC_GAMESCENE_SCORE, new UHandler(this.onGameSceneScore, this));
        this.regesterMsg(XPQznn.SUBID.SC_GAMESCENE_SELCARD, new UHandler(this.onGameSceneSelect, this));

        this.regesterMsg(XPQznn.SUBID.SC_GAMESCENE_OPEN, new UHandler(this.onGameSceneOpen, this));
        //this.regesterMsg(XPQznn.SUBID.SC_GAMESCENE_OPEN, new UHandler(this.onGameSceneOpen, this));
        // this.regesterMsg(XPQznn.SUBID.SC_GAMESCENE_END, new UHandler(this.onGameSceneEnd, this));

        this.regesterMsg(XPQznn.SUBID.NN_SUB_S_ADD_SCORE_RESULT, new UHandler(this.onAddScoreResult, this));
        this.regesterMsg(XPQznn.SUBID.NN_SUB_S_CALL_BANKER, new UHandler(this.onCallBanker, this));
        this.regesterMsg(XPQznn.SUBID.NN_SUB_S_CALL_BANKER_RESULT, new UHandler(this.onCallBankerResult, this));
        this.regesterMsg(XPQznn.SUBID.NN_SUB_S_GAME_END, new UHandler(this.onGameEnd, this));
        this.regesterMsg(XPQznn.SUBID.NN_SUB_S_GAME_START, new UHandler(this.onGameStart, this));
        this.regesterMsg(XPQznn.SUBID.NN_SUB_S_OPEN_CARD_RESULT, new UHandler(this.onOpenCardResult, this));
        this.regesterMsg(XPQznn.SUBID.NN_SUB_S_SEND_CARD, new UHandler(this.onSendCard, this));


        //发送选牌
        this.regesterMsg(XPQznn.SUBID.NN_SUB_S_SEND_SELECT_CARDS, new UHandler(this.onSelectCard, this));

        //确认选牌
        this.regesterMsg(XPQznn.SUBID.NN_SUB_S_SEL_CARD_RESULT, new UHandler(this.onSureSelectCard, this));
        // UMsgCenter.ins.regester(0, Game.Common.MAINID.MAIN_MESSAGE_CLIENT_TO_GAME_SERVER, GameServer.SUBID.SUB_S2C_USER_ENTER_NOTIFY,
        //     new UHandler(this.onUserEnterNotify, this));
        ...
        this.regesterMsg(XPQznn.SUBID.NN_SUB_C_ROUND_END_EXIT_RESULT, new UHandler(this.onEndExiitResult, this));
    ...
    regesterMsg(subId: number, handler: UHandler) {
        UMsgCenter.ins.regester(EGameType.XPQZNN,
    ...
    unregesterMsg(subId: number, handler: UHandler) {
        UMsgCenter.ins.unregester(EGameType.XPQZNN,
            Game.Common.MAINID.MAIN_MESSAGE_CLIENT_TO_GAME_LOGIC,
            subId,
            handler);
    }
    ...
    sendMsg(subId: number, data: any, handler: UHandler = null, unlock: boolean = false) {
        UMsgCenter.ins.sendPkg(EGameType.XPQZNN,
            Game.Common.MAINID.MAIN_MESSAGE_CLIENT_TO_GAME_LOGIC,
    ...
    //         case MXPQZNNModel.QZNN_GAMESTATUS.FREE:
    //             {
    ...
    private user_status_notify(userid: number, usstatus: number) { //data: GameServer.MSG_S2C_GameUserStatus
        ...
        if (usStatus == null || usStatus == 0) {
            if (this._battleplayer && this._battleplayer[userId]) {
                ...
                //cc.log("删除玩家", this._battleplayer[userId])
            }
            // if (userId === AppGame.ins.roleModel.useId) {
            //     AppGame.ins.showUI(ECommonUI.MsgBox, {
            //         type: 1, data: ULanHelper.FOURCE_EXIT,
        ...
        this.emit(UQZNNHelper.QZNN_SELF_EVENT.QZNN_UPDATE_PLAYERS_EVENT, this._battleplayer);
    }
    ...
    /**
     * 设置所有场景内玩家状态
     * @param gameStatus MXPQZNNModel.QZNN_GAMESTATUS状态
    ...
    private setAllSceneStatus(gameStatus: number, state: EQZNNState, cbPlayStatus?: number[]) {
        ...
        this._gameStatus = MXPQZNNModel.QZNN_GAMESTATUS.FREE;
        //如果是上座玩牌玩家，重新赋状态
        if (this._battleplayer[this.selfUserId] != null
            ...
            this._gameStatus = gameStatus;//MXPQZNNModel.QZNN_GAMESTATUS.CALL;
        }
    }
    /**设置房间内玩家信息 */
    ...
    private onGameSceneFree(data: XPQznn.NN_MSG_GS_FREE) {
        ...
        this.emit(UQZNNHelper.QZNN_EVENT.QZNN_GAMESCENE_FREE, data, ishideContinue);
        this.emit(UQZNNHelper.QZNN_SELF_EVENT.QZNN_UPDATE_PLAYERS_EVENT, this._battleplayer);
    ...
    onGameSceneCall(data: XPQznn.NN_MSG_GS_CALL) {
        ...
        let d = data.playStatus
        for (let k of d) {
            cbPlayStatus[cbPlayStatus.length] = k ? 1 : 0
        }

        this.setAllSceneStatus(MXPQZNNModel.QZNN_GAMESTATUS.CALL, EQZNNState.Gameing, cbPlayStatus);
        this.emit(UQZNNHelper.QZNN_EVENT.QZNN_GAMESCENE_CALL, data);
        this.emit(UQZNNHelper.QZNN_SELF_EVENT.QZNN_UPDATE_PLAYERS_EVENT, this._battleplayer);
        this.emit(UQZNNHelper.QZNN_SELF_EVENT.QZNN_UPDATE_ROOMID, roomId);
    ...
    onGameSceneScore(data: XPQznn.NN_MSG_GS_SCORE) {
        ...
        this.setAllSceneStatus(MXPQZNNModel.QZNN_GAMESTATUS.SCORE, EQZNNState.Gameing, cbPlayStatus);

        this.emit(UQZNNHelper.QZNN_EVENT.QZNN_GAMESCENE_SCORE, data);
    ...
    onGameSceneSelect(data: XPQznn.NN_MSG_GS_SELCARD) {
        if (this._battleplayer == null) {
            this._dataSceneTemp = data;
            this._sceneNum = 2
            this._isWaitGameServerMsg = true;
        ...
        this.emit(UQZNNHelper.QZNN_EVENT.QZNN_GAMESCENE_SELECT, data);
    ...
    onGameSceneOpen(data: XPQznn.NN_MSG_GS_OPEN) {
        ...
        this.setAllSceneStatus(MXPQZNNModel.QZNN_GAMESTATUS.OPEN, EQZNNState.Gameing, cbPlayStatus);

        this.emit(UQZNNHelper.QZNN_EVENT.QZNN_GAMESCENE_OPEN, data);
    ...
    private onUserEnterNotify(data: any) {
        ...
        // this.emit(UQZNNHelper.QZNN_SELF_EVENT.QZNN_UPDATE_PLAYERS_EVENT, this._roomPlayers);
    }
    ...
    /**玩家离开房间 */
    private sc_ts_player_left_room(data: GameServer.MSG_C2S_UserLeftMessageResponse) {
        UDebug.Log("玩家离开房间  sc_ts_player_left_room:" + JSON.stringify(data));
        this._battleplayer[data.userId] = null;
        if (data.retCode == 0) {
            ...
            if (data.type == ELeftType.ReturnToRoom) {
                ...
                this.emit(UQZNNHelper.QZNN_SELF_EVENT.QZNN_SC_TS_CANCLE_MATCH, true);
                this._state = EQZNNState.Wait;
                ...
                this.emit(UQZNNHelper.QZNN_SELF_EVENT.QZNN_SC_TS_START_MATCH, true);
            }

        }
        // else if (data.retCode == 2) {
        //     this.emit(UQZNNHelper.QZNN_SELF_EVENT.QZNN_CONTINUE_ACTIVE, true);
        // }
        else {
            if (data.type == ELeftType.ReturnToRoom) {
                this._retrunLobby = false;
                ...
                if (this._state != EQZNNState.Gameing) {
                    this.emit(UQZNNHelper.QZNN_SELF_EVENT.QZNN_SC_TS_CANCLE_MATCH, false);
                }
                ...
                this.emit(UQZNNHelper.QZNN_SELF_EVENT.QZNN_SC_TS_START_MATCH, false);
            }
    ...
    /**游戏开始 */
    onGameStart(data: XPQznn.NN_CMD_S_GameStart) {
        ...
        this.emit(UQZNNHelper.QZNN_EVENT.QZNN_GAME_START, data);
        this.emit(UQZNNHelper.QZNN_SELF_EVENT.QZNN_UPDATE_PLAYERS_EVENT, this._battleplayer);
    ...
    onCallBanker(data: any) {
        this.emit(UQZNNHelper.QZNN_EVENT.QZNN_CALL_BANKER, data);
        this._state = EQZNNState.Gameing;
    ...
    onCallBankerResult(data: any) {
        this.emit(UQZNNHelper.QZNN_EVENT.QZNN_CALL_BANKER_RESULT, data);
        this._state = EQZNNState.Gameing;
    ...
    onAddScoreResult(data: any) {  // private
        //    var addJettonUser = data.addJettonUser
        //    var jettonMultiple = data.jettonMultiple

        this.emit(UQZNNHelper.QZNN_EVENT.QZNN_ADD_SCORE_RESULT, data);
        this._state = EQZNNState.Gameing;

    }

    /**发牌消息 */
    onSendCard(data: XPQznn.NN_CMD_S_SendCard) {
        UDebug.log("发牌消息"+ data)
        this.emit(UQZNNHelper.QZNN_EVENT.QZNN_SEND_CARD, data);
        this._state = EQZNNState.Gameing;
    ...
    onOpenCardResult(data: any) {
        this.emit(UQZNNHelper.QZNN_EVENT.QZNN_OPEN_CARD_RESULT, data);
        this._state = EQZNNState.Gameing;

    }

    // 接收选牌
    onSelectCard(data: XPQznn.NN_CMD_S_SendSelCard) {
        this.emit(UQZNNHelper.QZNN_SELF_EVENT.QZNN_SELECT_CARD, data);
        this.emit(UQZNNHelper.QZNN_EVENT.QZNN_SEND_SELECT_CARD, data);
        this._state = EQZNNState.Gameing;
    }

    //选牌确认
    onSureSelectCard(data: XPQznn.NN_CMD_S_SelCardResult) {
        this.emit(UQZNNHelper.QZNN_SELF_EVENT.QZNN_SELECT_CARD_OK, data);
        this._state = EQZNNState.Gameing;
    }

    /**游戏结束 */
    onGameEnd(data: XPQznn.NN_CMD_S_GameEnd) {
        ...
        this.emit(UQZNNHelper.QZNN_EVENT.QZNN_GAME_END, data);
        this.emit(UQZNNHelper.QZNN_SELF_EVENT.QZNN_DJS_EVENT, 0);
    ...
    onEndExiitResult(data: XPQznn.NN_CMD_C_RoundEndExitResult) {
        // UDebug.log(data);
        this.emit(UQZNNHelper.QZNN_SELF_EVENT.QZNN_NEXT_EXIT, data.bExit);
    ...
    sendAddScore(jetton: number) {
        ...
        this.sendMsg(XPQznn.SUBID.NN_SUB_C_ADD_SCORE, data);
    }
    /**
     * 请求叫庄
     * @param callFlag 
    ...
    sendCallBanker(callFlag: number) {
        ...
        this.sendMsg(XPQznn.SUBID.NN_SUB_C_CALL_BANKER, data);
    }

    /**请求开牌 */
    sendOpenCard() {
        this.sendMsg(XPQznn.SUBID.NN_SUB_C_OPEN_CARD, {});
    }

    /**确定选牌 */
    sendSelectCard(cardIndex: number) {
        var data = {
            cardIndex: cardIndex,
        }
        this.sendMsg(XPQznn.SUBID.NN_SUB_C_SEL_CARD, data);
    }

    sendIsExitGame(isExit: boolean) {
        var data = {
            bExit: isExit,
        }
        this.sendMsg(XPQznn.SUBID.NN_SUB_C_ROUND_END_EXIT, data);
    }
    ...
    public getCardSuit(card: number): number {
        return (card & MXPQZNNModel.LOGIC_MASK.COLOR) >> 4;
    }
    /**获取一副完整的牌 not use*/
    ...
    public getCardValue(card: number): number {
        return card & MXPQZNNModel.LOGIC_MASK.VALUE;
    }
    ...
    /**根据真实玩家位置获取玩家信息 */
    getbattleplayerbyChairId(chairId: number, pl?: any): QZNNBattlePlayerInfo {
    ...
    /**根据ui玩家位置获取玩家信息 */
    getbattleplayerbySeatId(seatId: number, pl?: any): QZNNBattlePlayerInfo {
        let ppp = pl || this._battleplayer;//**将就他原来的写法 */
        for (const key in ppp) {
            if (ppp.hasOwnProperty(key)) {
                const element = ppp[key];
