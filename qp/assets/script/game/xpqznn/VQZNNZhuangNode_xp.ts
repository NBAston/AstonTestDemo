 ...
 */
@ccclass
export default class VXPQZNNZhuangNode extends cc.Component {
    /**
     * 该节点下需用的dic
    ...
    start() {
        ...
        for (let i = 0; i < this._maxplayer; i++) {
            ...
            this._zhuangNodeList[i.toString()].zhuang_Ani.setCompleteListener((event: any) => {
                ...
                AppGame.ins.xpqznnModel.emit(UQZNNHelper.QZNN_SELF_EVENT.QZNN_DJS_EVENT, null, false);
            });
    ...
    setBanker(index: number) {
        AppGame.ins.xpqznnModel.sBankerIndex = index;
    }
    /**获得庄的座位索引 */
    getBankerZhuangGuang(): number {
        var index = AppGame.ins.xpqznnModel.gBankerIndex;
        var bIndex = AppGame.ins.xpqznnModel.getUISeatId(index);
    ...
    showChooseBankerAni(indexs: number[]) {
        ...
        // UDebug.Log("AppGame.ins.xpqznnModel.gBankerIndex:"+AppGame.ins.xpqznnModel.gBankerIndex );

        if (AppGame.ins.xpqznnModel.gBankerIndex == null) return;
        ...
        if (indexs.length > 1) {
            for (let i = 0; i < indexs.length; i++) {
                ...
                let act = cc.sequence(cc.delayTime(i * 0.05), rep, cc.callFunc((node) => {
                    ...
                    // var bIndex = AppGame.ins.xpqznnModel.getUISeatId(AppGame.ins.xpqznnModel.gBankerIndex);
                    if (bIndex == indexs[i]) {
                        ...
                        AppGame.ins.xpqznnModel.emit(UQZNNHelper.QZNN_SELF_EVENT.QZNN_XUAN_ZHUANG_END_EVENT);
        ...
        {
            // var index1 = AppGame.ins.xpqznnModel.gBankerIndex;
            // var bIndex = AppGame.ins.xpqznnModel.getUISeatId(index1);
    ...
    private onRandomZhuang(data: any) {
        ...
        if (data.index != null && data.bankers != null) {
            ...
            // UQZNNScene.ins.getMusic.playZhuang(AppGame.ins.xpqznnModel.getPlayerSexByRealSeat(data.index));
        }
    }
     ...
     * @param data 
     */
    private onCallBankerResult(data:XPQznn.NN_CMD_S_CallBankerResult) {
        ...
        if (cbAddJettonTime != null) {
            ...
            AppGame.ins.xpqznnModel.emit(UQZNNHelper.QZNN_SELF_EVENT.QZNN_DJS_EVENT, djsData, true);
        }
        ...
        if (data.randBanker) {
            ...
            for (let i = 0; i < cbCallBankerUser.length; i++) {
                ...
                if (element > 0) {
                    var index = AppGame.ins.xpqznnModel.getUISeatId(i);
                    dic[index] = element;
