...
import { QZNN_SELF_SEAT } from "./model/MQZNNModel_xp";
import UQZNNScene from "./UQZNNScene_xp";
import UResManager from "../../common/base/UResManager";
import { EIconType } from "../../common/base/UAllenum";
import UQZNNHelper, { QZNNBattlePlayerInfo } from "./UQZNNHelper_xp";
 ...
 */
@ccclass
export default class VXPQZNNPlayersNode extends cc.Component {
    ...
    private addEvent() {
        // AppGame.ins.xpqznnModel.on(UQZNNHelper.QZNN_SELF_EVENT.QZNN_UPDATE_ROLEINFO, this.updateMyRoleInfo, this);
        ...
        AppGame.ins.xpqznnModel.on(UQZNNHelper.QZNN_SELF_EVENT.QZNN_XUAN_ZHUANG_END_EVENT, this.onXuanZhuangEnd, this);
        AppGame.ins.xpqznnModel.on(UQZNNHelper.QZNN_SELF_EVENT.QZNN_TOTALSCORE_EVENT, this.onTotalScore, this);

        AppGame.ins.xpqznnModel.on(UQZNNHelper.QZNN_SELF_EVENT.QZNN_UPDATE_PLAYERS_EVENT, this.onUpdatePlayers, this);
        AppGame.ins.xpqznnModel.on(UQZNNHelper.QZNN_SELF_EVENT.QZNN_SUB_S_SEND_CARD, this.onSendCard, this);
    ...
    private removeEvent() {
        // AppGame.ins.xpqznnModel.off(UQZNNHelper.QZNN_SELF_EVENT.QZNN_UPDATE_ROLEINFO, this.updateMyRoleInfo, this);
        ...
        AppGame.ins.xpqznnModel.off(UQZNNHelper.QZNN_SELF_EVENT.QZNN_SUB_S_SEND_CARD, this.onSendCard, this);

        AppGame.ins.xpqznnModel.off(UQZNNHelper.QZNN_SELF_EVENT.QZNN_XUAN_ZHUANG_END_EVENT, this.onXuanZhuangEnd, this);
        AppGame.ins.xpqznnModel.off(UQZNNHelper.QZNN_SELF_EVENT.QZNN_TOTALSCORE_EVENT, this.onTotalScore, this);

        AppGame.ins.xpqznnModel.off(UQZNNHelper.QZNN_SELF_EVENT.QZNN_UPDATE_PLAYERS_EVENT, this.onUpdatePlayers, this);
    ...
    private onResetScene(data?: any) {
        ...
        this.resetPopPos();
        AppGame.ins.xpqznnModel.updateBattlePlayer();

    }

    onGamestart(data: any) {
        var players = AppGame.ins.xpqznnModel.gBattlePlayer;
        this._myscore = -1;
        if (players != null) {
            for (const key in players) {
                if (players.hasOwnProperty(key)) {
                    const element = players[key];
                    var uipos = AppGame.ins.xpqznnModel.getUISeatId(element.chairId)
                    if (element.playStatus == 1 && uipos != null) {
                        this.setPlayerActiveByIndex(uipos, true);
                        this.setQZBSActive(uipos, false);
                        this.setXZBSActive(uipos, false);
                        this.setQPActive(uipos, false);
                    }
                }
            }
        }
    ...
    setQZBSJettonMultiple(index: number, jettonMultiple: number) {
        ...
        if (jettonMultiple == 0) {
            // str = "n"; // n:不抢  
            this._players[index.toString()].qzbs.node.children[0].active = true;
        }
        else {
            this._players[index.toString()].qzbs.node.children[0].active = false;
    ...
    private onUpdatePlayers(data: any) {
        if (data == null) return;
        UDebug.log("更新房间玩家信息",data)
        for (const key in data) {
            if (data.hasOwnProperty(key)) {
                const element:QZNNBattlePlayerInfo = data[key];

                let chairId = AppGame.ins.xpqznnModel.getUISeatId(element.chairId);
                ...
                //UDebug.log("22222222222222更新房间玩家信息", chairId, element.playStatus)
    ...
    private onAddScoreResult(data) {
        ...
        //#region  test 0-3 
        // AppGame.ins.xpqznnModel.sMeChairId = 0;
        //#endregion

        let addJettonUser = data.addJettonUser;
        let jettonMultiple = data.jettonMultiple;

        let chairId = AppGame.ins.xpqznnModel.getUISeatId(addJettonUser);
        ...
        this.setXZBSJettonMultiple(chairId, jettonMultiple);
    ...
    private onCallBanker(data: XPQznn.NN_CMD_S_CallBanker) {
        ...
        let chairId = AppGame.ins.xpqznnModel.getUISeatId(wCallBankerUser);

        UDebug.Log(wCallBankerUser + "=wCallBankerUser    chairId =" + chairId + "     callbanker:" + JSON.stringify(data));
        this.setQZBSActive(chairId, true);
        this.setQPActive(chairId, true);
        this.setQZBSJettonMultiple(chairId, cbCallMultiple);
        this.setQPAction(chairId, "qz");

        var pos = AppGame.ins.xpqznnModel.getPlayerSexByRealSeat(wCallBankerUser);
        if (cbCallMultiple > 0) {
    ...
    private onXuanZhuangEnd(data) {
        // var bankindex = AppGame.ins.xpqznnModel.gBankerIndex;//data.banker;
        // var index = AppGame.ins.xpqznnModel.getUISeatId(bankindex);
