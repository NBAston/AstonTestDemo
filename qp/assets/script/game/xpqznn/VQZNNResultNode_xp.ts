 ...
 */
@ccclass
export default class VXPQZNNResultNode extends cc.Component {
    /**
     * 4个座位的 输赢分显示节点
     * @param win 正数label
     * @param lose 负数label
    ...
    init() {
        ...
        for (let i = 0; i < this._maxplayer; i++) {
            ...
            this._targetPos.push(lose.node.getPosition());
            let tmp_pos = new cc.Vec2(lose.node.getPosition().x, (lose.node.getPosition().y - this._offestY));
    ...
    /**结束的分数 */
    private showEndScore(data: XPQznn.NN_CMD_S_GameEnd,pl:any) {
        ...
        UDebug.log("players:"+JSON.stringify(players));
     ...
     * @param data 
     */
    private onGameEnd(data:XPQznn.NN_CMD_S_GameEnd,pl) {
            ...
            if (iswin) {
                ...
                this._list[index].label.node.runAction(cc.sequence(cc.moveTo(1.0, this._targetPos[index]), cc.callFunc(() => {
                    ...
                    if (this._totalscore[index] != null) {
                        ...
                        AppGame.ins.xpqznnModel.emit(UQZNNHelper.QZNN_SELF_EVENT.QZNN_TOTALSCORE_EVENT, data);
                    }
                }, this)));
