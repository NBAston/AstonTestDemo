import VQZNNPincardNode from "./VQZNNPincardNode_xp";
import UNodeHelper from "../../common/utility/UNodeHelper";
import AppGame from "../../public/base/AppGame";
import UQZNNScene from "./UQZNNScene_xp";
import MQZNNModel, { QZNN_SELF_SEAT } from "./model/MQZNNModel_xp";
...
@ccclass
export default class VXPQZNNCardsNode extends cc.Component {
    ...
    /**初始化各组件 */
    private init() {
        this._maxplayer = AppGame.ins.xpqznnModel.maxPlayer;
        ...
        UEventHandler.addClick(this._btn_seepai.node, this.node, "VQZNNCardsNode_xp", "sendOpenCard");
        ...
        for (let i = 0; i < 9; i++) {
            ...
            // let key = AppGame.ins.xpqznnModel.cardValueToSpriteKey1(i+1);
            // UDebug.log("看这里------------------------- "+key)
            // this._selectCards[i].spriteFrame = UQZNNScene.ins.getSpriteFrame(key);

            //UEventHandler.addClick(this._selectCards[i].node, null, "VQZNNCardsNode_xp", "selectCard");
        }
    ...
    /**添加事件 */
    private addEvent() {
        AppGame.ins.xpqznnModel.on(UQZNNHelper.QZNN_SELF_EVENT.QZNN_FAPAI_COMPLETE, this.showCardBackByIndex, this);

        AppGame.ins.xpqznnModel.on(UQZNNHelper.QZNN_SELF_EVENT.QZNN_SC_GAMESCENE_OPEN, this.onGameSceneOpen, this);
        AppGame.ins.xpqznnModel.on(UQZNNHelper.QZNN_SELF_EVENT.QZNN_SUB_S_OPEN_CARD_RESULT, this.onOpenCardResult, this);

        AppGame.ins.xpqznnModel.on(UQZNNHelper.QZNN_SELF_EVENT.QZNN_SUB_S_ADD_SCORE_RESULT, this.onAddScoreResult, this);
        AppGame.ins.xpqznnModel.on(UQZNNHelper.QZNN_SELF_EVENT.QZNN_SUB_S_SEND_CARD1, this.onSendCard1, this);

        AppGame.ins.xpqznnModel.on(UQZNNHelper.QZNN_SELF_EVENT.QZNN_RESET_SCENE, this.onResetScene, this);
        // AppGame.ins.xpqznnModel.on(UQZNNHelper.QZNN_SELF_EVENT.QZNN_PINCARD_DONE, this.onPinCardDone, this);

        AppGame.ins.xpqznnModel.on(UQZNNHelper.QZNN_SELF_EVENT.QZNN_SCENE_OPEN_NOT_SEND_CARD, this.openNotSendCard, this);

        //发送选牌
        AppGame.ins.xpqznnModel.on(UQZNNHelper.QZNN_SELF_EVENT.QZNN_SELECT_CARD, this.onSelectCard, this);

        AppGame.ins.xpqznnModel.on(UQZNNHelper.QZNN_SELF_EVENT.QZNN_SELECT_CARD_OK, this.onSureSelectCard, this);

    }
    private removeEvent() {
        AppGame.ins.xpqznnModel.off(UQZNNHelper.QZNN_SELF_EVENT.QZNN_FAPAI_COMPLETE, this.showCardBackByIndex, this);

        AppGame.ins.xpqznnModel.off(UQZNNHelper.QZNN_SELF_EVENT.QZNN_SC_GAMESCENE_OPEN, this.onGameSceneOpen, this);
        AppGame.ins.xpqznnModel.off(UQZNNHelper.QZNN_SELF_EVENT.QZNN_SUB_S_OPEN_CARD_RESULT, this.onOpenCardResult, this);

        AppGame.ins.xpqznnModel.off(UQZNNHelper.QZNN_SELF_EVENT.QZNN_SUB_S_ADD_SCORE_RESULT, this.onAddScoreResult, this);
        AppGame.ins.xpqznnModel.off(UQZNNHelper.QZNN_SELF_EVENT.QZNN_SUB_S_SEND_CARD, this.onSendCard1, this);

        AppGame.ins.xpqznnModel.off(UQZNNHelper.QZNN_SELF_EVENT.QZNN_RESET_SCENE, this.onResetScene, this);
        // AppGame.ins.xpqznnModel.off(UQZNNHelper.QZNN_SELF_EVENT.QZNN_PINCARD_DONE, this.onPinCardDone, this);
        AppGame.ins.xpqznnModel.off(UQZNNHelper.QZNN_SELF_EVENT.QZNN_SCENE_OPEN_NOT_SEND_CARD, this.openNotSendCard, this);


        //发送选牌
        AppGame.ins.xpqznnModel.off(UQZNNHelper.QZNN_SELF_EVENT.QZNN_SELECT_CARD, this.onSelectCard, this);
        AppGame.ins.xpqznnModel.off(UQZNNHelper.QZNN_SELF_EVENT.QZNN_SELECT_CARD_OK, this.onSureSelectCard, this);
    ...
    private onAddScoreResult(data: any) {
        AppGame.ins.xpqznnModel.gameStatus = MQZNNModel.QZNN_GAMESTATUS.SEND;
    }
    /**
     * 开牌场景消息
     * @param data 
     */
    private onGameSceneOpen(data: XPQznn.NN_MSG_GS_OPEN) {


        var cbIsOpenCard = data.isOpenCard;
        //已开牌不显示拼牌界面
        if (cbIsOpenCard != null) {
            if (cbIsOpenCard[AppGame.ins.xpqznnModel.gMeChairId]) {
    ...
    private openNotSendCard(data: any) {
        ...
        for (let i = 0; i < this._maxplayer; i++) {
            ...
            let player = AppGame.ins.xpqznnModel.getbattleplayerbySeatId(i);
     ...
     * @param data 
     */
    private onOpenCardResult(data: XPQznn.NN_CMD_S_OpenCardResult) {
        ...
        var chairId = AppGame.ins.xpqznnModel.getUISeatId(wOpenCardUser);
        ...
        this.showCardTypeAni(chairId, cbCardData, selectCard, cbCardType, cbOxCardData);
     ...
     * @param data 
     */
    private onSendCard1(data: XPQznn.NN_CMD_S_SendCard) {
        ...
        let player = AppGame.ins.xpqznnModel.getbattleplayerbySeatId(0);
    ...
    private sendOpenCard() {
        ...
        AppGame.ins.xpqznnModel.sendOpenCard();
    }
    ...
    private selectCard(e: cc.Event.EventTouch) {
        ...
        AppGame.ins.xpqznnModel.sendSelectCard(idx);
    }
    ...
    private isExitGame(e: cc.Toggle) {
        ...
        AppGame.ins.xpqznnModel.sendIsExitGame(e.isChecked);
    }


    // 接收所有玩家选牌
    onSelectCard(data: XPQznn.NN_CMD_S_SendSelCard, selectedCard: number[]) {
        this._xpcardNode.active = true
        let d = data.toBeSelCardsData
        //let chairId = AppGame.ins.xpqznnModel.gMeChairId
        ...
        for (let j = 0; j < this._selectCards.length; j++) {
            for (let i = 0; i < 9; i++) {
                ...
                //var pos = AppGame.ins.xpqznnModel.getPlayerSexByRealSeat(j);
                var chairId = AppGame.ins.xpqznnModel.getUISeatId(j);
                ...
                if (cardNumber != 0) {
                    key = AppGame.ins.xpqznnModel.cardValueToSpriteKey1(cardNumber);
                } else {
                ...
                if (selectedCard) {
                    if (cardNumber == selectedCard[j]) {
                        let d = new XPQznn.NN_CMD_S_SelCardResult()
                        d.selCardUser = j
                        d.cardIndex = i
                        d.card = cardNumber
        ...
        if (selectedCard) {
            for (let i = 0; i < selectedCard.length; i++) {
                ...
                var chairId = AppGame.ins.xpqznnModel.getUISeatId(i);
                ...
                if (cardNumber == 0) {
                    ...
                    let key = AppGame.ins.xpqznnModel.cardValueToSpriteKey1(cardNumber);
    ...
    onSureSelectCard(data: XPQznn.NN_CMD_S_SelCardResult) {
        ...
        let chairId = AppGame.ins.xpqznnModel.getUISeatId(data.selCardUser)//选牌玩家
        ...
        let k = AppGame.ins.xpqznnModel.cardValueToSpriteKey1(data.card)
        this._cardsList[chairId.toString()]["card5"].spriteFrame = UQZNNScene.ins.getSpriteFrame(k);
    ...
    private setCardsSprite(index: number, values: number[], selectCard: number): void {
        ...
        for (let i = 0; i < 5; i++) {
            ...
            let key = AppGame.ins.xpqznnModel.cardValueToSpriteKey1(values[i] || 1);
            cards[path].spriteFrame = UQZNNScene.ins.getSpriteFrame(key);
            cards[path].node.opacity = 255
        }

        let k = AppGame.ins.xpqznnModel.cardValueToSpriteKey1(selectCard)
        cards["card5"].spriteFrame = UQZNNScene.ins.getSpriteFrame(k);
        UDebug.Log(selectCard + " 看这里 ------------------- card5:   " + k);
    ...
    setSpineCardType(index: number, cardtype: number): void {
        ...
        aniName = UQZNNHelper.CARD_TYPE2[cardtype];
        ...
        UQZNNScene.ins.getMusic.playPaiXing(cardtype, AppGame.ins.xpqznnModel.getPlayerSexByUISeat(index));
    ...
    showCardTypeAni(index: number, cbCardData: number[], selectCard: number, cbCardType?: number, cbOxCardData?: number[]) {
        ...
        // AppGame.ins.xpqznnModel.sortCardList(cbCardData,cbCardData.length);
        ...
        if (cbCardType != null) {
            ...
            cardtype = AppGame.ins.xpqznnModel.getCardType(cbCardData, cbCardData.length);
        }
        ...
        if (cbOxCardData != null && cbOxCardData.length == 5) {
            ...
            if (niuniuTemp.length == 5) {
                ...
                AppGame.ins.xpqznnModel.getOxCard(cbCardData);
            }
            ...
            var niuniuCard = AppGame.ins.xpqznnModel.getOxCard(cbCardData);
