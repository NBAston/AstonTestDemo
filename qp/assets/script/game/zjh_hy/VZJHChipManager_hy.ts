import VZJHChipItem_hy from "./VZJHChipItem_hy";
 ...
 * 筹码的控制类
 */
export default class VZJHChipManager_hy {
    /**
     * 根节点
     */
     ...
     * 节点池
     */
    private _pool: Array<VZJHChipItem_hy>;
    /**
     * 正在显示的
     */
    private _run: Array<VZJHChipItem_hy>;
     ...
     * 获取实例
     */
    private getInstance(): VZJHChipItem_hy {

        if (this._pool.length > 0) {
            return this._pool.shift();
        }
        let itemObj = cc.instantiate(this._prefab) as cc.Node;
        let item = itemObj.getComponent(VZJHChipItem_hy);
        if (!item) {
            item = itemObj.addComponent(VZJHChipItem_hy);
            item.init();
     ...
     * @param caller 
     */
    private reclaim(caller: VZJHChipItem_hy): void {
    ...
    /**根据objid获取item*/
    private getItem(objId: number): VZJHChipItem_hy {
        let len = this._run.length;
        for (let i = 0; i < len; i++) {
            const element = this._run[i];
