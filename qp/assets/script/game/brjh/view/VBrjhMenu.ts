...
@ccclass
export default class VBrjhMenu extends cc.Component {
    ...
    private onClickExit() {
        ...
        this.hide();
        AppGame.ins.brnnModel.sendLeftGame(this._room_info.gameId, this._room_info.roomId);
    }
    ...
    private onClickJiLv() {
        UAudioManager.ins.playSound("audio_click");
        AppGame.ins.showUI(ECommonUI.LB_Record, EGameType.BRJH);
        this.hide();
    }

    private onClickHelp() {
        UAudioManager.ins.playSound("audio_click");
        AppGame.ins.showUI(ECommonUI.ZJH_Help, {gameType: EGameType.BRJH});
        this.hide();
    }
