...
@ccclass
export default class VBrbjlMenu extends cc.Component {
    ...
    private onClickExit() {
        ...
        AppGame.ins.brbjlModel.sendLeftGame(this._room_info.gameId, this._room_info.roomId);
    ...
    private onClickJiLv() {
        VBrbjlScene.ins._music_mgr.playClick();
        AppGame.ins.showUI(ECommonUI.LB_Record, EGameType.BJL);
        this.hide();
    }

    private onClickHelp() {
        VBrbjlScene.ins._music_mgr.playClick();
        AppGame.ins.showUI(ECommonUI.ZJH_Help, {gameType: EGameType.BJL});
        this.hide();
    }
