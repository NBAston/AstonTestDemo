...
@ccclass
export default class VBrbjlDaluController extends cc.Component {
    ...
    initDaLu() {
        ...
        for (let index = 0; index < temp_data.length; index++) {
            if (temp_data[index]) {
                // UDebug.log("1111111temp_data[index].winArea = "+temp_data[index].winArea);
                // UDebug.log("this.TipSpace111111 = "+this.TipSpace.toString());
   ...
   /**
    ...
    clearLudanData() {
        ...
        this.Scoll_dayanlu.getComponent('VBrbjlXiaoluController').resetData();
        this.Scoll_xiaolu.getComponent('VBrbjlXiaoluController').resetData();
        this.Scoll_yueyoulu.getComponent('VBrbjlXiaoluController').resetData();
    ...
    initArray(){
     ...
     setTip(flag: number,isblink?: boolean){
        ...
        // console.log("初始化----flag == "+flag+" TipSpace[0] = "+this.TipSpace[0]+" this.TipSpace[1]="+this.TipSpace[1]);
    ...
    setDayanluTip(flag: number,isblink?:boolean){
        var js = this.Scoll_dayanlu.getComponent('VBrbjlXiaoluController')
        js.setTip(1,flag,isblink)
    }

    setXiaoluTip(flag: number,isblink?:boolean){
        var js = this.Scoll_xiaolu.getComponent('VBrbjlXiaoluController')
        js.setTip(2,flag,isblink)
    }
    setYueyouluTip(flag: number,isblink?:boolean){
        var js = this.Scoll_yueyoulu.getComponent('VBrbjlXiaoluController')
