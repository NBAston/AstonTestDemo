...
export default class ddz_Library_hy  {
    ...
    private searchHandler(cardType: ICardType, firstCard: number, cardTotal: number, myOriginCards: number[]) {
        ...
        mergers.forEach(ms => {
            if (ms.count >= cardType.pairCount && ms.origins.length <= 3) {
                var pairCount = ms.count
                //主牌是飞机时不能带三张相同的副牌
                if (pairCount == cardType.cardCount && cardType.cardCount == 3){
                    pairCount = 2
                }
                ...
                if (!main.bigger){
                    ...
                    for (let p = 0; p < pairCards.length; p++) {
                        ...
                        if (pairCount < itemCount ){
                            if (mainValues.includes(pair.value)) continue 
     ...
     * 0 对王 ， 1 四条2 ，其他-1
     * */
    public CheckMustGrab() {
        let handPokers = AppGame.ins.ddzModel_hy.handPokers;
        if(handPokers.indexOf(52) != -1 && handPokers.indexOf(53) != -1){
            return 0;
        }
        if(handPokers.indexOf(12) != -1 && handPokers.indexOf(25) != -1 && handPokers.indexOf(38) != -1 && handPokers.indexOf(51) != -1){
            return 1;
        }
        return -1;
    }
