...
export default class pdk_Library  {
    ...
    private searchHandler(cardType: ICardType, firstCard: number, cardTotal: number, myOriginCards: number[]) {
        ...
        //AAA是否是炸弹
        var bThreeAIsBomb = AppGame.ins.fPdkModel.gameCfgInfo ? AppGame.ins.fPdkModel.gameCfgInfo["ThreeAIsBomb"] : true
        //是否不能分割炸弹
        var bNotAllowedSeparateBomb = AppGame.ins.fPdkModel.gameCfgInfo ? AppGame.ins.fPdkModel.gameCfgInfo["NotAllowedSeparateBomb"] : false
        mergers.forEach(ms => {
            if (ms.value > firstCard && ms.count >= cardType.cardCount){
                //不能分割炸弹
                if (bNotAllowedSeparateBomb){
                    cardType.canSplit = cardType.canSplit.filter(item => item != 4)
                }
                if (ms.count === cardType.cardCount || cardType.canSplit.includes(ms.count)) {
                    if (!(bThreeAIsBomb && ms.count == 3 && ms.value == 11 && cardType.cardCount < 3)){
                        // 如果牌型是连续的
                        if(cardType.inarow) {
                            // 过滤掉 2和大小王
                            if (ms.value < 12) {
                            ...
                            if (cardType.cardTotal > cardType.cardCount) {
                                ...
                                if (!(bThreeAIsBomb && ms.count == 3 && ms.value == 11 && cardType.bewrite == "三张")){
                                    mainCards.push(ms)
                                }
                            }
                        }
        ...
        for (var i = 0; i < mergers.length; i++){
            var ms = mergers[i]
            //炸弹不可拆
            if (bNotAllowedSeparateBomb && ms.count == 4) continue
            //3A不可拆
            if (bNotAllowedSeparateBomb && bThreeAIsBomb && ms.count == 3 && ms.value == 11) continue
            if (ms.count >= 1) {
        ...
        // AAA排到最后
        var index = mainCards.findIndex(main => (main.bigger == true &&  main.value == 11))
        if (index !== -1) {
            mainCards.push(mainCards.splice(index, 1)[0])
        }
