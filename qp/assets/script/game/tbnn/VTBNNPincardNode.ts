import VTBNNPinCard from "./VTBNNPinCard";
 ...
 */
@ccclass
export default class VTBNNPinCardNode extends cc.Component {
    ...
    /**挂在5张牌的脚本 数组 */
    private _pingcardsCom: Array<VTBNNPinCard> = [];
    ...
    init(): void {
        for (let i = 0; i < this.pincards.length; i++) {
            let cardCom = this.pincards[i].getComponent(VTBNNPinCard);
            cardCom.setIndex = i;
        ...
        UEventHandler.addClick(this._btn_YN.node, this.node, "VTBNNPinCardNode", "onBtnYNClick");
        UEventHandler.addClick(this._btn_MN.node, this.node, "VTBNNPinCardNode", "onBtnMNClick");
    ...
    addEvent() {
        //监听分发的牌
        var event1 = MTBNNModel.ins.on(UTBNNHelper.TBNN_SELF_EVENT.TBNN_PINPAI_ADD, this.addPinCardEvent, this);
        var event2 = MTBNNModel.ins.on(UTBNNHelper.TBNN_SELF_EVENT.TBNN_PINPAI_DEL, this.delPinCardEvent, this);
    ...
    removeEvent() {
        ...
        //     // MTBNNModel.ins.off(this._eventList.pop());
        // }

        MTBNNModel.ins.off(UTBNNHelper.TBNN_SELF_EVENT.TBNN_PINPAI_ADD, this.addPinCardEvent, this);
        MTBNNModel.ins.off(UTBNNHelper.TBNN_SELF_EVENT.TBNN_PINPAI_DEL, this.delPinCardEvent, this);
    }
    ...
    setCardValue(arr: Array<number>) {
        ...
        for (let i = 0; i < arr.length; i++) {
            const element = arr[i];
            var key = MTBNNModel.ins.cardValueToSpriteKey1(element);
            // UDebug.log(key);
            this.pincards[i].spriteFrame = UTBNNScene.ins.getSpriteFrame(key);
    ...
    private onBtnYNClick(event) {
        ...
        // MTBNNModel.ins.emit(UTBNNHelper.TBNN_SELF_EVENT.TBNN_PINCARD_DONE);

        this.hide();
    }
    /**无牛按钮点击事件 */
    private onBtnMNClick(event) {
        var isYouNiu = MTBNNModel.ins.getOxCard(this._cardData);
    ...
    private getPinNum(num: number): number {
        let value = MTBNNModel.ins.getCardValue(num);
        let value1 = MTBNNModel.ins.getCardTrueValue(value);
        // UDebug.log(value1);
        return value1;
    }
    ...
    testGetCardType() {
        ...
        let type1 = MTBNNModel.CARDTYPE_NAME[MTBNNModel.ins.getCardType(card1, card1.length)];
        UDebug.log("牌型 :" + type1 + " 牌值" + card1);

        let card2 = [0x2C, 0x2C, 0x3D, 0x2B, 0x1D];//五花牛
        let type2 = MTBNNModel.CARDTYPE_NAME[MTBNNModel.ins.getCardType(card2, card2.length)];
        UDebug.log("牌型 :" + type2 + " 牌值" + card2);

        let card3 = [0x2B, 0x3D, 0x3D, 0x3B, 0x1A];//四花牛
        let type3 = MTBNNModel.CARDTYPE_NAME[MTBNNModel.ins.getCardType(card3, card3.length)];
        UDebug.log("牌型 :" + type3 + " 牌值" + card3);

        let card4 = [0x07, 0x1D, 0x0D, 0x3D, 0x2D];//炸弹
        let type4 = MTBNNModel.CARDTYPE_NAME[MTBNNModel.ins.getCardType(card4, card4.length)];
        UDebug.log("牌型 :" + type4 + " 牌值" + card4);

        let card5 = [0x32, 0x12, 0x11, 0x33, 0x21];//五小牛
        let type5 = MTBNNModel.CARDTYPE_NAME[MTBNNModel.ins.getCardType(card5, card5.length)];
