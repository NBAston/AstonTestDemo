 ...
 */
@ccclass
export default class VTBNNAniNode extends cc.Component {
    /**赢家框 */
    ...
    private addEvent() {
        // this._eventTongsha = MTBNNModel.ins.on(UTBNNHelper.TBNN_SELF_EVENT.TBNN_TONGSHA_EVENT, this.onTongSha, this);
        // this._eventTongpei = MTBNNModel.ins.on(UTBNNHelper.TBNN_SELF_EVENT.TBNN_TONGPEI_EVENT, this.onTongPei, this);

        MTBNNModel.ins.on(UTBNNHelper.TBNN_SELF_EVENT.TBNN_SUB_S_GAME_START, this.showGameStart, this);
        ...
        MTBNNModel.ins.on(UTBNNHelper.TBNN_SELF_EVENT.TBNN_TOTALSCORE_EVENT, this.onTotalScore, this);//分数滚动完事件
        MTBNNModel.ins.on(UTBNNHelper.TBNN_SELF_EVENT.TBNN_RESET_SCENE, this.onResetScene, this);//重置场景事件


    }

    private removeEvent() {
        MTBNNModel.ins.off(UTBNNHelper.TBNN_SELF_EVENT.TBNN_SUB_S_GAME_START, this.showGameStart, this);
    ...
    private playCoinFlyAni(start: number, end: number) {
        ...
        /**播10次声音 */
        //for (let j = 0; j < 5; j++) {
        //  UTBNNScene.ins.getMusic.playflyCoin();
        //}
    ...
    private showGameStart(data: any) {
        ...
        setTimeout(() => {
            if(AppGame.ins.appStatus.status == EAppStatus.Game && !UTBNNScene.ins.fromeDisconnect)
            MTBNNModel.ins.emit(UTBNNHelper.TBNN_SELF_EVENT.TBNN_START_ANI_COMPLETE)
        }, 3500);
        setTimeout(() => {
            if(AppGame.ins.appStatus.status == EAppStatus.Game && !UTBNNScene.ins.fromeDisconnect)
            MTBNNModel.ins.emit(UTBNNHelper.TBNN_SELF_EVENT.TBNN_SUB_S_SEND_CARD);
        }, 2000);
    }
    ...
    private onGameEnd(data: any,pl:any) {
        ...
        if (dLWScore != null) {
            ...
            var banker = MTBNNModel.ins.gBankerIndex;
            var bankerLocalPos = MTBNNModel.ins.getUISeatId(banker);
            ...
            for (let i = 0; i < dLWScore.length; i++) {
                let player = MTBNNModel.ins.getbattleplayerbyChairId(i,pl)
                if (player != null && player.playStatus == 1 && player.seatId >= 0 && player.seatId < 4) {
                    let score = dLWScore[i]
                    let sead_id = MTBNNModel.ins.getUISeatId(i)
                    if (score > 0) {
                        dindex = sead_id
                    } else {
            ...
            //             let player = MTBNNModel.ins.getbattleplayerbyChairId(j);
            //             if (player != null && player.playStatus == 1 && player.seatId >= 0 && player.seatId < 4) {

            //                 let index1 = MTBNNModel.ins.getUISeatId(j);
            ...
            //                 let index2 = MTBNNModel.ins.getUISeatId(j);
            ...
            //         let player = MTBNNModel.ins.getbattleplayerbyChairId(i);
            //         if (player != null && player.playStatus == 1 && player.seatId >= 0 && player.seatId < 4) {
    ...
    onTotalScore(data: any) {
        ...
        if (this._canplay) {
            //中途加入玩家，不显示输赢动画
            if (MTBNNModel.ins.gBattlePlayer && MTBNNModel.ins.gBattlePlayer[AppGame.ins.roleModel.useId]) {
    ...
    sendFapaiCompleteEvent(index: number) {
        ...
        MTBNNModel.ins.emit(UTBNNHelper.TBNN_SELF_EVENT.TBNN_FAPAI_COMPLETE, index);
    ...
    private setFaPaiAni(index: number, b?: boolean) {
        ...
        if (b) {
            for (let j = 0; j < 5; j++) {
                ...
                // UTBNNScene.ins.getMusic.playSendCard();
            }
        }
    }
    ...
    moveCardTo(node: cc.Node, playerIndex: number, cardIndex: number) {
        ...
        var seq = cc.sequence(cc.delayTime(dt_time),
            cc.spawn(
                ...
                cc.scaleTo(0.1, desc_scalex, desc_scaley),
                cc.moveTo(0.2, self.cardPos[playerIndex.toString()][cardIndex.toString()])
            ),
            cc.callFunc(() => {
    ...
    setGameStartAniActive(b: boolean) {
        ...
        if (b == true) {
            ...
            UTBNNScene.ins.getMusic.playStart();
        }
    }
    /**庄家通杀动画 */
