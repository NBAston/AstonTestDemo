 ...
 */
@ccclass
export default class VTBNNResultNode extends cc.Component {
    /**
     * 4个座位的 输赢分显示节点
     * @param win 正数label
     * @param lose 负数label
    ...
    onGameSceneEnd(data:any){
        this.bfromScene = true
        this.showEndScore(data.gameEnd)
    }
    ...
    scoreAdd(playerIndex: number, score1: number) {
        ...
        if (this._list){
            ...
            this.setScoreFunc(playerIndex, isWin, !this.bfromScene);
        }
            ...
            if (iswin) {
                ...
                this._list[index].label.node.runAction(cc.sequence(cc.moveTo(1.0, this._targetPos[index]), cc.callFunc(() => {
                    ...
                    if (this._totalscore[index] != null) {
                        ...
                        MTBNNModel.ins.emit(UTBNNHelper.TBNN_SELF_EVENT.TBNN_TOTALSCORE_EVENT, data);
                    }
                }, this)));
