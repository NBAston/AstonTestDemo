...
import UTBNNHelper, { ETBNNState, TBNNBattlePlayerInfo } from "../UTBNNHelper";
 ...
 * 作用:qznn 主要逻辑
 */
export default class MTBNNModel extends Model {
    // //大致流程  not use
    // group = {
    //     TBNN_SC_GAMESCENE_FREE: {
    //         on: {
    //             VTBNNMainNode: "显示继续按钮"
    //         }
    //     },
    //     TBNN_SC_GAMESCENE_CALL: {
    //         on: {
    //             VTBNNTBNode: "显示抢庄界面",
    //             VTBNNMainNode: "隐藏继续按钮"
    //         }
    //     },
    //     TBNN_SC_GAMESCENE_SCORE: {
    //         on: {
    //             VTBNNTBNode: "隐藏抢庄界面",
    //             VTBNNXiaZhuNode: "显示下注界面"
    //         }
    //     },
    //     TBNN_SC_GAMESCENE_OPEN: {
    //         on: {
    //             VTBNNXiaZhuNode: "隐藏下注界面",
    //         }
    //     },
    //     ///////scene end//////////////
    //     TBNN_SUB_S_GAME_START: {
    //         on: {
    //             VTBNNMainNode: "倒计时显示/回调事件",
    //             VTBNNAniNode: "开始动画"
    //         }
    //     },
    //     TBNN_SUB_S_CALL_BANKER: {//时间未到的叫庄结果
    //         on: {
    //             VTBNNPlayersNode: "显示点了的玩家的叫庄结果",
    //             VTBNNTBNode: "是自己就隐藏叫庄按钮",
    //         }
    //     },
    //     TBNN_SUB_S_CALL_BANKER_RESULT: {
    //         on: {
    //             VTBNNZhunagNode: "显示点了的玩家的叫庄结果",
    //         }
    //     }
    // }
    ...
    static s_ins: MTBNNModel;

    static get ins(): MTBNNModel {
        if (MTBNNModel.s_ins == null) {
            MTBNNModel.s_ins = new MTBNNModel();
            MTBNNModel.s_ins.init();
        }
        return MTBNNModel.s_ins;
    }
     ...
     * 游戏状态
     */
    public _state: ETBNNState = ETBNNState.Wait;
    ...
    exit(): void {
        ...
        this.unregesterMsg(Tbnn.SUBID.SC_GAMESCENE_END, new UHandler(this.onGameSceneEnd, this));
    ...
    qznnRegest() {
        ...
        this.regesterMsg(Tbnn.SUBID.SC_GAMESCENE_END, new UHandler(this.onGameSceneEnd, this));
    ...
    regesterMsg(subId: number, handler: UHandler) {
        UMsgCenter.ins.regester(EGameType.TBNN,
    ...
    unregesterMsg(subId: number, handler: UHandler) {
        UMsgCenter.ins.unregester(EGameType.TBNN,
            Game.Common.MAINID.MAIN_MESSAGE_CLIENT_TO_GAME_LOGIC,
            subId,
            handler);
    }
    ...
    sendMsg(subId: number, data: any, handler: UHandler = null, unlock: boolean = false) {
        UMsgCenter.ins.sendPkg(EGameType.TBNN,
            Game.Common.MAINID.MAIN_MESSAGE_CLIENT_TO_GAME_LOGIC,
    ...
    //         case MtbnnModel.TBNN_GAMESTATUS.FREE:
    //             {
    //                 this.emit(TBNN_SC_GAMESCENE_FREE, data);
    //             }
    //             break;
    ...
    /**
     * 设置所有场景内玩家状态
     * @param gameStatus MtbnnModel.TBNN_GAMESTATUS状态
     * @param state ETBNNState状态
     * @param cbPlayStatus 各玩家状态
     */
    private setAllSceneStatus(gameStatus: number, state: ETBNNState, cbPlayStatus?: boolean[]) {
        //赋值房间内玩家状态
    ...
    public updateBattlePlayer() {
        ...
        this.emit(UTBNNHelper.TBNN_SELF_EVENT.TBNN_UPDATE_PLAYERS_EVENT, this._battleplayer);
    }
    ...
    private onGameSceneFree(data: any) {
        ...
        this._gameStatus = MTBNNModel.TBNN_GAMESTATUS.FREE;
        ...
        this.emit(UTBNNHelper.TBNN_EVENT.TBNN_GAMESCENE_FREE, data, ishideContinue);
        this.updateBattlePlayer();
    ...
    //     this.setAllSceneStatus(MTBNNModel.TBNN_GAMESTATUS.CALL, ETBNNState.Gameing, cbPlayStatus);
    //     this.emit(UTBNNHelper.TBNN_EVENT.TBNN_GAMESCENE_CALL, data);
    //     this.updateBattlePlayer();
    //     this.emit(UTBNNHelper.TBNN_SELF_EVENT.TBNN_UPDATE_ROOMID, roomId);
    ...
    onGameSceneScore(data: Tbnn.NN_MSG_GS_SCORE) {
        ...
        this.setAllSceneStatus(MTBNNModel.TBNN_GAMESTATUS.SCORE, ETBNNState.Gameing, cbPlayStatus);

        this.emit(UTBNNHelper.TBNN_EVENT.TBNN_GAMESCENE_SCORE, data);
    ...
    onGameSceneOpen(data: Tbnn.NN_MSG_GS_OPEN) {
        ...
        this.setAllSceneStatus(MTBNNModel.TBNN_GAMESTATUS.OPEN, ETBNNState.Gameing, cbPlayStatus);

        this.emit(UTBNNHelper.TBNN_EVENT.TBNN_GAMESCENE_OPEN, data);
     ...
     onGameSceneEnd(data) {
        ...
        this.gameStatus = MTBNNModel.TBNN_GAMESTATUS.OVER
        this._state = ETBNNState.Watching

        this.emit(UTBNNHelper.TBNN_EVENT.TBNN_GAMESCENE_END, data);
    ...
    private onUserEnterNotify(data: any) {
        ...
        // this.emit(UTBNNHelper.TBNN_SELF_EVENT.TBNN_UPDATE_PLAYERS_EVENT, this._roomPlayers);
    }
    ...
    private sc_ts_player_left_room(data: any) {
        if (data.retCode == 0) {
            ...
            if (data.type == ELeftType.ReturnToRoom) {
                ...
                this.emit(UTBNNHelper.TBNN_SELF_EVENT.TBNN_SC_TS_CANCLE_MATCH, true);
                this._state = ETBNNState.Wait;
                ...
                this.emit(UTBNNHelper.TBNN_SELF_EVENT.TBNN_SC_TS_START_MATCH, true);
            }
            ...
            if (data.type == ELeftType.ReturnToRoom) {
                ...
                if (this._state != ETBNNState.Gameing) {
                    this.emit(UTBNNHelper.TBNN_SELF_EVENT.TBNN_SC_TS_CANCLE_MATCH, false);
                }
                ...
                this.emit(UTBNNHelper.TBNN_SELF_EVENT.TBNN_SC_TS_START_MATCH, false);
            }
    ...
    onGameStart(data: any) {
        ...
        this.emit(UTBNNHelper.TBNN_EVENT.TBNN_GAME_START, data);
        this.updateBattlePlayer();
    ...
    //     this.emit(UTBNNHelper.TBNN_EVENT.TBNN_CALL_BANKER, data);
    //     this._state = ETBNNState.Gameing;
    ...
    //     this.emit(UTBNNHelper.TBNN_EVENT.TBNN_CALL_BANKER_RESULT, data);
    //     this._state = ETBNNState.Gameing;
    ...
    onAddScoreResult(data: any) {  // private
        ...
        this.emit(UTBNNHelper.TBNN_EVENT.TBNN_ADD_SCORE_RESULT, data);
        this._state = ETBNNState.Gameing;
    ...
    /**发牌消息 */
    onSendCard(data: any) {
        this.emit(UTBNNHelper.TBNN_EVENT.TBNN_SEND_CARD, data);
        this._state = ETBNNState.Gameing;
    ...
    onOpenCardResult(data: any) {
        this.emit(UTBNNHelper.TBNN_EVENT.TBNN_OPEN_CARD_RESULT, data);
        this._state = ETBNNState.Gameing;
    ...
    onGameEnd(data: any) {
        ...
        this.emit(UTBNNHelper.TBNN_EVENT.TBNN_GAME_END, data);
        this.emit(UTBNNHelper.TBNN_SELF_EVENT.TBNN_DJS_EVENT, 0);
    ...
    onNextExit(data: any) {
        UDebug.log(data);
        this.emit(UTBNNHelper.TBNN_SELF_EVENT.TBNN_NEXT_EXIT, data);
     ...
     */
     sendFreshGameScene() {
        this.sendMsg(Tbnn.SUBID.CS_GAMESCENE_FRESH, {});
    }
    ...
    public getCardSuit(card: number): number {
        return (card & MTBNNModel.LOGIC_MASK.COLOR) >> 4;
    }
    /**获取一副完整的牌 not use*/
    ...
    public getCardValue(card: number): number {
        return card & MTBNNModel.LOGIC_MASK.VALUE;
    }
    ...
    game_show(){
        //清空上局场景
        this.emit(UTBNNHelper.TBNN_SELF_EVENT.TBNN_GAME_TO_SHOW);
        //请求最新场景
