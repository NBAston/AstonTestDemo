 ...
 */
@ccclass
export default class VTBNNXiaZhuNode extends cc.Component {
    ...
    private addEvent() {
        ...
        MTBNNModel.ins.on(UTBNNHelper.TBNN_SELF_EVENT.TBNN_SC_GAMESCENE_RELINE,this.onReLine,this)
    ...
    private removeEvent() {
        ...
        MTBNNModel.ins.off(UTBNNHelper.TBNN_SELF_EVENT.TBNN_START_ANI_COMPLETE,this.onStartAniComplete,this)
    	  MTBNNModel.ins.off(UTBNNHelper.TBNN_SELF_EVENT.TBNN_SC_GAMESCENE_RELINE,this.onReLine,this)
	}
    onDestroy(){
        this.removeEvent();
    ...
    updateXiazhuBtns(zhus: number[], length?: number) {
        ...
        for (let i = 0; i < length; i++) {
            ...
            if(zhus[i] > 0) {
                ...
                UEventHandler.addClick(item, this.node, "VTBNNXiaZhuNode", "sendXiaZhuInfo", i);
    ...
    private sendXiaZhuInfo(event, customEventData) {
        ...
        MTBNNModel.ins.sendAddScore(customEventData);
    }
    ...
    private onAddScoreResult(data: any) {
        ...
        // let chairId = MTBNNModel.ins.changeToLocalPos(wAddJettonUser);

        // UDebug.Log("gMeChairId :" + MTBNNModel.ins.gMeChairId);
        // UDebug.Log("wAddJettonUser：" + wAddJettonUser);

        // UDebug.Log("onAddScoreResult:" + JSON.stringify(data));

        if(wAddJettonUser == MTBNNModel.ins.gMeChairId)
        {
            this.setXiaZhuNodeActive(false);
        }
    ...
    private onGameSceneScore(data: Tbnn.NN_MSG_GS_SCORE) {
        if (data.userJettonMultiple[MTBNNModel.ins.gMeChairId] == 0){
            this.setXiaZhuNodeActive(true);
        }
        ...
        //     }
            var djsData = cbTimeLeave;
             MTBNNModel.ins.emit(UTBNNHelper.TBNN_SELF_EVENT.TBNN_DJS_EVENT, djsData);
        // }

        // MTBNNModel.ins.emit(UTBNNHelper.TBNN_SELF_EVENT.TBNN_DJS_EVENT)
    }

    private onXuanZhuangEnd(data:any){
        var dwBankerUser = MTBNNModel.ins.gBankerIndex;
    ...
    {   
        UTBNNScene.ins.getMusic.playKsxz();
        this.setXiaZhuNodeActive(true);
    }
