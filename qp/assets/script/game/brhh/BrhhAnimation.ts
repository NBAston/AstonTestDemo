...
export default class BrhhAnimation {
    ...
    static moveChip(chip: cc.Node, pos: cc.Vec2, funcBack: Function) {
        ...
        let delt_time = 0.3;
        let pos1 = pos;
        let pos2 = chip.getPosition();
        // UDebug.Log('x=' + pos2.x + '   y=' + pos2.y);
        
        let anit_x = (pos2.x - pos1.x) / 15;
        let anit_y = (pos2.y - pos1.y) / 15;

        let del_time = Math.random()*0.1;

        chip.stopAllActions();
        chip.runAction(cc.sequence( cc.delayTime(del_time), cc.moveBy(delt_time * 0.5, anit_x, anit_y), 
            cc.moveTo(delt_time, pos).easing(cc.easeQuadraticActionOut()),
    ...
    static betMove(chip: cc.Node, stop_pos: cc.Vec2, funcBack: Function, dt_time: number = 0) {
        var act_time = 0.2;  // 移动速度
        chip.opacity = 200;

        var disc_scale = 0.5;


        var rote = 360//Math.random() * 250 + 110;
    ...
    */
   static bankerChipMove(chip: cc.Node, stop_pos: cc.Vec2, funcBack?: Function, dt_time:number = 0.3) {
       var act_time = 0.6;
       chip.opacity = 200;
