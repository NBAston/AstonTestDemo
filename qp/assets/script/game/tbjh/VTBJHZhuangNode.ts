 ...
 */
@ccclass
export default class VTBJHZhuangNode extends cc.Component {
    /**
     * 该节点下需用的dic
    ...
    start() {
        ...
        for (let i = 0; i < this._maxplayer; i++) {
            ...
            this._zhuangNodeList[i.toString()].zhuang_Ani.setCompleteListener((event: any) => {
                UTBJHScene.ins.getMusic.playZhuang();
                MTBJHModel.ins.emit(UTBJHHelper.TBJH_SELF_EVENT.TBJH_DJS_EVENT, null, false);
            });
    ...
    setBanker(index: number) {
        MTBJHModel.ins.sBankerIndex = index;
    }
    /**获得庄的座位索引 */
    getBankerZhuangGuang(): number {
        var index = MTBJHModel.ins.gBankerIndex;
        var bIndex = MTBJHModel.ins.getUISeatId(index);
    ...
    showChooseBankerAni(indexs: number[]) {
        ...
        // UDebug.Log("MTBJHModel.ins.gBankerIndex:"+MTBJHModel.ins.gBankerIndex );

        if (MTBJHModel.ins.gBankerIndex == null) return;
        ...
        if (indexs.length > 1) {
            for (let i = 0; i < indexs.length; i++) {
                ...
                let rep = cc.repeat(cc.sequence(
                    ...
                    cc.fadeOut(0.05),
                    cc.callFunc(() => {
                        UTBJHScene.ins.getMusic.playChooseZhuang();
                    })
                ), 4);
                //每个人延迟执行，并加回调
                let act = cc.sequence(cc.delayTime(i * 0.05), rep, cc.callFunc((node) => {

                    // var bIndex = MTBJHModel.ins.getUISeatId(MTBJHModel.ins.gBankerIndex);
                    if (bIndex == indexs[i]) {
                        ...
                        MTBJHModel.ins.emit(UTBJHHelper.TBJH_SELF_EVENT.TBJH_XUAN_ZHUANG_END_EVENT);
        ...
        {
            // var index1 = MTBJHModel.ins.gBankerIndex;
            // var bIndex = MTBJHModel.ins.getUISeatId(index1);
    ...
    private onRandomZhuang(data: any) {
        ...
        if (data.index != null && data.bankers != null) {
            ...
            // UTBJHScene.ins.getMusic.playZhuang(MTBJHModel.ins.getPlayerSexByRealSeat(data.index));
        }
    }
    ...
    private onCallBankerResult(data) {
        ...
        if (cbAddJettonTime != null) {
            ...
            MTBJHModel.ins.emit(UTBJHHelper.TBJH_SELF_EVENT.TBJH_DJS_EVENT, djsData, true);
        }
        MTBJHModel.ins.emit(UTBJHHelper.TBJH_SELF_EVENT.TBJH_XUAN_ZHUANG_END_EVENT);
        return
        ...
        if (cbRandBanker == 1) {
            ...
            for (let i = 0; i < cbCallBankerUser.length; i++) {
                ...
                if (element > 0) {
                    var index = MTBJHModel.ins.getUISeatId(i);
                    dic[index] = element;
