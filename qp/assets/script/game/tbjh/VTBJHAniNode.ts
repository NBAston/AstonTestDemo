 ...
 */
@ccclass
export default class VTBJHAniNode extends cc.Component {
    /**赢家框 */
    ...
    start() {
        ...
        */
        ...
        this._gameStartAni.setCompleteListener((event) => {
            if (!self._isRest) {
                MTBJHModel.ins.emit(UTBJHHelper.TBJH_SELF_EVENT.TBJH_START_ANI_COMPLETE);
            }
            self.setGameStartAniActive(false);
        });
        this.setGameStartAniActive(false);
    ...
    private addEvent() {
        // this._eventTongsha = MTBJHModel.ins.on(UTBJHHelper.TBJH_SELF_EVENT.TBJH_TONGSHA_EVENT, this.onTongSha, this);
        // this._eventTongpei = MTBJHModel.ins.on(UTBJHHelper.TBJH_SELF_EVENT.TBJH_TONGPEI_EVENT, this.onTongPei, this);

        MTBJHModel.ins.on(UTBJHHelper.TBJH_SELF_EVENT.TBJH_SUB_S_GAME_START, this.showGameStart, this);
        ...
        MTBJHModel.ins.on(UTBJHHelper.TBJH_SELF_EVENT.TBJH_TOTALSCORE_EVENT, this.onTotalScore, this);//分数滚动完事件
        MTBJHModel.ins.on(UTBJHHelper.TBJH_SELF_EVENT.TBJH_RESET_SCENE, this.onResetScene, this);//重置场景事件
    ...
    private removeEvent() {
        MTBJHModel.ins.off(UTBJHHelper.TBJH_SELF_EVENT.TBJH_SUB_S_GAME_START, this.showGameStart, this);
        ...
        MTBJHModel.ins.off(UTBJHHelper.TBJH_SELF_EVENT.TBJH_TOTALSCORE_EVENT, this.onTotalScore, this);
    ...
    private playCoinFlyAni(start: number, end: number) {
        ...
        /**播10次声音 */
        //for (let j = 0; j < 5; j++) {
        //  UTBJHScene.ins.getMusic.playflyCoin();
        //}
    ...
    private onSendCard(data: any) {
        ...
        var players = MTBJHModel.ins.gBattlePlayer;
        if (players != null) {
            for (const key in players) {
                if (players.hasOwnProperty(key)) {
                    const element = players[key];
                    var uipos = MTBJHModel.ins.getUISeatId(element.chairId);
    ...
    private onGameEnd(data: any,pl:any) {
        ...
        if (dLWScore != null) {
            ...
            var banker = MTBJHModel.ins.gBankerIndex;
            var bankerLocalPos = MTBJHModel.ins.getUISeatId(banker);
            ...
            for (let i = 0; i < dLWScore.length; i++) {
                let player = MTBJHModel.ins.getbattleplayerbyChairId(i,pl)
                if (player != null && player.playStatus == 1 && player.seatId >= 0 && player.seatId < 4) {
                    let score = dLWScore[i]
                    let sead_id = MTBJHModel.ins.getUISeatId(i)
                    if (score > 0) {
                        dindex = sead_id
                    } else {
            ...
            //             let player = MTBJHModel.ins.getbattleplayerbyChairId(j);
            //             if (player != null && player.playStatus == 1 && player.seatId >= 0 && player.seatId < 4) {

            //                 let index1 = MTBJHModel.ins.getUISeatId(j);
            ...
            //                 let index2 = MTBJHModel.ins.getUISeatId(j);
            ...
            //         let player = MTBJHModel.ins.getbattleplayerbyChairId(i);
            //         if (player != null && player.playStatus == 1 && player.seatId >= 0 && player.seatId < 4) {
    ...
    sendFapaiCompleteEvent(index: number) {
        ...
        MTBJHModel.ins.emit(UTBJHHelper.TBJH_SELF_EVENT.TBJH_FAPAI_COMPLETE, index);
    ...
    private setFaPaiAni(index: number, b?: boolean) {
        ...
        if (b) {
            for (let j = 0; j < 3; j++) {
                ...
                // UTBJHScene.ins.getMusic.playSendCard();
            }
        }
    ...
    setGameStartAniActive(b: boolean) {
        ...
        if (b == true) {
            ...
            UTBJHScene.ins.getMusic.playStart();
        }
    }
    /**庄家通杀动画 */
