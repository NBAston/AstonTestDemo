import MTBJHModel, { TBJH_SELF_SEAT } from "./model/MTBJHModel";
import UNodeHelper from "../../common/utility/UNodeHelper";
import UEventHandler from "../../common/utility/UEventHandler";
import UTBJHHelper from "./UTBJHHelper";
import UDebug from "../../common/utility/UDebug";
import UTBJHScene from "./UTBJHScene";
import VTBJHPincardNode from "./VTBJHPincardNode"
...
@ccclass
export default class VTBJHCardsNode extends cc.Component {

    private _pincardNode: VTBJHPincardNode = null;
    /**
     * 牌组节点下的各节点
     * @param card0-4 :牌
     * @param node 牌组的牌的父节点
    ...
    /**初始化各组件 */
    private init() {
        this._maxplayer = MTBJHModel.ins.maxPlayer;

        this._pincardNode = UNodeHelper.getComponent(this.node, "pincardsNode", VTBJHPincardNode);

        this._btn_seepai = UNodeHelper.getComponent(this.node, "btn_seepai", cc.Button);
        UEventHandler.addClick(this._btn_seepai.node, this.node, "VTBJHCardsNode", "sendOpenCard");
    ...
    /**添加事件 */
    private addEvent() {
        MTBJHModel.ins.on(UTBJHHelper.TBJH_SELF_EVENT.TBJH_FAPAI_COMPLETE, this.showCardBackByIndex, this);

        MTBJHModel.ins.on(UTBJHHelper.TBJH_SELF_EVENT.TBJH_SC_GAMESCENE_OPEN, this.onGameSceneOpen, this);
        MTBJHModel.ins.on(UTBJHHelper.TBJH_SELF_EVENT.TBJH_SUB_S_OPEN_CARD_RESULT, this.onOpenCardResult, this);

        MTBJHModel.ins.on(UTBJHHelper.TBJH_SELF_EVENT.TBJH_SUB_S_ADD_SCORE_RESULT, this.onAddScoreResult, this);
        MTBJHModel.ins.on(UTBJHHelper.TBJH_SELF_EVENT.TBJH_SUB_S_SEND_CARD, this.onSendCard, this);

        MTBJHModel.ins.on(UTBJHHelper.TBJH_SELF_EVENT.TBJH_RESET_SCENE, this.onResetScene, this);
        // MTBJHModel.ins.on(UTBJHHelper.TBJH_SELF_EVENT.TBJH_PINCARD_DONE, this.onPinCardDone, this);

        MTBJHModel.ins.on(UTBJHHelper.TBJH_SELF_EVENT.TBJH_SCENE_OPEN_NOT_SEND_CARD, this.openNotSendCard, this);


    }
    private removeEvent() {
        MTBJHModel.ins.off(UTBJHHelper.TBJH_SELF_EVENT.TBJH_FAPAI_COMPLETE, this.showCardBackByIndex, this);

        MTBJHModel.ins.off(UTBJHHelper.TBJH_SELF_EVENT.TBJH_SC_GAMESCENE_OPEN, this.onGameSceneOpen, this);
        MTBJHModel.ins.off(UTBJHHelper.TBJH_SELF_EVENT.TBJH_SUB_S_OPEN_CARD_RESULT, this.onOpenCardResult, this);

        MTBJHModel.ins.off(UTBJHHelper.TBJH_SELF_EVENT.TBJH_SUB_S_ADD_SCORE_RESULT, this.onAddScoreResult, this);
        MTBJHModel.ins.off(UTBJHHelper.TBJH_SELF_EVENT.TBJH_SUB_S_SEND_CARD, this.onSendCard, this);

        MTBJHModel.ins.off(UTBJHHelper.TBJH_SELF_EVENT.TBJH_RESET_SCENE, this.onResetScene, this);
        // MTBJHModel.ins.off(UTBJHHelper.TBJH_SELF_EVENT.TBJH_PINCARD_DONE, this.onPinCardDone, this);
        MTBJHModel.ins.off(UTBJHHelper.TBJH_SELF_EVENT.TBJH_SCENE_OPEN_NOT_SEND_CARD, this.openNotSendCard, this);
    ...
    private onAddScoreResult(data: any) {
        MTBJHModel.ins.gameStatus = MTBJHModel.TBJH_GAMESTATUS.SEND;
    }
    /**
     * 开牌场景消息
     * @param data 
    ...
    private onGameSceneOpen(data: any) {
        ...
        if (cbIsOpenCard != null) {
            if (cbIsOpenCard[MTBJHModel.ins.gMeChairId] == 1) {
    ...
    private openNotSendCard(data: any) {
        ...
        for (let i = 0; i < this._maxplayer; i++) {
            ...
            let player = MTBJHModel.ins.getbattleplayerbySeatId(i);
    ...
    private onOpenCardResult(data: Tbnn.NN_CMD_S_OpenCardResult) {
        ...
        var chairId = MTBJHModel.ins.getUISeatId(wOpenCardUser);

        if (chairId == TBJH_SELF_SEAT) {
    ...
    private showCardBack(index: number) {
        ...
        for (let i = 0; i < CARD_COUNT; i++) {
            ...
            let key = "poker_b1";
            this._cardsList[index.toString()][path].spriteFrame = UTBJHScene.ins.getSpriteFrame(key);
    ...
    private showCardBackByIndex(index: number) {
        ...
        if (index == 0 && this._cbCardData != null) {
            ...
            let player = MTBJHModel.ins.getbattleplayerbySeatId(0);
            ...
            //MTBJHModel.ins.randCardData(this._cbCardData,this._cbCardData.length);
            ...
            UTBJHScene.ins.getMusic.playQkp();
        }
        else {
            this.showCardBack(index);
        }
    ...
    private setCardsSprite(index: number, values: number[]): void {
        ...
        for (let i = 0; i < CARD_COUNT; i++) {
            ...
            let key = MTBJHModel.ins.cardValueToSpriteKey1(values[i]);

            // UDebug.Log("key:"+JSON.stringify(key)+ "  index:"+index);

            cards[path].spriteFrame = UTBJHScene.ins.getSpriteFrame(key);
        }
    }
    ...
    setSpineCardType(index: number, cardtype: number): void {
        ...
        // }
        // cardtype = 14;
        aniName = UTBJHHelper.CARD_TYPE2[cardtype];
        ...
        UTBJHScene.ins.getMusic.playPaiXing(aniName, MTBJHModel.ins.getPlayerSexByUISeat(index));
    ...
    showCardTypeAni1(index: number, cbCardData: number[], cbCardType?: number, cbOxCardData?: number[]) {
        ...
        // MTBJHModel.ins.sortCardList(cbCardData,cbCardData.length);
        ...
        if (cbCardType != null) {
            ...
            cardtype = MTBJHModel.ins.getCardType(cbCardData, cbCardData.length);
        }
        ...
        if (cbOxCardData != null && cbOxCardData.length == CARD_COUNT) {
            ...
            if (niuniuTemp.length == CARD_COUNT) {
                ...
                MTBJHModel.ins.getOxCard(cbCardData);
            }

        }
        else {
            //找出牛牛牌 cbCardData 的值已排序过
            var niuniuCard = MTBJHModel.ins.getOxCard(cbCardData);
