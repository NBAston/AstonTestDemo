...
import UTBJHHelper, { ETBJHState, TBJHBattlePlayerInfo } from "../UTBJHHelper";
import MBaseGameModel from "../../../public/hall/MBaseGameModel";
import { Tbnn, Game } from "../../../common/cmd/proto";
...
import { MAX_COUNT, OX_FIVE_SMALL, OX_GOLDEN_BULL, OX_BOMB, OX_SILVERY_BULL, OX_VALUE0 } from "../../xpqznn/UQZNNHelper_xp";



/**qznn 自己ui的位置 */
export const TBJH_SELF_SEAT = 0;

/**
 * 创建:dz
 * 作用:qznn 主要逻辑
 */
export default class MTBJHModel extends Model {
    // //大致流程  not use
    // group = {
    //     TBJH_SC_GAMESCENE_FREE: {
    //         on: {
    //             VTBJHMainNode: "显示继续按钮"
    //         }
    //     },
    //     TBJH_SC_GAMESCENE_CALL: {
    //         on: {
    //             VTBJHTBNode: "显示抢庄界面",
    //             VTBJHMainNode: "隐藏继续按钮"
    //         }
    //     },
    //     TBJH_SC_GAMESCENE_SCORE: {
    //         on: {
    //             VTBJHTBNode: "隐藏抢庄界面",
    //             VTBJHXiaZhuNode: "显示下注界面"
    //         }
    //     },
    //     TBJH_SC_GAMESCENE_OPEN: {
    //         on: {
    //             VTBJHXiaZhuNode: "隐藏下注界面",
    //         }
    //     },
    //     ///////scene end//////////////
    //     TBJH_SUB_S_GAME_START: {
    //         on: {
    //             VTBJHMainNode: "倒计时显示/回调事件",
    //             VTBJHAniNode: "开始动画"
    //         }
    //     },
    //     TBJH_SUB_S_CALL_BANKER: {//时间未到的叫庄结果
    //         on: {
    //             VTBJHPlayersNode: "显示点了的玩家的叫庄结果",
    //             VTBJHTBNode: "是自己就隐藏叫庄按钮",
    //         }
    //     },
    //     TBJH_SUB_S_CALL_BANKER_RESULT: {
    //         on: {
    //             VTBJHZhunagNode: "显示点了的玩家的叫庄结果",
    //         }
    //     }
    // }


    static TBJH_GAMESTATUS = {
        /**空闲状态/结算 */
        FREE: 0,
        /**匹配状态 */
        MATCH: 1,
    ...
    static s_ins: MTBJHModel;

    static get ins(): MTBJHModel {
        if (MTBJHModel.s_ins == null) {
            MTBJHModel.s_ins = new MTBJHModel();
            MTBJHModel.s_ins.init();
        }
        return MTBJHModel.s_ins;
    }
     ...
     * 游戏状态
     */
    private _state: ETBJHState = ETBJHState.Wait;

    /**本局的玩家 */
    private _battleplayer: { [key: number]: TBJHBattlePlayerInfo };

    get gBattlePlayer(): { [key: number]: TBJHBattlePlayerInfo } {
        return this._battleplayer;
    }
    /**
     * 游戏结束的时候copy一份用，因为代码里面延迟执行了 当延迟的时候会造成数据的不一致
     */
    get CopyBattlePlayer():{ [key: number]: TBJHBattlePlayerInfo }{
        let data={};
    ...
    exit(): void {
        ...
        this.unregesterMsg(Tbnn.SUBID.SC_GAMESCENE_FREE, new UHandler(this.onGameSceneFree, this));
        //this.regesterMsg(Tbnn.SUBID.SC_GAMESCENE_CALL, new UHandler(this.onGameSceneCall, this)); //叫庄场景
        this.unregesterMsg(Tbnn.SUBID.SC_GAMESCENE_SCORE, new UHandler(this.onGameSceneScore, this));
        this.unregesterMsg(Tbnn.SUBID.SC_GAMESCENE_OPEN, new UHandler(this.onGameSceneOpen, this));
        // this.regesterMsg(qznn.SUBID.SC_GAMESCENE_END, new UHandler(this.onGameSceneEnd, this));

        this.unregesterMsg(Tbnn.SUBID.NN_SUB_S_ADD_SCORE_RESULT, new UHandler(this.onAddScoreResult, this));
        //this.regesterMsg(Tbnn.SUBID.NN_SUB_S_CALL_BANKER, new UHandler(this.onCallBanker, this)); //叫庄
        //this.regesterMsg(Tbnn.SUBID.NN_SUB_S_CALL_BANKER_RESULT, new UHandler(this.onCallBankerResult, this)); //叫庄结果
        this.unregesterMsg(Tbnn.SUBID.NN_SUB_S_GAME_END, new UHandler(this.onGameEnd, this));
        this.unregesterMsg(Tbnn.SUBID.NN_SUB_S_GAME_START, new UHandler(this.onGameStart, this));
        this.unregesterMsg(Tbnn.SUBID.NN_SUB_S_OPEN_CARD_RESULT, new UHandler(this.onOpenCardResult, this));
        this.unregesterMsg(Tbnn.SUBID.NN_SUB_S_SEND_CARD, new UHandler(this.onSendCard, this));

        this.unregesterMsg(Tbnn.SUBID.NN_SUB_C_ROUND_END_EXIT_RESULT, new UHandler(this.onNextExit , this));
        this._roomInfo = null;
        this._battleplayer = {};
    ...
    qznnRegest() {
        ...
        this.regesterMsg(Tbnn.SUBID.SC_GAMESCENE_FREE, new UHandler(this.onGameSceneFree, this));
        //this.regesterMsg(Tbnn.SUBID.SC_GAMESCENE_CALL, new UHandler(this.onGameSceneCall, this)); //叫庄场景
        this.regesterMsg(Tbnn.SUBID.SC_GAMESCENE_SCORE, new UHandler(this.onGameSceneScore, this));
        this.regesterMsg(Tbnn.SUBID.SC_GAMESCENE_OPEN, new UHandler(this.onGameSceneOpen, this));
        // this.regesterMsg(qznn.SUBID.SC_GAMESCENE_END, new UHandler(this.onGameSceneEnd, this));

        this.regesterMsg(Tbnn.SUBID.NN_SUB_S_ADD_SCORE_RESULT, new UHandler(this.onAddScoreResult, this));
        //this.regesterMsg(Tbnn.SUBID.NN_SUB_S_CALL_BANKER, new UHandler(this.onCallBanker, this)); //叫庄
        //this.regesterMsg(Tbnn.SUBID.NN_SUB_S_CALL_BANKER_RESULT, new UHandler(this.onCallBankerResult, this)); //叫庄结果
        this.regesterMsg(Tbnn.SUBID.NN_SUB_S_GAME_END, new UHandler(this.onGameEnd, this));
        this.regesterMsg(Tbnn.SUBID.NN_SUB_S_GAME_START, new UHandler(this.onGameStart, this));
        this.regesterMsg(Tbnn.SUBID.NN_SUB_S_OPEN_CARD_RESULT, new UHandler(this.onOpenCardResult, this));
        this.regesterMsg(Tbnn.SUBID.NN_SUB_S_SEND_CARD, new UHandler(this.onSendCard, this));

        this.regesterMsg(Tbnn.SUBID.NN_SUB_C_ROUND_END_EXIT_RESULT, new UHandler(this.onNextExit , this));
    ...
    regesterMsg(subId: number, handler: UHandler) {
        UMsgCenter.ins.regester(EGameType.TBJH,
    ...
    unregesterMsg(subId: number, handler: UHandler) {
        UMsgCenter.ins.unregester(EGameType.TBJH,
            Game.Common.MAINID.MAIN_MESSAGE_CLIENT_TO_GAME_LOGIC,
            subId,
            handler);
    }
    ...
    sendMsg(subId: number, data: any, handler: UHandler = null, unlock: boolean = false) {
        UMsgCenter.ins.sendPkg(EGameType.TBJH,
            Game.Common.MAINID.MAIN_MESSAGE_CLIENT_TO_GAME_LOGIC,
    ...
    //         case MtbnnModel.TBJH_GAMESTATUS.FREE:
    //             {
    //                 this.emit(TBJH_SC_GAMESCENE_FREE, data);
    //             }
    //             break;
    ...
    /**
     * 设置所有场景内玩家状态
     * @param gameStatus MtbnnModel.TBJH_GAMESTATUS状态
     * @param state ETBJHState状态
     * @param cbPlayStatus 各玩家状态
     */
    private setAllSceneStatus(gameStatus: number, state: ETBJHState, cbPlayStatus?: boolean[]) {
        //赋值房间内玩家状态
        ...
        //默认赋状态 旁观 自由人
        this._state = ETBJHState.Watching;
        this._gameStatus = MTBJHModel.TBJH_GAMESTATUS.FREE;
        //如果是上座玩牌玩家，重新赋状态
        if (this._battleplayer[this.selfUserId] != null
            && this._battleplayer[this.selfUserId].playStatus == 1) {
            this._state = state;//ETBJHState.Gameing;
            this._gameStatus = gameStatus;//MtbnnModel.TBJH_GAMESTATUS.CALL;
        }
    }
    /**设置房间内玩家信息 */
    ...
    public updateBattlePlayer() {
        UDebug.Log("更新玩家列表---------------------"+this._battleplayer)
        this.emit(UTBJHHelper.TBJH_SELF_EVENT.TBJH_UPDATE_PLAYERS_EVENT, this._battleplayer);
    }

    private onGameSceneFree(data: any) {

        // this.emit(TBJH_UPDATE_PLAYERS_EVENT, this._roomPlayers);

        if (this._state != ETBJHState.Match) {
            this._state = ETBJHState.Wait;
        }
        ...
        this.emit(UTBJHHelper.TBJH_EVENT.TBJH_GAMESCENE_FREE, data, ishideContinue);
    ...
    //     this.setAllSceneStatus(MTBJHModel.TBJH_GAMESTATUS.CALL, ETBJHState.Gameing, cbPlayStatus);
    //     this.emit(UTBJHHelper.TBJH_EVENT.TBJH_GAMESCENE_CALL, data);
    //     this.updateBattlePlayer();
    //     this.emit(UTBJHHelper.TBJH_SELF_EVENT.TBJH_UPDATE_ROOMID, roomId);
    ...
    onGameSceneScore(data: Tbnn.NN_MSG_GS_SCORE) {
        ...
        this.setAllSceneStatus(MTBJHModel.TBJH_GAMESTATUS.SCORE, ETBJHState.Gameing, cbPlayStatus);

        this.emit(UTBJHHelper.TBJH_EVENT.TBJH_GAMESCENE_SCORE, data);
    ...
    onGameSceneOpen(data: Tbnn.NN_MSG_GS_OPEN) {
        ...
        this.setAllSceneStatus(MTBJHModel.TBJH_GAMESTATUS.OPEN, ETBJHState.Gameing, cbPlayStatus);

        this.emit(UTBJHHelper.TBJH_EVENT.TBJH_GAMESCENE_OPEN, data);
    ...
    // onGameSceneEnd(data) {
    //  this.emit(TBJH_SC_GAMESCENE_END, data);
    // }
    ...
    private onUserEnterNotify(data: any) {
        ...
        // this.emit(UTBJHHelper.TBJH_SELF_EVENT.TBJH_UPDATE_PLAYERS_EVENT, this._roomPlayers);
    }
    ...
    private add_battle_player(element: RoomPlayerInfo): TBJHBattlePlayerInfo {
        if (!this._battleplayer) this._battleplayer = {};
        
        let item = new TBJHBattlePlayerInfo();
    ...
    private sc_ts_player_left_room(data: any) {
        if (data.retCode == 0) {
            ...
            if (data.type == ELeftType.ReturnToRoom) {
                if (this._state != ETBJHState.AlreadyLeft) {
                    this._state = ETBJHState.AlreadyLeft;
                    if (this._roomInfo) {
                ...
                this.emit(UTBJHHelper.TBJH_SELF_EVENT.TBJH_SC_TS_CANCLE_MATCH, true);
                this._state = ETBJHState.Wait;
                ...
                this._state = ETBJHState.Match;

                // AppGame.ins.roomModel.requestMatch();
                this.emit(UTBJHHelper.TBJH_SELF_EVENT.TBJH_SC_TS_START_MATCH, true);
            }
            ...
            if (data.type == ELeftType.ReturnToRoom) {
                ...
                if (this._state != ETBJHState.Gameing) {
                    this.emit(UTBJHHelper.TBJH_SELF_EVENT.TBJH_SC_TS_CANCLE_MATCH, false);
                }
                ...
                this.emit(UTBJHHelper.TBJH_SELF_EVENT.TBJH_SC_TS_START_MATCH, false);
            }
    ...
    saveRoomInfo(data: RoomInfo): void {
        ...
        cc.log("this._roomInfo:" + JSON.stringify(this._roomInfo));
    }
    ...
    exitGame(): void {
        ...
        switch (this._state) {
            ...
            case ETBJHState.Gameing:
            case ETBJHState.LeftGame:
                {
                    AppGame.ins.showTips(ULanHelper.SG_CANT_EXIT_GAME);
                }
                break;
            case ETBJHState.Watching:
            // {
            ...
            // }
            // break;
            case ETBJHState.Match:
            case ETBJHState.Wait:
                {
                    if (this._roomInfo ) {
    ...
    requestMatch(): void {
        ...
        if (this._state == ETBJHState.LeftGame) {
            return;
        }
        this._state = ETBJHState.LeftGame;
    ...
    onGameStart(data: any) {
        ...
        this._state = ETBJHState.Gameing;

        this.emit(UTBJHHelper.TBJH_EVENT.TBJH_GAME_START, data);
        this.updateBattlePlayer();
    ...
    //     this.emit(UTBJHHelper.TBJH_EVENT.TBJH_CALL_BANKER, data);
    //     this._state = ETBJHState.Gameing;
    ...
    //     this.emit(UTBJHHelper.TBJH_EVENT.TBJH_CALL_BANKER_RESULT, data);
    //     this._state = ETBJHState.Gameing;
    ...
    onAddScoreResult(data: any) {  // private
        ...
        this.emit(UTBJHHelper.TBJH_EVENT.TBJH_ADD_SCORE_RESULT, data);
        this._state = ETBJHState.Gameing;
    ...
    /**发牌消息 */
    onSendCard(data: any) {
        this.emit(UTBJHHelper.TBJH_EVENT.TBJH_SEND_CARD, data);
        this._state = ETBJHState.Gameing;
    ...
    onOpenCardResult(data: any) {
        this.emit(UTBJHHelper.TBJH_EVENT.TBJH_OPEN_CARD_RESULT, data);
        this._state = ETBJHState.Gameing;
    ...
    onGameEnd(data: any) {
        ...
        this.emit(UTBJHHelper.TBJH_EVENT.TBJH_GAME_END, data);
        this.emit(UTBJHHelper.TBJH_SELF_EVENT.TBJH_DJS_EVENT, 0);
    ...
    /**设置状态 */
    setReconnectState(state: ETBJHState, isRestRoominfo?: boolean) {
        this._state = state;
    ...
    onNextExit(data: any) {
        UDebug.log(data);
        this.emit(UTBJHHelper.TBJH_SELF_EVENT.TBJH_NEXT_EXIT, data);
    ...
    sendAddScore(jetton: number) {
        ...
        this.sendMsg(Tbnn.SUBID.NN_SUB_C_ADD_SCORE, data);
    }
    /**
     * 请求叫庄
     * @param callFlag 
    ...
    sendCallBanker(callFlag: number) {
        ...
        // this.sendMsg(Tbnn.SUBID.NN_SUB_C_CALL_BANKER, data);
    }

    /**请求开牌 */
    sendOpenCard() {
        this.sendMsg(Tbnn.SUBID.NN_SUB_C_OPEN_CARD, {});
    }
    ...
    sendNextExit(b: boolean): void {
        ...
        this.sendMsg(Tbnn.SUBID.NN_SUB_C_ROUND_END_EXIT, data);
    }
    ...
    public getCardSuit(card: number): number {
        return (card & MTBJHModel.LOGIC_MASK.COLOR) >> 4;
    }
    /**获取一副完整的牌 not use*/
    ...
    public getCardValue(card: number): number {
        return card & MTBJHModel.LOGIC_MASK.VALUE;
    }
    ...
    /**根据真实玩家位置获取玩家信息 */
    getbattleplayerbyChairId(chairId: number,pl?:any): TBJHBattlePlayerInfo {
    ...
    /**根据ui玩家位置获取玩家信息 */
    getbattleplayerbySeatId(seatId: number,pl?:any): TBJHBattlePlayerInfo {
        let ppp=pl || this._battleplayer;//**将就他原来的写法 */
        for (const key in ppp) {
            if (ppp.hasOwnProperty(key)) {
                const element = ppp[key];
    ...
    /**获取刚进房间时候 显示自己的信息 */
    getshowselfinfo(): TBJHBattlePlayerInfo {
        let bb = new TBJHBattlePlayerInfo();
        bb.seatId = TBJH_SELF_SEAT;
