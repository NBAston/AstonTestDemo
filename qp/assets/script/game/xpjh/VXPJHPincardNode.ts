...
import MQZNNModel from "./model/MXPJHModel";
import AppGame from "../../public/base/AppGame";
import UNodeHelper from "../../common/utility/UNodeHelper";
import UEventHandler from "../../common/utility/UEventHandler";
import VQZNNPinCard from "./VXPJHPinCard";
 ...
 */
@ccclass
export default class VXPJHPincardNode extends cc.Component {
    ...
    init(): void {
        ...
        UEventHandler.addClick(this._btn_YN.node, this.node, "VXPJHPincardNode", "onBtnYNClick");
        UEventHandler.addClick(this._btn_MN.node, this.node, "VXPJHPincardNode", "onBtnMNClick");
    ...
    addEvent() {
        //监听分发的牌
        var event1 = AppGame.ins.xpqzjhModel.on(UXPJHHelper.QZNN_SELF_EVENT.QZNN_PINPAI_ADD, this.addPinCardEvent, this);
        var event2 = AppGame.ins.xpqzjhModel.on(UXPJHHelper.QZNN_SELF_EVENT.QZNN_PINPAI_DEL, this.delPinCardEvent, this);
    ...
    removeEvent() {
        ...
        //     // AppGame.ins.xpqzjhModel.off(this._eventList.pop());
        // }

        AppGame.ins.xpqzjhModel.off(UXPJHHelper.QZNN_SELF_EVENT.QZNN_PINPAI_ADD, this.addPinCardEvent, this);
        AppGame.ins.xpqzjhModel.off(UXPJHHelper.QZNN_SELF_EVENT.QZNN_PINPAI_DEL, this.delPinCardEvent, this);
    }
    ...
    setCardValue(arr: Array<number>) {
        ...
        for (let i = 0; i < arr.length; i++) {
            const element = arr[i];
            var key = AppGame.ins.xpqzjhModel.cardValueToSpriteKey1(element);
            // UDebug.log(key);
    ...
    private onBtnYNClick(event) {
        ...
        // AppGame.ins.xpqzjhModel.emit(UXPJHHelper.QZNN_SELF_EVENT.QZNN_PINCARD_DONE);

        this.hide();
    }
    /**无牛按钮点击事件 */
    private onBtnMNClick(event) {
        var isYouNiu = AppGame.ins.xpqzjhModel.getOxCard(this._cardData);
    ...
    private getPinNum(num: number): number {
        let value = AppGame.ins.xpqzjhModel.getCardValue(num);
        let value1 = AppGame.ins.xpqzjhModel.getCardTrueValue(value);
        // UDebug.log(value1);
        return value1;
    }
    ...
    testGetCardType() {
        ...
        let type1 = MQZNNModel.CARDTYPE_NAME[AppGame.ins.xpqzjhModel.getCardType(card1, card1.length)];
        UDebug.log("牌型 :" + type1 + " 牌值" + card1);

        let card2 = [0x2C, 0x2C, 0x3D, 0x2B, 0x1D];//五花牛
        let type2 = MQZNNModel.CARDTYPE_NAME[AppGame.ins.xpqzjhModel.getCardType(card2, card2.length)];
        UDebug.log("牌型 :" + type2 + " 牌值" + card2);

        let card3 = [0x2B, 0x3D, 0x3D, 0x3B, 0x1A];//四花牛
        let type3 = MQZNNModel.CARDTYPE_NAME[AppGame.ins.xpqzjhModel.getCardType(card3, card3.length)];
        UDebug.log("牌型 :" + type3 + " 牌值" + card3);

        let card4 = [0x07, 0x1D, 0x0D, 0x3D, 0x2D];//炸弹
        let type4 = MQZNNModel.CARDTYPE_NAME[AppGame.ins.xpqzjhModel.getCardType(card4, card4.length)];
        UDebug.log("牌型 :" + type4 + " 牌值" + card4);

        let card5 = [0x32, 0x12, 0x11, 0x33, 0x21];//五小牛
        let type5 = MQZNNModel.CARDTYPE_NAME[AppGame.ins.xpqzjhModel.getCardType(card5, card5.length)];
