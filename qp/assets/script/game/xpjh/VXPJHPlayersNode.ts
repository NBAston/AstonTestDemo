...
import { QZNN_SELF_SEAT } from "./model/MXPJHModel";
import UQZNNScene from "./UXPJHScene";
import UResManager from "../../common/base/UResManager";
import { EIconType } from "../../common/base/UAllenum";
import UXPJHHelper, { QZNNBattlePlayerInfo } from "./UXPJHHelper";
 ...
 */
@ccclass
export default class VXPJHPlayersNode extends cc.Component {
    ...
    /**发牌时的Y位置 qzbs/xzbs*/
    private _fpPosY: Array<number> = [62, 70, -90.5, 84.1];
    /**抢庄下注时的Y位置 qp*/
    private _qzQpPosY: Array<number> = [50.8, 44, -43.8, 50.8];//-63.8
    /**发牌时的Y位置 qp*/
    private _fpQpPosY: Array<number> = [50.8, 60, -80, 75.7]; // -101.3
    ...
    private addEvent() {
        // AppGame.ins.xpqzjhModel.on(UXPJHHelper.QZNN_SELF_EVENT.QZNN_UPDATE_ROLEINFO, this.updateMyRoleInfo, this);

        AppGame.ins.xpqzjhModel.on(UXPJHHelper.QZNN_SELF_EVENT.QZNN_SUB_S_GAME_START, this.onGamestart, this);
        AppGame.ins.xpqzjhModel.on(UXPJHHelper.QZNN_SELF_EVENT.QZNN_SUB_S_CALL_BANKER, this.onCallBanker, this);
        ...
        AppGame.ins.xpqzjhModel.on(UXPJHHelper.QZNN_SELF_EVENT.QZNN_TOTALSCORE_EVENT, this.onTotalScore, this);

        AppGame.ins.xpqzjhModel.on(UXPJHHelper.QZNN_SELF_EVENT.QZNN_UPDATE_PLAYERS_EVENT, this.onUpdatePlayers, this);
        AppGame.ins.xpqzjhModel.on(UXPJHHelper.QZNN_SELF_EVENT.QZNN_SUB_S_SEND_CARD, this.onSendCard, this);
    ...
    private removeEvent() {
        // AppGame.ins.xpqzjhModel.off(UXPJHHelper.QZNN_SELF_EVENT.QZNN_UPDATE_ROLEINFO, this.updateMyRoleInfo, this);

        AppGame.ins.xpqzjhModel.off(UXPJHHelper.QZNN_SELF_EVENT.QZNN_SUB_S_GAME_START, this.onGamestart, this);
        AppGame.ins.xpqzjhModel.off(UXPJHHelper.QZNN_SELF_EVENT.QZNN_SUB_S_CALL_BANKER, this.onCallBanker, this);
        AppGame.ins.xpqzjhModel.off(UXPJHHelper.QZNN_SELF_EVENT.QZNN_SUB_S_ADD_SCORE_RESULT, this.onAddScoreResult, this);

        AppGame.ins.xpqzjhModel.off(UXPJHHelper.QZNN_SELF_EVENT.QZNN_SUB_S_SEND_CARD, this.onSendCard, this);

        AppGame.ins.xpqzjhModel.off(UXPJHHelper.QZNN_SELF_EVENT.QZNN_XUAN_ZHUANG_END_EVENT, this.onXuanZhuangEnd, this);
        AppGame.ins.xpqzjhModel.off(UXPJHHelper.QZNN_SELF_EVENT.QZNN_TOTALSCORE_EVENT, this.onTotalScore, this);

        AppGame.ins.xpqzjhModel.off(UXPJHHelper.QZNN_SELF_EVENT.QZNN_UPDATE_PLAYERS_EVENT, this.onUpdatePlayers, this);
            ...
            else//自己 就更新信息
            {
                ...
                if (myscore == 0) {
                    if (this._myscore != null && this._myscore != -1) {
                        ...
                        UDebug.Log(`获取的节点钱 ${myscore}`)
                    } else {
                        myscore = AppGame.ins.roleModel.score;
                        UDebug.Log(`获取的角色钱 ${myscore}`)
                    }
                ...
                UDebug.Log(`穿过去的钱 ${myInfo.coin}`)
                this.updateMyRoleInfo(myInfo);
            }
        }
        this.resetPopPos();
        AppGame.ins.xpqzjhModel.updateBattlePlayer();

    }

    onGamestart(data: any) {
        var players = AppGame.ins.xpqzjhModel.gBattlePlayer;
        this._myscore = -1;
        if (players != null) {
            for (const key in players) {
                if (players.hasOwnProperty(key)) {
                    const element = players[key];
                    var uipos = AppGame.ins.xpqzjhModel.getUISeatId(element.chairId)
    ...
    private onUpdatePlayers(data: any) {
        ...
        for (const key in data) {
            if (data.hasOwnProperty(key)) {
                ...
                let chairId = AppGame.ins.xpqzjhModel.getUISeatId(element.chairId);
    ...
    private onAddScoreResult(data) {
        ...
        //#region  test 0-3 
        // AppGame.ins.xpqzjhModel.sMeChairId = 0;
        //#endregion

        let addJettonUser = data.addJettonUser;
        let jettonMultiple = data.jettonMultiple;

        let chairId = AppGame.ins.xpqzjhModel.getUISeatId(addJettonUser);
     ...
     * @param data 
     */
    private onCallBanker(data: XPQzjh.NN_CMD_S_CallBanker) {


        let wCallBankerUser = data.callBankerUser;
        let cbCallMultiple = data.callMultiple;

        let chairId = AppGame.ins.xpqzjhModel.getUISeatId(wCallBankerUser);
        ...
        var pos = AppGame.ins.xpqzjhModel.getPlayerSexByRealSeat(wCallBankerUser);
        if (cbCallMultiple > 0) {
    ...
    private onXuanZhuangEnd(data) {
        // var bankindex = AppGame.ins.xpqzjhModel.gBankerIndex;//data.banker;
        // var index = AppGame.ins.xpqzjhModel.getUISeatId(bankindex);
