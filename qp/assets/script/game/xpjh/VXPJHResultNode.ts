 ...
 */
@ccclass
export default class VXPJHResultNode extends cc.Component {
    /**
     * 4个座位的 输赢分显示节点
     * @param win 正数label
     * @param lose 负数label
    ...
    /**结束的分数 */
    private showEndScore(data: XPQzjh.NN_CMD_S_GameEnd,pl:any) {
     ...
     * @param data 
     */
    private onGameEnd(data:XPQzjh.NN_CMD_S_GameEnd,pl) {
    ...
    private setScoreFunc(index: number, iswin: boolean, isAction?: boolean, content1?: string): void {
        ...
        if (iswin) {
            content = "+" + UStringHelper.getMoneyFormat(score / 100);
            ...
            if (iswin) {
                ...
                this._list[index].label.node.runAction(cc.sequence(cc.moveTo(1.0, this._targetPos[index]), cc.callFunc(() => {
                    ...
                    if (this._totalscore[index] != null) {
                        ...
                        AppGame.ins.xpqzjhModel.emit(UXPJHHelper.QZNN_SELF_EVENT.QZNN_TOTALSCORE_EVENT, data);
                    }
                }, this)));
