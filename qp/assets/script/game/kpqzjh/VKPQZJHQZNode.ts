 ...
 */
@ccclass
export default class VKPQZJHQZNode extends cc.Component {
    ...
    private addEvent() {
        ...
        UEventHandler.addClick(this._btnBuQiang.node,
            this.node, "VKPQZJHQZNode", "sendBeiShuInfo", 0);
        ...
        // MKPQZJHModel.ins.on(UKPQZJHHelper.QZNN_SELF_EVENT.QZNN_OPEN_QIANGZHUANG, this.openQiangZhuangPanel, this);
    ...
    private removeEvent() {
        ...
        // MKPQZJHModel.ins.off(UKPQZJHHelper.QZNN_SELF_EVENT.QZNN_OPEN_QIANGZHUANG, this.openQiangZhuangPanel, this);
    ...
    private sendBeiShuInfo(event, customEventData) {
        ...
        if (customEventData >= 0) {//负数置灰,不发送
            MKPQZJHModel.ins.sendCallBanker(customEventData);
        }
    }
    ...
    private onCallBanker(data: any) {
        ...
        // cc.log("gMeChairId :" + MKPQZJHModel.ins.gMeChairId);
        ...
        if (cbCallBankerMultiple != null) {
            ...
            for (let i = 0; i < mycbCallBankerMultiple.length; i++) {
                ...
                if (this._qznnList && this._qznnList[i.toString()]) {
                    ...
                    this._qznnList[i.toString()].label.string = mycbCallBankerMultiple[i].toString() + "b";//"倍"
                    ...
                    if (mycbCallBankerMultiple[i] > 0) {
                        ...
                        UEventHandler.addClick(this._qznnList[i.toString()].btn.node,
                            this.node, "VKPQZJHQZNode", "sendBeiShuInfo", mycbCallBankerMultiple[i]);
                    }
                    else {
            ...
            // for (let i = 0; i < 3; i++) {
            //     UEventHandler.addClick(this._qznnList[i.toString()].btn.node,
            //         this.node, "VKPQZJHQZNode", "sendBeiShuInfo", (i + 1));
            // }
        }
    }
    /**抢庄结束 隐藏抢庄按钮*/
    ...
    private onGameSceneCall(data: any) {
        ...
        if (data && data.callBankerMultiple && data.playStatus) {
            let cmdData = {
                cbCallBankerMultiple: data.callBankerMultiple,
                cbPlayStatus: data.playStatus
            }
