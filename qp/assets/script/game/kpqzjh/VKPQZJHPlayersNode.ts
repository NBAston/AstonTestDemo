 ...
 */
@ccclass
export default class VKPQZJHPlayersNode extends cc.Component {
    ...
    /**抢庄下注时的Y位置 qp*/
    private _qzQpPosY: Array<number> = [50.8, 75.7, -76.5, 75.7];//[50.8, 44, -43.8, 44];//-63.8
    /**发牌时的Y位置 qp*/
    ...
    private addEvent() {
        // MKPQZJHModel.ins.on(UKPQZJHHelper.QZNN_SELF_EVENT.QZNN_UPDATE_ROLEINFO, this.updateMyRoleInfo, this);

        MKPQZJHModel.ins.on(UKPQZJHHelper.QZNN_SELF_EVENT.QZNN_SUB_S_GAME_START, this.onGamestart, this);
        MKPQZJHModel.ins.on(UKPQZJHHelper.QZNN_SELF_EVENT.QZNN_SUB_S_CALL_BANKER, this.onCallBanker, this);
        ...
        MKPQZJHModel.ins.on(UKPQZJHHelper.QZNN_SELF_EVENT.QZNN_TOTALSCORE_EVENT, this.onTotalScore, this);

        MKPQZJHModel.ins.on(UKPQZJHHelper.QZNN_SELF_EVENT.QZNN_UPDATE_PLAYERS_EVENT, this.onUpdatePlayers, this);
    ...
    private removeEvent() {
        // MKPQZJHModel.ins.off(UKPQZJHHelper.QZNN_SELF_EVENT.QZNN_UPDATE_ROLEINFO, this.updateMyRoleInfo, this);

        MKPQZJHModel.ins.off(UKPQZJHHelper.QZNN_SELF_EVENT.QZNN_SUB_S_GAME_START, this.onGamestart, this);
        MKPQZJHModel.ins.off(UKPQZJHHelper.QZNN_SELF_EVENT.QZNN_SUB_S_CALL_BANKER, this.onCallBanker, this);
        ...
        MKPQZJHModel.ins.off(UKPQZJHHelper.QZNN_SELF_EVENT.QZNN_UPDATE_PLAYERS_EVENT, this.onUpdatePlayers, this);
    ...
    private onResetScene(data?: any) {
        ...
        for (let i = 0; i < this._maxplayer; i++) {
            ...
            if (i != QZNN_SELF_SEAT) {
                UDebug.log(QZNN_SELF_SEAT + "i=====" +i);
                this._players[i.toString()].node.active = false;
            }
            else//自己 就更新信息
            {
                var myscore = UKPQZJHScene.ins.myScore;
                if (myscore == 0) {
        ...
        this.resetPopPos();
        MKPQZJHModel.ins.updateBattlePlayer();
        //         if (element.seatId == 0) {
     ...
     * @param iconIndex 
     */
    setPlayerIcon(index: number, iconIndex: number) {
        //0-7 男 8-15 女
        // let gender = "man";
        // if (iconIndex > 7) {
        //     gender = "women";
        ...
        // UResManager.load(iconIndex, EIconType.Head, this._players[index.toString()]["touxiang"]);
        UResManager.load(iconIndex, EIconType.Head, this._players[index.toString()]["touxiang"],this._players[index.toString()].headImgUrl);
    }
    /**
     * 设置玩家昵称
     * @param index 座位索引
    ...
    private onUpdatePlayers(data: any) {
        ...
        for (const key in data) {
            if (data.hasOwnProperty(key)) {
                ...
                let seadId = MKPQZJHModel.ins.getUISeatId(element.chairId);
                 if (!element.isExit && element.playStatus >= 0 && seadId != null) { 
                    this.setPlayerNickName(seadId, ""+element.userId);
                    this.setPlayerCoin(seadId, element.score);
                    this.setPlayerIcon(seadId, element.headId);
                    this.setPlayerActiveByIndex(seadId, true);
                    this.setPlayerHeadBox(seadId, element.headboxId);
                    this.setPlayerVip(seadId, element.vipLevel);
                    // if(element.vipLevel == 0){
                    //     this.node.children[seadId].getChildByName("vip_03").active = false;
                    //     this.node.children[seadId].getChildByName("vip").active = false;
                    // }
                    this.setPlayerWsz(seadId,element.playStatus == 0)
                }else{
                    this.setPlayerActiveByIndex(seadId, false);
                    delete data[key];
                }
    ...
    private onAddScoreResult(data) {
        ...
        //#region  test 0-3 
        // MKPQZJHModel.ins.sMeChairId = 0;
        //#endregion

        let wAddJettonUser = data.addJettonUser;
        let cbJettonMultiple = data.jettonMultiple;

        let chairId = MKPQZJHModel.ins.getUISeatId(wAddJettonUser);
    ...
    private onCallBanker(data) {
        ...
        let chairId = MKPQZJHModel.ins.getUISeatId(wCallBankerUser);
        ...
        var pos = MKPQZJHModel.ins.getPlayerSexByRealSeat(wCallBankerUser);
        if (cbCallMultiple > 0) {
            UKPQZJHScene.ins.getMusic.playQiangZhung(true, pos);
        }
        else {
            UKPQZJHScene.ins.getMusic.playQiangZhung(false, pos);
        }
    ...
    private onXuanZhuangEnd(data) {
        // var bankindex = MKPQZJHModel.ins.gBankerIndex;//data.banker;
        // var index = MKPQZJHModel.ins.getUISeatId(bankindex);
        // //不抢，随机选庄后 变为抢1
