 ...
 */
@ccclass
export default class VKPQZJHZhuangNode extends cc.Component {
    /**
     * 该节点下需用的dic
    ...
    start() {
        ...
        for (let i = 0; i < this._maxplayer; i++) {
            ...
            this._zhuangNodeList[i.toString()].zhuang_Ani.setCompleteListener((event: any) => {
                UKPQZJHScene.ins.getMusic.playZhuang();
                MKPQZJHModel.ins.emit(UKPQZJHHelper.QZNN_SELF_EVENT.QZNN_DJS_EVENT, null, false);
            });
    ...
    setBanker(index: number) {
        MKPQZJHModel.ins.sBankerIndex = index;
    }
    /**获得庄的座位索引 */
    getBankerZhuangGuang(): number {
        var index = MKPQZJHModel.ins.gBankerIndex;
        var bIndex = MKPQZJHModel.ins.getUISeatId(index);
    ...
    showChooseBankerAni(indexs: number[]) {
        ...
        // UDebug.Log("MKPQZJHModel.ins.gBankerIndex:"+MKPQZJHModel.ins.gBankerIndex );

        if (MKPQZJHModel.ins.gBankerIndex == null) return;
        ...
        if (indexs.length > 1) {
            for (let i = 0; i < indexs.length; i++) {
                ...
                let rep = cc.repeat(cc.sequence(
                    ...
                    cc.fadeOut(0.05),
                    cc.callFunc(() => {
                        UKPQZJHScene.ins.getMusic.playChooseZhuang();
                    })
                ), 4);
                //每个人延迟执行，并加回调
                let act = cc.sequence(cc.delayTime(i * 0.05), rep, cc.callFunc((node) => {

                    // var bIndex = MKPQZJHModel.ins.getUISeatId(MKPQZJHModel.ins.gBankerIndex);
                    if (bIndex == indexs[i]) {
                        ...
                        MKPQZJHModel.ins.emit(UKPQZJHHelper.QZNN_SELF_EVENT.QZNN_XUAN_ZHUANG_END_EVENT);
        ...
        {
            // var index1 = MKPQZJHModel.ins.gBankerIndex;
            // var bIndex = MKPQZJHModel.ins.getUISeatId(index1);
    ...
    private onRandomZhuang(data: any) {
        ...
        if (data.index != null && data.bankers != null) {
            ...
            // UKPQZJHScene.ins.getMusic.playZhuang(MKPQZJHModel.ins.getPlayerSexByRealSeat(data.index));
        }
    }
    ...
    private onCallBankerResult(data) {
        ...
        if (cbAddJettonTime != null) {
            ...
            MKPQZJHModel.ins.emit(UKPQZJHHelper.QZNN_SELF_EVENT.QZNN_DJS_EVENT, djsData, true);
        }
        ...
        if (cbRandBanker == 1) {
            ...
            for (let i = 0; i < cbCallBankerUser.length; i++) {
                ...
                if (element > 0) {
                    var index = MKPQZJHModel.ins.getUISeatId(i);
                    dic[index] = element;
