 ...
 */
@ccclass
export default class VQZJHAniNode extends cc.Component {
    /**赢家框 */
    ...
    start() {
        ...
        */
        ...
        this._gameStartAni.setCompleteListener((event) => {
            if (!self._isRest) {
                AppGame.ins.qzjhModel.emit(UQZJHHelper.QZJH_SELF_EVENT.QZJH_START_ANI_COMPLETE);
            }
            self.setGameStartAniActive(false);
        });
        this.setGameStartAniActive(false);
    ...
    private addEvent() {
        // this._eventTongsha = AppGame.ins.qzjhModel.on(UQZJHHelper.QZJH_SELF_EVENT.QZJH_TONGSHA_EVENT, this.onTongSha, this);
        // this._eventTongpei = AppGame.ins.qzjhModel.on(UQZJHHelper.QZJH_SELF_EVENT.QZJH_TONGPEI_EVENT, this.onTongPei, this);

        AppGame.ins.qzjhModel.on(UQZJHHelper.QZJH_SELF_EVENT.QZJH_SUB_S_GAME_START, this.showGameStart, this);
        ...
        AppGame.ins.qzjhModel.on(UQZJHHelper.QZJH_SELF_EVENT.QZJH_TOTALSCORE_EVENT, this.onTotalScore, this);//分数滚动完事件
        AppGame.ins.qzjhModel.on(UQZJHHelper.QZJH_SELF_EVENT.QZJH_RESET_SCENE, this.onResetScene, this);//重置场景事件
    ...
    private removeEvent() {
        AppGame.ins.qzjhModel.off(UQZJHHelper.QZJH_SELF_EVENT.QZJH_SUB_S_GAME_START, this.showGameStart, this);
    ...
    private playCoinFlyAni(start: number, end: number) {
        ...
        /**播10次声音 */
        for (let j = 0; j < 5; j++) {
            UQZJHScene.ins.getMusic.playflyCoin();
        }
    ...
    private onGameEnd(data: Qzjh.NN_CMD_S_GameEnd,pl:any) {
        ...
        if (dLWScore != null) {
            ...
            var banker = AppGame.ins.qzjhModel.gBankerIndex;
            var bankerLocalPos = AppGame.ins.qzjhModel.getUISeatId(banker);
            ...
            if (bankerLocalPos != null)//有庄家
            {
                if (cbEndType == 1) {//通杀
                    ...
                    for (let j = 0; j < dLWScore.length; j++) {
                        let player = AppGame.ins.qzjhModel.getbattleplayerbyChairId(j,pl);
                        if (player != null && player.playStatus == 1 && player.seatId >= 0 && player.seatId < 4) {

                            let index1 = AppGame.ins.qzjhModel.getUISeatId(j);
                    ...
                    for (let j = 0; j < dLWScore.length; j++) {
                        ...
                        if (player != null && player.playStatus == 1 && player.seatId >= 0 && player.seatId < 4) {
                            ...
                            let index2 = AppGame.ins.qzjhModel.getUISeatId(j);
                ...
                for (let i = 0; i < dLWScore.length; i++) {
                    ...
                    let player = AppGame.ins.qzjhModel.getbattleplayerbyChairId(i,pl);
                    if (player != null && player.playStatus == 1 && player.seatId >= 0 && player.seatId < 4) {
    ...
    sendFapaiCompleteEvent(index: number) {
        ...
        AppGame.ins.qzjhModel.emit(UQZJHHelper.QZJH_SELF_EVENT.QZJH_FAPAI_COMPLETE, index);
    ...
    private setFaPaiAni(index: number, b?: boolean) {
        ...
        if (b) {
            for (let j = 0; j < 3; j++) {
                ...
                // UQZJHScene.ins.getMusic.playSendCard();
            }
        }
    }
    ...
    setGameStartAniActive(b: boolean) {
        ...
        if (b == true) {
            ...
            UQZJHScene.ins.getMusic.playStart();
        }
    }
    /**庄家通杀动画 */
    ...
    setWinAniActive(b: boolean, score?: number) {
        ...
        if (b == true) {
            ...
            if (score < 0) {
                ...
                name = "lost"; 
                this._winAni.setAnimation(0, name, false);
                UAudioManager.ins.playGameSound(AppGame.ins.roomModel.BundleName,'audio_qzjh_nsl');
            }
            else if (score > 0) {
                this._winAni.setAnimation(0, name, false);
                UAudioManager.ins.playGameSound(AppGame.ins.roomModel.BundleName,'audio_qzjh_gxnyl');
            }
            else if (score == 0) {
