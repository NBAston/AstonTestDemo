...
import MQZJHModel from "./model/MQZJHModel";
import AppGame from "../../public/base/AppGame";
import UNodeHelper from "../../common/utility/UNodeHelper";
import UEventHandler from "../../common/utility/UEventHandler";
import VQZJHPinCard from "./VQZJHPinCard";
 ...
 */
@ccclass
export default class VQZJHPincardNode extends cc.Component {
    ...
    /**挂在5张牌的脚本 数组 */
    private _pingcardsCom: Array<VQZJHPinCard> = [];
    ...
    init(): void {
        for (let i = 0; i < this.pincards.length; i++) {
            let cardCom = this.pincards[i].getComponent(VQZJHPinCard);
            cardCom.setIndex = i;
        ...
        UEventHandler.addClick(this._btn_YN.node, this.node, "VQZJHPincardNode", "onBtnYNClick");
        UEventHandler.addClick(this._btn_MN.node, this.node, "VQZJHPincardNode", "onBtnMNClick");
    ...
    addEvent() {
        //监听分发的牌
        var event1 = AppGame.ins.qzjhModel.on(UQZJHHelper.QZJH_SELF_EVENT.QZJH_PINPAI_ADD, this.addPinCardEvent, this);
        var event2 = AppGame.ins.qzjhModel.on(UQZJHHelper.QZJH_SELF_EVENT.QZJH_PINPAI_DEL, this.delPinCardEvent, this);
    ...
    removeEvent() {
        ...
        //     // AppGame.ins.qzjhModel.off(this._eventList.pop());
        // }

        AppGame.ins.qzjhModel.off(UQZJHHelper.QZJH_SELF_EVENT.QZJH_PINPAI_ADD, this.addPinCardEvent, this);
        AppGame.ins.qzjhModel.off(UQZJHHelper.QZJH_SELF_EVENT.QZJH_PINPAI_DEL, this.delPinCardEvent, this);
    }
    ...
    setCardValue(arr: Array<number>) {
        ...
        for (let i = 0; i < arr.length; i++) {
            const element = arr[i];
            var key = AppGame.ins.qzjhModel.cardValueToSpriteKey1(element);
            // UDebug.log(key);
            this.pincards[i].spriteFrame = UQZJHScene.ins.getSpriteFrame(key);
    ...
    private onBtnYNClick(event) {
        ...
        // AppGame.ins.qzjhModel.emit(UQZJHHelper.QZJH_SELF_EVENT.QZJH_PINCARD_DONE);

        this.hide();
    }
    /**无牛按钮点击事件 */
    private onBtnMNClick(event) {
        var isYouNiu = AppGame.ins.qzjhModel.getOxCard(this._cardData);
    ...
    private getPinNum(num: number): number {
        let value = AppGame.ins.qzjhModel.getCardValue(num);
        let value1 = AppGame.ins.qzjhModel.getCardTrueValue(value);
        // UDebug.log(value1);
        return value1;
    }
    ...
    testGetCardType() {
        ...
        let type1 = MQZJHModel.CARDTYPE_NAME[AppGame.ins.qzjhModel.getCardType(card1, card1.length)];
        UDebug.log("牌型 :" + type1 + " 牌值" + card1);

        let card2 = [0x2C, 0x2C, 0x3D, 0x2B, 0x1D];//五花牛
        let type2 = MQZJHModel.CARDTYPE_NAME[AppGame.ins.qzjhModel.getCardType(card2, card2.length)];
        UDebug.log("牌型 :" + type2 + " 牌值" + card2);

        let card3 = [0x2B, 0x3D, 0x3D, 0x3B, 0x1A];//四花牛
        let type3 = MQZJHModel.CARDTYPE_NAME[AppGame.ins.qzjhModel.getCardType(card3, card3.length)];
        UDebug.log("牌型 :" + type3 + " 牌值" + card3);

        let card4 = [0x07, 0x1D, 0x0D, 0x3D, 0x2D];//炸弹
        let type4 = MQZJHModel.CARDTYPE_NAME[AppGame.ins.qzjhModel.getCardType(card4, card4.length)];
        UDebug.log("牌型 :" + type4 + " 牌值" + card4);

        let card5 = [0x32, 0x12, 0x11, 0x33, 0x21];//五小牛
        let type5 = MQZJHModel.CARDTYPE_NAME[AppGame.ins.qzjhModel.getCardType(card5, card5.length)];
