 ...
 */
@ccclass
export default class VQZJHZhuangNode extends cc.Component {
    /**
     * 该节点下需用的dic
    ...
    start() {
        ...
        for (let i = 0; i < this._maxplayer; i++) {
            ...
            this._zhuangNodeList[i.toString()].zhuang_Ani.setCompleteListener((event: any) => {
                UQZJHScene.ins.getMusic.playZhuang();
                AppGame.ins.qzjhModel.emit(UQZJHHelper.QZJH_SELF_EVENT.QZJH_DJS_EVENT, null, false);
            });
    ...
    setBanker(index: number) {
        AppGame.ins.qzjhModel.sBankerIndex = index;
    }
    /**获得庄的座位索引 */
    getBankerZhuangGuang(): number {
        var index = AppGame.ins.qzjhModel.gBankerIndex;
        var bIndex = AppGame.ins.qzjhModel.getUISeatId(index);
    ...
    showChooseBankerAni(indexs: number[]) {
        ...
        // UDebug.Log("AppGame.ins.qzjhModel.gBankerIndex:"+AppGame.ins.qzjhModel.gBankerIndex );

        if (AppGame.ins.qzjhModel.gBankerIndex == null) return;
        ...
        if (indexs.length > 1) {
            for (let i = 0; i < indexs.length; i++) {
                ...
                let rep = cc.repeat(cc.sequence(
                    ...
                    cc.fadeOut(0.05),
                    cc.callFunc(() => {
                        UQZJHScene.ins.getMusic.playChooseZhuang();
                    })
                ), 4);
                //每个人延迟执行，并加回调
                let act = cc.sequence(cc.delayTime(i * 0.05), rep, cc.callFunc((node) => {

                    // var bIndex = AppGame.ins.qzjhModel.getUISeatId(AppGame.ins.qzjhModel.gBankerIndex);
                    if (bIndex == indexs[i]) {
                        ...
                        AppGame.ins.qzjhModel.emit(UQZJHHelper.QZJH_SELF_EVENT.QZJH_XUAN_ZHUANG_END_EVENT);
        ...
        {
            // var index1 = AppGame.ins.qzjhModel.gBankerIndex;
            // var bIndex = AppGame.ins.qzjhModel.getUISeatId(index1);
    ...
    private onRandomZhuang(data: any) {
        ...
        if (data.index != null && data.bankers != null) {
            ...
            // UQZJHScene.ins.getMusic.playZhuang(AppGame.ins.qzjhModel.getPlayerSexByRealSeat(data.index));
        }
    }
    ...
    private onCallBankerResult(data) {
        ...
        if (cbAddJettonTime != null) {
            ...
            AppGame.ins.qzjhModel.emit(UQZJHHelper.QZJH_SELF_EVENT.QZJH_DJS_EVENT, djsData, true);
        }
        ...
        if (cbRandBanker == 1) {
            ...
            for (let i = 0; i < cbCallBankerUser.length; i++) {
                ...
                if (element > 0) {
                    var index = AppGame.ins.qzjhModel.getUISeatId(i);
                    dic[index] = element;
