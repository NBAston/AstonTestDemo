...
import UQZJHHelper, { MAX_COUNT, OX_GOLDEN_BULL, OX_BOMB, OX_VALUE0, OX_FIVE_SMALL, OX_SILVERY_BULL, EQZJHState, QZJHBattlePlayerInfo } from "../UQZJHHelper";
import UHandler from "../../../common/utility/UHandler";
import { EGameType, ECommonUI, ELeftType, ELevelType } from "../../../common/base/UAllenum";
import UMsgCenter from "../../../common/net/UMsgCenter";
import { Game, Qzjh, GameServer } from "../../../common/cmd/proto";
...
/**qznn 自己ui的位置 */
export const QZJH_SELF_SEAT = 0;

/**
 * 创建:dz
 * 作用:qznn 主要逻辑
 */
export default class MQZJHModel extends Model {
    // //大致流程  not use
    // group = {
    //     QZJH_SC_GAMESCENE_FREE: {
    //         on: {
    //             VQZJHMainNode: "显示继续按钮"
    //         }
    //     },
    //     QZJH_SC_GAMESCENE_CALL: {
    //         on: {
    //             VQZJHQZNode: "显示抢庄界面",
    //             VQZJHMainNode: "隐藏继续按钮"
    //         }
    //     },
    //     QZJH_SC_GAMESCENE_SCORE: {
    //         on: {
    //             VQZJHQZNode: "隐藏抢庄界面",
    //             VQZJHXiaZhuNode: "显示下注界面"
    //         }
    //     },
    //     QZJH_SC_GAMESCENE_OPEN: {
    //         on: {
    //             VQZJHXiaZhuNode: "隐藏下注界面",
    //         }
    //     },
    //     ///////scene end//////////////
    //     QZJH_SUB_S_GAME_START: {
    //         on: {
    //             VQZJHMainNode: "倒计时显示/回调事件",
    //             VQZJHAniNode: "开始动画"
    //         }
    //     },
    //     QZJH_SUB_S_CALL_BANKER: {//时间未到的叫庄结果
    //         on: {
    //             VQZJHPlayersNode: "显示点了的玩家的叫庄结果",
    //             VQZJHQZNode: "是自己就隐藏叫庄按钮",
    //         }
    //     },
    //     QZJH_SUB_S_CALL_BANKER_RESULT: {
    //         on: {
    //             VQZJHZhunagNode: "显示点了的玩家的叫庄结果",
    //         }
    //     }
    // }


    static QZJH_GAMESTATUS = {
        /**空闲状态/结算 */
        FREE: 0,
        /**匹配状态 */
        MATCH: 1,
     ...
     * 游戏状态
     */
    private _state: EQZJHState = EQZJHState.Wait;

    /**本局的玩家 */
    private _battleplayer: { [key: number]: QZJHBattlePlayerInfo };

    get gBattlePlayer(): { [key: number]: QZJHBattlePlayerInfo } {
        return this._battleplayer;
    }
    get CopyBattlePlayer():{ [key: number]: QZJHBattlePlayerInfo }{
        let data={};
    ...
    exit(): void {
        ...
        this.unregesterMsg(Qzjh.SUBID.SC_GAMESCENE_FREE, new UHandler(this.onGameSceneFree, this));
        this.unregesterMsg(Qzjh.SUBID.SC_GAMESCENE_CALL, new UHandler(this.onGameSceneCall, this));
        this.unregesterMsg(Qzjh.SUBID.SC_GAMESCENE_SCORE, new UHandler(this.onGameSceneScore, this));
        this.unregesterMsg(Qzjh.SUBID.SC_GAMESCENE_OPEN, new UHandler(this.onGameSceneOpen, this));
        // this.regesterMsg(qznn.SUBID.SC_GAMESCENE_END, new UHandler(this.onGameSceneEnd, this));

        this.unregesterMsg(Qzjh.SUBID.NN_SUB_S_ADD_SCORE_RESULT, new UHandler(this.onAddScoreResult, this));
        this.unregesterMsg(Qzjh.SUBID.NN_SUB_S_CALL_BANKER, new UHandler(this.onCallBanker, this));
        this.unregesterMsg(Qzjh.SUBID.NN_SUB_S_CALL_BANKER_RESULT, new UHandler(this.onCallBankerResult, this));
        this.unregesterMsg(Qzjh.SUBID.NN_SUB_S_GAME_END, new UHandler(this.onGameEnd, this));
        this.unregesterMsg(Qzjh.SUBID.NN_SUB_S_GAME_START, new UHandler(this.onGameStart, this));
        this.unregesterMsg(Qzjh.SUBID.NN_SUB_S_OPEN_CARD_RESULT, new UHandler(this.onOpenCardResult, this));
        this.unregesterMsg(Qzjh.SUBID.NN_SUB_S_SEND_CARD, new UHandler(this.onSendCard, this));
        this._roomInfo = null;
        this._battleplayer = {};
    ...
    qznnRegest() {
        ...
        this.regesterMsg(Qzjh.SUBID.SC_GAMESCENE_FREE, new UHandler(this.onGameSceneFree, this));
        this.regesterMsg(Qzjh.SUBID.SC_GAMESCENE_CALL, new UHandler(this.onGameSceneCall, this));
        this.regesterMsg(Qzjh.SUBID.SC_GAMESCENE_SCORE, new UHandler(this.onGameSceneScore, this));
        this.regesterMsg(Qzjh.SUBID.SC_GAMESCENE_OPEN, new UHandler(this.onGameSceneOpen, this));
        // this.regesterMsg(qznn.SUBID.SC_GAMESCENE_END, new UHandler(this.onGameSceneEnd, this));

        this.regesterMsg(Qzjh.SUBID.NN_SUB_S_ADD_SCORE_RESULT, new UHandler(this.onAddScoreResult, this));
        this.regesterMsg(Qzjh.SUBID.NN_SUB_S_CALL_BANKER, new UHandler(this.onCallBanker, this));
        this.regesterMsg(Qzjh.SUBID.NN_SUB_S_CALL_BANKER_RESULT, new UHandler(this.onCallBankerResult, this));
        this.regesterMsg(Qzjh.SUBID.NN_SUB_S_GAME_END, new UHandler(this.onGameEnd, this));
        this.regesterMsg(Qzjh.SUBID.NN_SUB_S_GAME_START, new UHandler(this.onGameStart, this));
        this.regesterMsg(Qzjh.SUBID.NN_SUB_S_OPEN_CARD_RESULT, new UHandler(this.onOpenCardResult, this));
        this.regesterMsg(Qzjh.SUBID.NN_SUB_S_SEND_CARD, new UHandler(this.onSendCard, this));
        ...
        // this.regesterMsg(qznn.SUBID.NN_SUB_S_OPERATE_FAIL, new UHandler(this.onOperateFail, this));
        // this.regesterMsg(qznn.SUBID.NN_SUB_S_GAME_START_AI, new UHandler(this.onGameStartAI, this));

    }

    // qznnUnregester(){
    //     this.unregesterMsg(qznn.SUBID.NN_SUB_S_ADD_SCORE_RESULT, new UHandler(this.onAddScoreResult, this));
    //     this.unregesterMsg(qznn.SUBID.NN_SUB_S_CALL_BANKER, new UHandler(this.onCallBanker, this));
    //     this.unregesterMsg(qznn.SUBID.NN_SUB_S_CALL_BANKER_RESULT, new UHandler(this.onCallBankerResult, this));
    //     this.unregesterMsg(qznn.SUBID.NN_SUB_S_GAME_END, new UHandler(this.onGameEnd, this));
    //     this.unregesterMsg(qznn.SUBID.NN_SUB_S_GAME_START, new UHandler(this.onGameStart, this));
    //     this.unregesterMsg(qznn.SUBID.NN_SUB_S_GAME_START_AI, new UHandler(this.onGameStartAI, this));
    //     this.unregesterMsg(qznn.SUBID.NN_SUB_S_OPEN_CARD_RESULT, new UHandler(this.onOpenCardResult, this));
    //     this.unregesterMsg(qznn.SUBID.NN_SUB_S_OPERATE_FAIL, new UHandler(this.onOperateFail, this));
    //     this.unregesterMsg(qznn.SUBID.NN_SUB_S_SEND_CARD, new UHandler(this.onSendCard, this));
    // }


    /**注册qznn网络消息事件 */
    regesterMsg(subId: number, handler: UHandler) {
        UMsgCenter.ins.regester(EGameType.QZJH,
    ...
    unregesterMsg(subId: number, handler: UHandler) {
        UMsgCenter.ins.unregester(EGameType.QZJH,
            Game.Common.MAINID.MAIN_MESSAGE_CLIENT_TO_GAME_LOGIC,
            subId,
            handler);
    }
    ...
    sendMsg(subId: number, data: any, handler: UHandler = null, unlock: boolean = false) {
        UMsgCenter.ins.sendPkg(EGameType.QZJH,
            Game.Common.MAINID.MAIN_MESSAGE_CLIENT_TO_GAME_LOGIC,
    ...
    //         case MQZJHModel.QZJH_GAMESTATUS.FREE:
    //             {
    //                 this.emit(QZJH_SC_GAMESCENE_FREE, data);
    //             }
    //             break;
    ...
    /**
     * 设置所有场景内玩家状态
     * @param gameStatus MQZJHModel.QZJH_GAMESTATUS状态
     * @param state EQZJHState状态
     * @param cbPlayStatus 各玩家状态
     */
    private setAllSceneStatus(gameStatus: number, state: EQZJHState, cbPlayStatus?: boolean[]) {
        //赋值房间内玩家状态
        ...
        //默认赋状态 旁观 自由人
        this._state = EQZJHState.Watching;
        this._gameStatus = MQZJHModel.QZJH_GAMESTATUS.FREE;
        //如果是上座玩牌玩家，重新赋状态
        if (this._battleplayer[this.selfUserId] != null
            && this._battleplayer[this.selfUserId].playStatus == 1) {
            this._state = state;//EQZJHState.Gameing;
            this._gameStatus = gameStatus;//MQZJHModel.QZJH_GAMESTATUS.CALL;
        }
    }
    /**设置房间内玩家信息 */
    ...
    public updateBattlePlayer() {
        this.emit(UQZJHHelper.QZJH_SELF_EVENT.QZJH_UPDATE_PLAYERS_EVENT, this._battleplayer);
    }

    private onGameSceneFree(data: Qzjh.NN_MSG_GS_FREE) {

        // this.emit(QZJH_UPDATE_PLAYERS_EVENT, this._roomPlayers);

        this._currentDizhu = data.cellScore || 1;

        if (this._state != EQZJHState.Match) {
            this._state = EQZJHState.Wait;
        }
        ...
        this.emit(UQZJHHelper.QZJH_EVENT.QZJH_GAMESCENE_FREE, data, ishideContinue);
    ...
    onGameSceneCall(data: Qzjh.NN_MSG_GS_CALL) {
        ...
        this.setAllSceneStatus(MQZJHModel.QZJH_GAMESTATUS.CALL, EQZJHState.Gameing, cbPlayStatus);
        this.emit(UQZJHHelper.QZJH_EVENT.QZJH_GAMESCENE_CALL, data);
        this.updateBattlePlayer();
        this.emit(UQZJHHelper.QZJH_SELF_EVENT.QZJH_UPDATE_ROOMID, roomId);
    ...
    onGameSceneScore(data: Qzjh.NN_MSG_GS_SCORE) {
        ...
        this.setAllSceneStatus(MQZJHModel.QZJH_GAMESTATUS.SCORE, EQZJHState.Gameing, cbPlayStatus);

        this.emit(UQZJHHelper.QZJH_EVENT.QZJH_GAMESCENE_SCORE, data);
    ...
    onGameSceneOpen(data: Qzjh.NN_MSG_GS_OPEN) {
        ...
        this.setAllSceneStatus(MQZJHModel.QZJH_GAMESTATUS.OPEN, EQZJHState.Gameing, cbPlayStatus);

        this.emit(UQZJHHelper.QZJH_EVENT.QZJH_GAMESCENE_OPEN, data);
    ...
    // onGameSceneEnd(data) {
    //  this.emit(QZJH_SC_GAMESCENE_END, data);
    // }
    ...
    private onUserEnterNotify(data: any) {
        ...
        // this.emit(UQZJHHelper.QZJH_SELF_EVENT.QZJH_UPDATE_PLAYERS_EVENT, this._roomPlayers);
    }
    ...
    private add_battle_player(element: RoomPlayerInfo): QZJHBattlePlayerInfo {

        if (!this._battleplayer) this._battleplayer = {};
        let item = new QZJHBattlePlayerInfo();
        ...
        UDebug.Log("-------------------"+JSON.stringify(item));
        this.setRoomPlayersInfo();
    ...
    private sc_ts_player_left_room(data: any) {
        ...
        if (data.retCode == 0) {
            ...
            if (data.type == ELeftType.ReturnToRoom) {
                if (this._state != EQZJHState.AlreadyLeft) {
                    this._state = EQZJHState.AlreadyLeft;
                    if (this._roomInfo) {
                ...
                this.emit(UQZJHHelper.QZJH_SELF_EVENT.QZJH_SC_TS_CANCLE_MATCH, true);
                this._state = EQZJHState.Wait;
                ...
                this.emit(UQZJHHelper.QZJH_SELF_EVENT.QZJH_SC_TS_START_MATCH, true);
            }

        }
        // else if (data.retCode == 2) {
        //     this.emit(UQZJHHelper.QZJH_SELF_EVENT.QZJH_CONTINUE_ACTIVE, true);
        // }
        else {
            if (data.type == ELeftType.ReturnToRoom) {
                this._retrunLobby = false;
                ...
                //     type: 1, data: ULanHelper.SG_CANT_EXIT_GAME
                //     //     , handler: UHandler.create((a) => {
                //     //     if (a) {
                //     //         this.emit(UQZJHHelper.QZJH_SELF_EVENT.QZJH_CONTINUE_ACTIVE,true);
                //     //     }
                //     // }, this)
                ...
                if (this._state != EQZJHState.Gameing) {
                    this.emit(UQZJHHelper.QZJH_SELF_EVENT.QZJH_SC_TS_CANCLE_MATCH, false);
                }
                ...
                this.emit(UQZJHHelper.QZJH_SELF_EVENT.QZJH_SC_TS_START_MATCH, false);
            }
    ...
    exitGame(): void {
        ...
        switch (this._state) {
            ...
            case EQZJHState.Gameing:
            case EQZJHState.LeftGame:
                {
                ...
                break;
            case EQZJHState.Watching:
            case EQZJHState.Match:
            case EQZJHState.Wait:
                {
                    if (this._roomInfo ) {
    ...
    requestMatch(): void {
        ...
        if (this._state == EQZJHState.LeftGame) {
            return;
        }
        this._state = EQZJHState.LeftGame;
    ...
    /**游戏开始 */
    onGameStart(data: Qzjh.NN_CMD_S_GameStart) {
        ...
        this._state = EQZJHState.Gameing;

        this.emit(UQZJHHelper.QZJH_EVENT.QZJH_GAME_START, data);
        this.updateBattlePlayer();
    ...
    /**叫庄 */
    onCallBanker(data: Qzjh.NN_CMD_S_CallBanker) {
        this.emit(UQZJHHelper.QZJH_EVENT.QZJH_CALL_BANKER, data);
        this._state = EQZJHState.Gameing;

    }

    /**叫庄结果 */
    onCallBankerResult(data: Qzjh.NN_CMD_S_CallBankerResult) {
        this.emit(UQZJHHelper.QZJH_EVENT.QZJH_CALL_BANKER_RESULT, data);
        this._state = EQZJHState.Gameing;

    }

    /**下注结果 */
    onAddScoreResult(data: Qzjh.NN_CMD_S_AddScoreResult) {  // private
        //    var wAddJettonUser = data.wAddJettonUser
        //    var cbJettonMultiple = data.cbJettonMultiple

        this.emit(UQZJHHelper.QZJH_EVENT.QZJH_ADD_SCORE_RESULT, data);
        this._state = EQZJHState.Gameing;

    }

    /**发牌消息 */
    onSendCard(data: Qzjh.NN_CMD_S_SendCard) {
        this.emit(UQZJHHelper.QZJH_EVENT.QZJH_SEND_CARD, data);
        this._state = EQZJHState.Gameing;

    }

    /**开牌结果 */
    onOpenCardResult(data: Qzjh.NN_CMD_S_OpenCardResult) {
        this.emit(UQZJHHelper.QZJH_EVENT.QZJH_OPEN_CARD_RESULT, data);
        this._state = EQZJHState.Gameing;

    }

    /**游戏结束 */
    onGameEnd(data: Qzjh.NN_CMD_S_GameEnd) {
        ...
        this.emit(UQZJHHelper.QZJH_EVENT.QZJH_GAME_END, data);
        this.emit(UQZJHHelper.QZJH_SELF_EVENT.QZJH_DJS_EVENT, 0);
    ...
    /**设置状态 */
    setReconnectState(state: EQZJHState, isRestRoominfo?: boolean) {
        this._state = state;
    ...
    sendAddScore(jetton: number) {
        ...
        this.sendMsg(Qzjh.SUBID.NN_SUB_C_ADD_SCORE, data);
    }
    /**
     * 请求叫庄
     * @param callFlag 
    ...
    sendCallBanker(callFlag: number) {
        ...
        this.sendMsg(Qzjh.SUBID.NN_SUB_C_CALL_BANKER, data);
    }

    /**请求开牌 */
    sendOpenCard() {
        this.sendMsg(Qzjh.SUBID.NN_SUB_C_OPEN_CARD, {});
    }
    ...
    sendNextExit(b: boolean) {
        ...
        this.sendMsg(Qzjh.SUBID.NN_SUB_C_ROUND_END_EXIT, data);
    }
    //#endregion
    ...
    public getCardSuit(card: number): number {
        return (card & MQZJHModel.LOGIC_MASK.COLOR) >> 4;
    }
    /**获取一副完整的牌 not use*/
    ...
    public getCardValue(card: number): number {
        return card & MQZJHModel.LOGIC_MASK.VALUE;
    }
    ...
    /**根据真实玩家位置获取玩家信息 */
    getbattleplayerbyChairId(chairId: number,pl?:any): QZJHBattlePlayerInfo {
    ...
    /**根据ui玩家位置获取玩家信息 */
    getbattleplayerbySeatId(seatId: number,pl?:any): QZJHBattlePlayerInfo {
        let ppp=pl || this._battleplayer;//**将就他原来的写法 */
        for (const key in ppp) {
            if (ppp.hasOwnProperty(key)) {
                const element = ppp[key];
    ...
    /**获取刚进房间时候 显示自己的信息 */
    getshowselfinfo(): QZJHBattlePlayerInfo {
        let bb = new QZJHBattlePlayerInfo();
        bb.seatId = QZJH_SELF_SEAT;
