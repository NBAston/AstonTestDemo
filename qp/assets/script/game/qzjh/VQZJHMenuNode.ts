 ...
 */
@ccclass
export default class VQZJHMenuNode extends VBaseUI {//cc.Component {
    // onLoad () {}
    ...
    start() {
        ...
        UEventListener.get(this.node).onClick = new UHandler(() => {
            AppGame.ins.closeUI(ECommonUI.QZJH_Menu);
        }, this);
        ...
        if (this.btn_exit != null) {
            ...
            // handler.component = this.getComponent("VQZJHMenuNode");
    ...
    onBtnExitClick(event) {
        ...
        AppGame.ins.qzjhModel.exitGame();
    ...
    /**设置按钮点击事件 */
    onBtnSetClick(event) {
        AppGame.ins.showUI(ECommonUI.QZJH_Setting);
    }
    /**记录按钮点击事件 */
    onBtnRecordClick(event) {
        AppGame.ins.showUI(ECommonUI.QZJH_Record, AppGame.ins.roomModel.requestGameRecords(EGameType.QZJH));
    }
    /**牌型按钮点击事件 */
    onBtnPXClick(event) {
        AppGame.ins.showUI(ECommonUI.QZJH_PX);
    }

    /**帮助按钮点击事件 */
    onBtnHelpClick(event) {
        AppGame.ins.showUI(ECommonUI.QZJH_Help);
    }
    /**反馈按钮点击事件 */
    onBtnFanKuiClick(event) {
        AppGame.ins.showUI(ECommonUI.QZJH_FanKui);
    }
    /**拦截小框点击事件冒泡 */
