 ...
 */
@ccclass
export default class VQZJHXiaZhuNode extends cc.Component {
    ...
    updateXiazhuBtns(zhus: number[], length?: number) {
        ...
        for (let i = 0; i < length; i++) {
            ...
            if(zhus[i] > 0) {
                ...
                UEventHandler.addClick(item, this.node, "VQZJHXiaZhuNode", "sendXiaZhuInfo", i);
    ...
    private sendXiaZhuInfo(event, customEventData) {
        ...
        AppGame.ins.qzjhModel.sendAddScore(customEventData);
    }
    ...
    private onAddScoreResult(data: any) {
        ...
        // let chairId = AppGame.ins.qzjhModel.changeToLocalPos(wAddJettonUser);

        // UDebug.Log("gMeChairId :" + AppGame.ins.qzjhModel.gMeChairId);
    ...
    private onGameSceneScore(data: any) {
        ...
        //         callback:"xz"
        //     }
        //     AppGame.ins.qzjhModel.emit(UQZJHHelper.QZJH_SELF_EVENT.QZJH_DJS_EVENT, djsData);
        // }

        // AppGame.ins.qzjhModel.emit(UQZJHHelper.QZJH_SELF_EVENT.QZJH_DJS_EVENT)
    }

    private onXuanZhuangEnd(data:any){
        var dwBankerUser = AppGame.ins.qzjhModel.gBankerIndex;
