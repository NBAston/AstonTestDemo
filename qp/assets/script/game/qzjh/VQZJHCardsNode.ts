import VQZJHPincardNode from "./VQZJHPincardNode";
import UNodeHelper from "../../common/utility/UNodeHelper";
import AppGame from "../../public/base/AppGame";
import UQZJHScene from "./UQZJHScene";
import MQZJHModel, { QZJH_SELF_SEAT } from "./model/MQZJHModel";
...
const CARD_COUNT = 3;  // 牌的数量  

@ccclass
export default class VQZJHCardsNode extends cc.Component {

    private _pincardNode: VQZJHPincardNode = null;
    /**
     * 牌组节点下的各节点
     * @param card0-4 :牌
     * @param node 牌组的牌的父节点
    ...
    private init() {
        ...
        this._pincardNode = UNodeHelper.getComponent(this.node, "pincardsNode", VQZJHPincardNode);

        this._btn_seepai = UNodeHelper.getComponent(this.node, "btn_seepai", cc.Button);
        UEventHandler.addClick(this._btn_seepai.node, this.node, "VQZJHCardsNode", "sendOpenCard");
    ...
    /**添加事件 */
    private addEvent() {
        AppGame.ins.qzjhModel.on(UQZJHHelper.QZJH_SELF_EVENT.QZJH_FAPAI_COMPLETE, this.showCardBackByIndex, this);

        AppGame.ins.qzjhModel.on(UQZJHHelper.QZJH_SELF_EVENT.QZJH_SC_GAMESCENE_OPEN, this.onGameSceneOpen, this);
        AppGame.ins.qzjhModel.on(UQZJHHelper.QZJH_SELF_EVENT.QZJH_SUB_S_OPEN_CARD_RESULT, this.onOpenCardResult, this);
        ...
        // AppGame.ins.qzjhModel.on(UQZJHHelper.QZJH_SELF_EVENT.QZJH_PINCARD_DONE, this.onPinCardDone, this);
    ...
    private removeEvent() {
        AppGame.ins.qzjhModel.off(UQZJHHelper.QZJH_SELF_EVENT.QZJH_FAPAI_COMPLETE, this.showCardBackByIndex, this);

        AppGame.ins.qzjhModel.off(UQZJHHelper.QZJH_SELF_EVENT.QZJH_SC_GAMESCENE_OPEN, this.onGameSceneOpen, this);
        AppGame.ins.qzjhModel.off(UQZJHHelper.QZJH_SELF_EVENT.QZJH_SUB_S_OPEN_CARD_RESULT, this.onOpenCardResult, this);
        ...
        // AppGame.ins.qzjhModel.off(UQZJHHelper.QZJH_SELF_EVENT.QZJH_PINCARD_DONE, this.onPinCardDone, this);
    ...
    private onAddScoreResult(data: any) {
        AppGame.ins.qzjhModel.gameStatus = MQZJHModel.QZJH_GAMESTATUS.SEND;
    }
    /**
     * 开牌场景消息
     * @param data 
    ...
    private onGameSceneOpen(data: any) {
        ...
        if (cbIsOpenCard != null) {
            if (cbIsOpenCard[AppGame.ins.qzjhModel.gMeChairId] == 1) {
    ...
    private openNotSendCard(data: any) {
        ...
        for (let i = 0; i < this._maxplayer; i++) {
            ...
            let player = AppGame.ins.qzjhModel.getbattleplayerbySeatId(i);
    ...
    private onOpenCardResult(data: any) {
        ...
        var chairId = AppGame.ins.qzjhModel.getUISeatId(wOpenCardUser);

        if (chairId == QZJH_SELF_SEAT) {
        ...
        this.showCardTypeAni(chairId, cbCardData, cbCardType);
    ...
    private sendOpenCard() {
        ...
        AppGame.ins.qzjhModel.sendOpenCard();
    }
    ...
    private showCardBack(index: number) {
        ...
        for (let i = 0; i < CARD_COUNT; i++) {
            ...
            let key = "poker_b1";
            this._cardsList[index.toString()][path].spriteFrame = UQZJHScene.ins.getSpriteFrame(key);
    ...
    private showCardBackByIndex(index: number) {
        ...
        if (index == 0 && this._cbCardData != null) {
            ...
            //AppGame.ins.qzjhModel.randCardData(this._cbCardData,this._cbCardData.length);
        ...
        //#region 3 发牌后自己显示牌背
        // this.showCardBack(index);
        let player = AppGame.ins.qzjhModel.getbattleplayerbySeatId(0);
    ...
    private setCardsSprite(index: number, values: number[]): void {
        ...
        for (let i = 0; i < CARD_COUNT; i++) {
            ...
            let key = AppGame.ins.qzjhModel.cardValueToSpriteKey1(values[i]);

            // UDebug.Log("key:"+JSON.stringify(key)+ "  index:"+index);

            cards[path].spriteFrame = UQZJHScene.ins.getSpriteFrame(key);
        }
    }
    ...
    setSpineCardType(index: number, cardtype: number): void {
        ...
        // }
        // cardtype = 14;
        aniName = UQZJHHelper.CARD_TYPE2[cardtype];
        ...
        UQZJHScene.ins.getMusic.playPaiXing(aniName, AppGame.ins.qzjhModel.getPlayerSexByUISeat(index));
