...
import UQZNNHelper, { MAX_COUNT, OX_GOLDEN_BULL, OX_BOMB, OX_VALUE0, OX_FIVE_SMALL, OX_SILVERY_BULL, EQZNNState, QZNNBattlePlayerInfo } from "../UQZNNHelper";
import UHandler from "../../../common/utility/UHandler";
import { EGameType, ECommonUI, ELeftType, ELevelType } from "../../../common/base/UAllenum";
import UMsgCenter from "../../../common/net/UMsgCenter";
import { Game, Qznn, GameServer } from "../../../common/cmd/proto";
 ...
 * 作用:qznn 主要逻辑
 */
export default class MQZNNModel extends Model {
    // //大致流程  not use
    // group = {
    ...
    exit(): void {
        ...
        this.unregesterMsg(Qznn.SUBID.SC_GAMESCENE_FREE, new UHandler(this.onGameSceneFree, this));
        this.unregesterMsg(Qznn.SUBID.SC_GAMESCENE_CALL, new UHandler(this.onGameSceneCall, this));
        this.unregesterMsg(Qznn.SUBID.SC_GAMESCENE_SCORE, new UHandler(this.onGameSceneScore, this));
        this.unregesterMsg(Qznn.SUBID.SC_GAMESCENE_OPEN, new UHandler(this.onGameSceneOpen, this));
        this.unregesterMsg(Qznn.SUBID.SC_GAMESCENE_END, new UHandler(this.onGameSceneEnd, this));
    ...
    qznnRegest() {
        ...
        this.regesterMsg(Qznn.SUBID.SC_GAMESCENE_FREE, new UHandler(this.onGameSceneFree, this));
        this.regesterMsg(Qznn.SUBID.SC_GAMESCENE_CALL, new UHandler(this.onGameSceneCall, this));
        this.regesterMsg(Qznn.SUBID.SC_GAMESCENE_SCORE, new UHandler(this.onGameSceneScore, this));
        this.regesterMsg(Qznn.SUBID.SC_GAMESCENE_OPEN, new UHandler(this.onGameSceneOpen, this));
        this.regesterMsg(Qznn.SUBID.SC_GAMESCENE_END, new UHandler(this.onGameSceneEnd, this));

        this.regesterMsg(Qznn.SUBID.NN_SUB_S_ADD_SCORE_RESULT, new UHandler(this.onAddScoreResult, this));
        this.regesterMsg(Qznn.SUBID.NN_SUB_S_CALL_BANKER, new UHandler(this.onCallBanker, this));
        this.regesterMsg(Qznn.SUBID.NN_SUB_S_CALL_BANKER_RESULT, new UHandler(this.onCallBankerResult, this));
        this.regesterMsg(Qznn.SUBID.NN_SUB_S_GAME_END, new UHandler(this.onGameEnd, this));
        this.regesterMsg(Qznn.SUBID.NN_SUB_S_GAME_START, new UHandler(this.onGameStart, this));
        this.regesterMsg(Qznn.SUBID.NN_SUB_S_OPEN_CARD_RESULT, new UHandler(this.onOpenCardResult, this));
        this.regesterMsg(Qznn.SUBID.NN_SUB_S_SEND_CARD, new UHandler(this.onSendCard, this));
    ...
    regesterMsg(subId: number, handler: UHandler) {
        UMsgCenter.ins.regester(EGameType.QZNN,
    ...
    unregesterMsg(subId: number, handler: UHandler) {
        UMsgCenter.ins.unregester(EGameType.QZNN,
            Game.Common.MAINID.MAIN_MESSAGE_CLIENT_TO_GAME_LOGIC,
            subId,
            handler);
    }
    ...
    sendMsg(subId: number, data: any, handler: UHandler = null, unlock: boolean = false) {
        UMsgCenter.ins.sendPkg(EGameType.QZNN,
            Game.Common.MAINID.MAIN_MESSAGE_CLIENT_TO_GAME_LOGIC,
    ...
    //         case MQZNNModel.QZNN_GAMESTATUS.FREE:
    //             {
    ...
    /**
     * 设置所有场景内玩家状态
     * @param gameStatus MQZNNModel.QZNN_GAMESTATUS状态
     * @param state EQZNNState状态
     * @param cbPlayStatus 各玩家状态
     */
    private setAllSceneStatus(gameStatus: number, state: EQZNNState, cbPlayStatus?: boolean[]) {
        //赋值房间内玩家状态
    ...
    private onGameSceneFree(data: Qznn.NN_MSG_GS_FREE) {
        ...
        this._gameStatus = MQZNNModel.QZNN_GAMESTATUS.FREE;
    ...
    onGameSceneCall(data: Qznn.NN_MSG_GS_CALL) {
        ...
        this.setAllSceneStatus(MQZNNModel.QZNN_GAMESTATUS.CALL, EQZNNState.Gameing, cbPlayStatus);
    ...
    onGameSceneScore(data: Qznn.NN_MSG_GS_SCORE) {
        ...
        this.setAllSceneStatus(MQZNNModel.QZNN_GAMESTATUS.SCORE, EQZNNState.Gameing, cbPlayStatus);
    ...
    onGameSceneOpen(data:any) {
        ...
        this.setAllSceneStatus(MQZNNModel.QZNN_GAMESTATUS.OPEN, EQZNNState.Gameing, cbPlayStatus);
    ...
    onGameSceneEnd(data:any) {
        ...
        this.gameStatus = MQZNNModel.QZNN_GAMESTATUS.OVER
        //this.setAllSceneStatus(MQZNNModel.QZNN_GAMESTATUS.OVER, EQZNNState.Gameing, cbPlayStatus);
 
        this.emit(UQZNNHelper.QZNN_EVENT.QZNN_GAMESCENE_END, data);
    ...
    /**游戏开始 */
    onGameStart(data: Qznn.NN_CMD_S_GameStart) {
    ...
    /**叫庄 */
    onCallBanker(data: Qznn.NN_CMD_S_CallBanker) {
    ...
    /**叫庄结果 */
    onCallBankerResult(data: Qznn.NN_CMD_S_CallBankerResult) {
    ...
    /**下注结果 */
    onAddScoreResult(data: Qznn.NN_CMD_S_AddScoreResult) {  // private
    ...
    /**发牌消息 */
    onSendCard(data: Qznn.NN_CMD_S_SendCard) {
    ...
    /**开牌结果 */
    onOpenCardResult(data: Qznn.NN_CMD_S_OpenCardResult) {
    ...
    /**游戏结束 */
    onGameEnd(data: Qznn.NN_CMD_S_GameEnd) {
    ...
    // private refreshPlayer(data) {
    //     let dTotalScore = data.totalScore;
    //     for (let i = 0; i < dTotalScore.length; i++) { 
    //         let keys = Object.keys(this._battleplayer);
    //         for (let index1 = 0; index1 < keys.length; index1++) { 
    //               let player = this._battleplayer[keys[index1]];
    //               let chairId = player.chairId;
    //               if(chairId == i) {
    //                 this._battleplayer[keys[index1]].score = dTotalScore[i];
    //               } 
    //         }
    //     }
    // }
     ...
     */
    sendFreshGameScene() {
        this.sendMsg(Qznn.SUBID.CS_GAMESCENE_FRESH, {});
    }
    ...
    sendAddScore(jetton: number) {
        ...
        this.sendMsg(Qznn.SUBID.NN_SUB_C_ADD_SCORE, data);
    }
    /**
     * 请求叫庄
     * @param callFlag 
    ...
    sendCallBanker(callFlag: number) {
        ...
        this.sendMsg(Qznn.SUBID.NN_SUB_C_CALL_BANKER, data);
    }

    /**请求开牌 */
    sendOpenCard() {
        this.sendMsg(Qznn.SUBID.NN_SUB_C_OPEN_CARD, {});
    }
    ...
    sendNextExit(b: boolean) {
        ...
        this.sendMsg(Qznn.SUBID.NN_SUB_C_ROUND_END_EXIT, data);
    }
    //#endregion
    ...
    public getCardSuit(card: number): number {
        return (card & MQZNNModel.LOGIC_MASK.COLOR) >> 4;
    }
    /**获取一副完整的牌 not use*/
    ...
    public getCardValue(card: number): number {
        return card & MQZNNModel.LOGIC_MASK.VALUE;
    }
    ...
    game_show(){
        //清空上局场景
        this.emit(UQZNNHelper.QZNN_SELF_EVENT.QZNN_GAME_TO_SHOW);
        //请求最新场景
        this.sendFreshGameScene()
    }
    //#endregion
}   
