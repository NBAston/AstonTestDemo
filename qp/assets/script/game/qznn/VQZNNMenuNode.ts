 ...
 */
@ccclass
export default class VQZNNMenuNode extends VBaseUI {//cc.Component {
    // onLoad () {}
    ...
    start() {
        ...
        if (this.btn_exit != null) {
            ...
            // handler.component = this.getComponent("VQZNNMenuNode");
    ...
    onBtnExitClick(event) {
        ...
        AppGame.ins.qznnModel.exitGame();
    ...
    onBtnRecordClick(event) {
        AppGame.ins.showUI(ECommonUI.QZNN_Record, AppGame.ins.roomModel.requestGameRecords(EGameType.QZNN));
    }
    /**牌型按钮点击事件 */
    onBtnPXClick(event) {
