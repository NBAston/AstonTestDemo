import VQZNNPincardNode from "./VQZNNPincardNode";
...
@ccclass
export default class VQZNNCardsNode extends cc.Component {
    ...
    private init() {
        ...
        UEventHandler.addClick(this._btn_seepai.node, this.node, "VQZNNCardsNode", "sendOpenCard");
    ...
    /**添加事件 */
    private addEvent() {
        AppGame.ins.qznnModel.on(UQZNNHelper.QZNN_SELF_EVENT.QZNN_FAPAI_COMPLETE, this.showCardBackByIndex, this);
        AppGame.ins.qznnModel.on(UQZNNHelper.QZNN_SELF_EVENT.QZNN_SC_GAMESCENE_OPEN, this.onGameSceneOpen, this);
        AppGame.ins.qznnModel.on(UQZNNHelper.QZNN_SELF_EVENT.QZNN_SUB_S_OPEN_CARD_RESULT, this.onOpenCardResult, this);
    ...
    private removeEvent() {
        AppGame.ins.qznnModel.off(UQZNNHelper.QZNN_SELF_EVENT.QZNN_FAPAI_COMPLETE, this.showCardBackByIndex, this);
        AppGame.ins.qznnModel.off(UQZNNHelper.QZNN_SELF_EVENT.QZNN_SC_GAMESCENE_OPEN, this.onGameSceneOpen, this);
        AppGame.ins.qznnModel.off(UQZNNHelper.QZNN_SELF_EVENT.QZNN_SUB_S_OPEN_CARD_RESULT, this.onOpenCardResult, this);
    ...
    private onAddScoreResult(data: any) {
        AppGame.ins.qznnModel.gameStatus = MQZNNModel.QZNN_GAMESTATUS.SEND;
    }
    /**
     * 开牌场景消息
     * @param data 
    ...
    private onGameSceneOpen(data: any) {
        ...
        if (cbIsOpenCard != null) {
            if (cbIsOpenCard[AppGame.ins.qznnModel.gMeChairId] == 1) {
    ...
    private onOpenCardResult(data: any) {
        ...
        var chairId = AppGame.ins.qznnModel.getUISeatId(wOpenCardUser);
    ...
    private sendOpenCard() {
        ...
        AppGame.ins.qznnModel.sendOpenCard();
    }
    ...
    private showCardBackByIndex(index: number) {
        ...
        //#region 2发牌后自己的牌显示
            ...
            if (index == 0 && this._cbCardData != null) {
                ...
                //AppGame.ins.qznnModel.randCardData(this._cbCardData,this._cbCardData.length);
                ...
                //if (AppGame.ins.qznnModel.gameStatus == MQZNNModel.QZNN_GAMESTATUS.CALL){
                this.showCardBack(index);
            }
        ...
        //#region 3 发牌后自己显示牌背
        // this.showCardBack(index);
        let player = AppGame.ins.qznnModel.getbattleplayerbySeatId(0);
        if (player != null && player.playStatus == 1) {
            //if (AppGame.ins.qznnModel.gameStatus == MQZNNModel.QZNN_GAMESTATUS.SEND)
            //this._btn_seepai.node.active = true;
        }else {
            AppGame.ins.qznnModel.emit(UQZNNHelper.QZNN_SELF_EVENT.QZNN_MOVE_NEXT_CMD);
        }
    ...
    private setCardsSprite(index: number, values: number[]): void {
        ...
        for (let i = 0; i < 5; i++) {
            ...
            let key = AppGame.ins.qznnModel.cardValueToSpriteKey1(values[i]);
    ...
    setSpineCardType(index: number, cardtype: number): void {
        ...
        UQZNNScene.ins.getMusic.playPaiXing(cardtype, AppGame.ins.qznnModel.getPlayerSexByUISeat(index));
    ...
    showCardTypeAni(index: number, cbCardData: number[], cbCardType?: number, cbOxCardData?: number[]) {
        ...
        // AppGame.ins.qznnModel.sortCardList(cbCardData,cbCardData.length);
        ...
        if (cbCardType != null) {
            ...
            cardtype = AppGame.ins.qznnModel.getCardType(cbCardData, cbCardData.length);
        }
        ...
        if (cbOxCardData != null && cbOxCardData.length == 5) {
            ...
            if (niuniuTemp.length == 5) {
                ...
                AppGame.ins.qznnModel.getOxCard(cbCardData);
            }
            ...
            var niuniuCard = AppGame.ins.qznnModel.getOxCard(cbCardData);
