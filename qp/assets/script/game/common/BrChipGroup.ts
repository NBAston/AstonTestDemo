...
const UNCHIP_Y = -60;  // 不可用的时候筹码下滑点
const AVCHIP_Y = -46;  // 可用的时候筹码下滑点
const GoldRate = 100;


export default class BrChipGroup {
    ...
    unChip(index: number, isback?: boolean) {
        ...
        var lab_chip = UNodeHelper.getComponent(this._chips[index], 'Background/lab_num', cc.Label);
        if(lab_chip) {
            // lab_chip.node.opacity = 200;
            lab_chip.node.color = this._un_color;
        ...
        if(back){
            back.string = this._chip_un_lab[index];
            // back.node.opacity = 200;
        }
        // if (!back['material'])
        //     back['material'] = ShaderManager.useCostomShader(back.getComponent(cc.Label), new GrayShader());
        // if (isback) {
        //     back['material'].setDefine('gray', true);
        // } else {
        //     back['material'].setDefine('gray', false);
        // }
    ...
    avChip(index: number) {
        ...
        var lab_chip = UNodeHelper.getComponent(this._chips[index], 'Background/lab_num', cc.Label);
        if(lab_chip) {
            lab_chip.node.opacity = 255;
            lab_chip.node.color  = this._av_color_arr[index];
        ...
        if(back){
            back.string = this._chip_av_lab[index];
            back.node.opacity = 255;
        }
        // let back = UNodeHelper.find(node, 'Background');
        // if (!back['material'])
        //     back['material'] = ShaderManager.useCostomShader(back.getComponent(cc.Label), new GrayShader());
        // back['material'].setDefine('gray', false);
    }
    ...
    moveToBack(node: cc.Node) {
        node.stopAllActions();
        node.runAction(cc.moveTo(0.2, node.x, UNCHIP_Y));
    }

    moveToUp(node: cc.Node) {
        node.stopAllActions();
        node.runAction(cc.moveTo(0.2, node.x, AVCHIP_Y));
    }
