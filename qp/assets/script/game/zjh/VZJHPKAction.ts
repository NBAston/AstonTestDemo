...
import { UIZJHCompare } from "./UZJHClass";
 ...
 */
@ccclass
export default class VZJHPKAction extends cc.Component {
    /**比牌效果的节点 */
    public _vsNode: cc.Node;
    /**选择比牌的节点 */
    public _bipaiNode: cc.Node;
    /**
     * 看牌
     */
    ...
    private _zjh: VZJH;
    /**左边的玩家 */
    private _vsLeft: VComparePlayer;
    /**右边的玩家 */
    private _vsRight: VComparePlayer;
    /**比较的动画 */
    ...
    /**是否已经点击 */
    init(zjh: VZJH, vs: cc.Node, bipai: cc.Node, res: USpriteFrames) {
        this._zjh = zjh;
        this._vsNode = vs;
        ...
        this._vsLeft = new VComparePlayer(vs_left, vs_leftPai, res, true);
        let vs_rightPai = UNodeHelper.find(vs, "vs_rightpai");
        let vs_right = UNodeHelper.find(vs, "vs_right");
        this._vsRight = new VComparePlayer(vs_right, vs_rightPai, res, false);
    ...
    /**播放角色移动画面 */
    private playRoleAction(comp: VComparePlayer, start: cc.Vec2, end: cc.Vec2, paiStart: cc.Vec2, paiEnd: cc.Vec2): void {
    ...
    private compare(idx: number): void {
        AppGame.ins.zjhModel.requestComparePoker(idx);
        this.hideCompareUI();
    }
    hideCompareUI(): void {
