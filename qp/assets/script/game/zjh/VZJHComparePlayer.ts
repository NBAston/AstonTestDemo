 ...
 * 比牌玩家
 */
export default class VComparePlayer {
    /**节点 */
    ...
    bind(nickName: string, headId: number, gold: number, vip: number, headboxId: number, headImgUrl: string): void {
        ...
        if (AppGame.ins.currRoomKind == 2) {
            ...
            if (nickName == AppGame.ins.roleModel.useId.toString()) {
                this._name.string = nickName;
            } else {
                this._name.string = UStringHelper.coverName(nickName);

            }

        }
        this._gold.string = UStringHelper.formatPlayerCoin((gold * ZJH_SCALE));
    ...
    showwin() {
        ...
        this._win.setAnimation(0, "red_win", false);
    }
    /**隐藏胜利 */
    hidewin(): void {
        this._win.node.active = false;
    ...
    showlose(): void {
        ...
        this._lose.setAnimation(0, "blue_win", false);
    }
    reset(): void {
        this.hidewin();
