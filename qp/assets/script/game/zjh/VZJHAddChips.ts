...
import { UIZJHChips } from "./UZJHClass";
 ...
 */
@ccclass
export default class VAddChips extends cc.Component {
    ...
    init(op): void {
        ...
        for (let i = 0; i < len1; i++) {
            ...
            UEventHandler.addClick(cnd, this.node, "VZJHAddChips", "onselectchip", i)
    ...
    private onselectchip(evt: any, idx: number): void {
        ...
        if (dt.canUse) {
            AppGame.ins.zjhModel.requestJiaZhu(idx);
        }
        this._op.clickmusic();
        this.cuopai_toggle.node.active = true;
    }
