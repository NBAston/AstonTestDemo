 ...
 */
@ccclass
export default class VKPQZNNQZNode extends cc.Component {
    ...
    private addEvent() {
        ...
        UEventHandler.addClick(this._btnBuQiang.node,
            this.node, "VKPQZNNQZNode", "sendBeiShuInfo", 0);
        ...
        // MKPQZNNModel.ins.on(UKPQZNNHelper.QZNN_SELF_EVENT.QZNN_OPEN_QIANGZHUANG, this.openQiangZhuangPanel, this);
    ...
    private removeEvent() {
        ...
        // MKPQZNNModel.ins.off(UKPQZNNHelper.QZNN_SELF_EVENT.QZNN_OPEN_QIANGZHUANG, this.openQiangZhuangPanel, this);
    ...
    private sendBeiShuInfo(event, customEventData) {
        ...
        if (customEventData >= 0) {//负数置灰,不发送
            MKPQZNNModel.ins.sendCallBanker(customEventData);
        }
    }
    ...
    private onCallBanker(data: any) {
        ...
        // cc.log("gMeChairId :" + MKPQZNNModel.ins.gMeChairId);
        // cc.log("wCallBankerUser:" + wCallBankerUser);

        if (wCallBankerUser == MKPQZNNModel.ins.gMeChairId) {
            this.setQZNdoeActive(false);
        }
    ...
    private onGameStart(data: any) {
        ...
        if (cbCallBankerMultiple != null) {
                ...
                if (this._qznnList && this._qznnList[i.toString()]) {
                    ...
                    if (mycbCallBankerMultiple[i] > 0) {
                        ...
                        UEventHandler.addClick(this._qznnList[i.toString()].btn.node,
                            this.node, "VKPQZNNQZNode", "sendBeiShuInfo", mycbCallBankerMultiple[i]);
                    }
                    else {
            ...
            // for (let i = 0; i < 3; i++) {
            //     UEventHandler.addClick(this._qznnList[i.toString()].btn.node,
            //         this.node, "VKPQZNNQZNode", "sendBeiShuInfo", (i + 1));
            // }
        }
    }
    /**抢庄结束 隐藏抢庄按钮*/
