...
import MKPQZNNModel, { QZNN_SELF_SEAT } from "./model/MKPQZNNModel";
...
@ccclass
export default class VKPQZNNCardsNode extends cc.Component {
    ...
    private init() {
        ...
        UEventHandler.addClick(this._btn_seepai.node, this.node, "VKPQZNNCardsNode", "sendOpenCard");
    ...
    /**添加事件 */
    private addEvent() {
        MKPQZNNModel.ins.on(UKPQZNNHelper.QZNN_SELF_EVENT.QZNN_FAPAI_COMPLETE, this.showCardBackByIndex, this);

        MKPQZNNModel.ins.on(UKPQZNNHelper.QZNN_SELF_EVENT.QZNN_SC_GAMESCENE_OPEN, this.onGameSceneOpen, this);
        ...
        MKPQZNNModel.ins.on(UKPQZNNHelper.QZNN_SELF_EVENT.QZNN_SUB_S_OPEN_CARD_RESULT, this.onOpenCardResult, this);

        MKPQZNNModel.ins.on(UKPQZNNHelper.QZNN_SELF_EVENT.QZNN_SUB_S_ADD_SCORE_RESULT, this.onAddScoreResult, this);
        ...
        // MKPQZNNModel.ins.on(UKPQZNNHelper.QZNN_SELF_EVENT.QZNN_PINCARD_DONE, this.onPinCardDone, this);

        MKPQZNNModel.ins.on(UKPQZNNHelper.QZNN_SELF_EVENT.QZNN_SCENE_OPEN_NOT_SEND_CARD, this.openNotSendCard, this);

        AppGame.ins.appStatus.on(AppStatus.GAME_TO_BACK, this.onGameToBack, this);
    }
    private removeEvent() {
        MKPQZNNModel.ins.off(UKPQZNNHelper.QZNN_SELF_EVENT.QZNN_FAPAI_COMPLETE, this.showCardBackByIndex, this);

        MKPQZNNModel.ins.off(UKPQZNNHelper.QZNN_SELF_EVENT.QZNN_SC_GAMESCENE_OPEN, this.onGameSceneOpen, this);
        MKPQZNNModel.ins.off(UKPQZNNHelper.QZNN_SELF_EVENT.QZNN_SC_GAMESCENE_END, this.onGameSceneEnd, this);

        MKPQZNNModel.ins.off(UKPQZNNHelper.QZNN_SELF_EVENT.QZNN_SUB_S_OPEN_CARD_RESULT, this.onOpenCardResult, this);
        
        MKPQZNNModel.ins.off(UKPQZNNHelper.QZNN_SELF_EVENT.QZNN_SUB_S_ADD_SCORE_RESULT, this.onAddScoreResult, this);
        ...
        // MKPQZNNModel.ins.off(UKPQZNNHelper.QZNN_SELF_EVENT.QZNN_PINCARD_DONE, this.onPinCardDone, this);
        MKPQZNNModel.ins.off(UKPQZNNHelper.QZNN_SELF_EVENT.QZNN_SCENE_OPEN_NOT_SEND_CARD, this.openNotSendCard, this);
    ...
    //     let player = MKPQZNNModel.ins.getbattleplayerbySeatId(0);
    ...
    private onAddScoreResult(data: any) {
        MKPQZNNModel.ins.gameStatus = MKPQZNNModel.QZNN_GAMESTATUS.SEND;
    }
    /**
     * 开牌场景消息
     * @param data 
    ...
    private onGameSceneOpen(data: any) {
        ...
        if (cbIsOpenCard != null) {
            if (cbIsOpenCard[MKPQZNNModel.ins.gMeChairId] == 1) {
        ...
        this._btn_seepai.node.active = false;
        let player = MKPQZNNModel.ins.getbattleplayerbySeatId(0);
        if (player && (player.userStatus == 5 || player.userStatus == 6)) {
            this._btn_seepai.node.active = false;
        }
        ...
        let chairId = 0;
        let index = 0;
        let myChairId = MKPQZNNModel.ins.gMeChairId;
        ...
        for (let i = 0; i < this._maxplayer; i++) {
            ...
            let player = MKPQZNNModel.ins.getbattleplayerbySeatId(i);
            if (player != null && player.playStatus == 1 && player.seatId >= 0 && player.seatId < 4) {
                this.showCardBackByIndex(i, cbIsOpenCard);
            }
        }

        if(player.playStatus == 1 && !cbIsOpenCard[MKPQZNNModel.ins.gMeChairId]) {
            let param = {
    ...
    private onOpenCardResult(data: any) {
        ...
        var chairId = MKPQZNNModel.ins.getUISeatId(wOpenCardUser);
    ...
    private playFanPai() {
        ...
        this._cardsList["0"].card4.node.runAction(cc.sequence(cc.scaleTo(0.2, 0, 1.2), cc.callFunc((node) => {
            ...
            sp_card.spriteFrame = UKPQZNNScene.ins.getSpriteFrame(key);
        }, this), cc.scaleTo(0.1, 1, 1)));

        setTimeout(() => {
            this._cardsList["0"].card4.node.setScale(1, 1)
            MKPQZNNModel.ins.sendOpenCard();
    ...
    private showCardBack(index: number) {
        ...
        for (let i = 0; i < 5; i++) {
            ...
            let path = "card" + i.toString();
            let key = "poker_b1";
            this._cardsList[index.toString()][path].spriteFrame = UKPQZNNScene.ins.getSpriteFrame(key);
    ...
    private showCardBackByIndex(index: number, isOpenCard: any = null) {
        ...
        if (index == 0 && this._cbCardData != null) {
            ...
            //MKPQZNNModel.ins.randCardData(this._cbCardData,this._cbCardData.length);
            UDebug.Log("******randCards-------------" + JSON.stringify(randCards));
            if (isOpenCard && !isOpenCard[MKPQZNNModel.ins.gMeChairId]) {
                UDebug.Log('置0')
                randCards[4] = 0;
            }
            ...
            let chairId = MKPQZNNModel.ins.getUIChairId(index);
    ...
    private setCardsSprite(index: number, values: number[]): void {
        ...
        for (let i = 0; i < 5; i++) {
            ...
            let key = MKPQZNNModel.ins.cardValueToSpriteKey1(values[i]);
            if (values[i] == 0) {//无牌值
                key = "poker_b1";
            }

            cards[path].spriteFrame = UKPQZNNScene.ins.getSpriteFrame(key);
    ...
    setSpineCardType(index: number, cardtype: number): void {
        ...
        aniName = UKPQZNNHelper.CARD_TYPE2[cardtype];
        ...
        UKPQZNNScene.ins.getMusic.playPaiXing(cardtype, MKPQZNNModel.ins.getPlayerSexByUISeat(index));
    ...
    showCardTypeAni1(index: number, cbCardData: number[], cbCardType?: number, cbOxCardData?: number[]) {
        ...
        // MKPQZNNModel.ins.sortCardList(cbCardData,cbCardData.length);
        ...
        if (cbCardType != null) {
            ...
            cardtype = MKPQZNNModel.ins.getCardType(cbCardData, cbCardData.length);
        }
        ...
        if (cbOxCardData != null && cbOxCardData.length == 5) {
            ...
            if (niuniuTemp.length == 5) {
                ...
                MKPQZNNModel.ins.getOxCard(cbCardData);
            }

        }
        else {
            //找出牛牛牌 cbCardData 的值已排序过
            var niuniuCard = MKPQZNNModel.ins.getOxCard(cbCardData);
