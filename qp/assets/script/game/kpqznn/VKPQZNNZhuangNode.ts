 ...
 */
@ccclass
export default class VKPQZNNZhuangNode extends cc.Component {
    /**
     * 该节点下需用的dic
    ...
    start() {
        ...
        for (let i = 0; i < this._maxplayer; i++) {
            ...
            this._zhuangNodeList[i.toString()].zhuang_Ani.setCompleteListener((event: any) => {
                UKPQZNNScene.ins.getMusic.playZhuang();
                MKPQZNNModel.ins.emit(UKPQZNNHelper.QZNN_SELF_EVENT.QZNN_DJS_EVENT, null, false);
            });
    ...
    setBanker(index: number) {
        MKPQZNNModel.ins.sBankerIndex = index;
    }
    /**获得庄的座位索引 */
    getBankerZhuangGuang(): number {
        var index = MKPQZNNModel.ins.gBankerIndex;
        var bIndex = MKPQZNNModel.ins.getUISeatId(index);
    ...
    showChooseBankerAni(indexs: number[]) {
        ...
        // UDebug.Log("MKPQZNNModel.ins.gBankerIndex:"+MKPQZNNModel.ins.gBankerIndex );

        if (MKPQZNNModel.ins.gBankerIndex == null) return;
        ...
        if (indexs.length > 1) {
            for (let i = 0; i < indexs.length; i++) {
                ...
                let rep = cc.repeat(cc.sequence(
                    ...
                    cc.fadeOut(0.05),
                    cc.callFunc(() => {
                        UKPQZNNScene.ins.getMusic.playChooseZhuang();
                    })
                ), 4);
                //每个人延迟执行，并加回调
                let act = cc.sequence(cc.delayTime(i * 0.05), rep, cc.callFunc((node) => {

                    // var bIndex = MKPQZNNModel.ins.getUISeatId(MKPQZNNModel.ins.gBankerIndex);
                    if (bIndex == indexs[i]) {
                ...
                MKPQZNNModel.ins.emit(UKPQZNNHelper.QZNN_SELF_EVENT.QZNN_XUAN_ZHUANG_END_EVENT);
        ...
        {
            // var index1 = MKPQZNNModel.ins.gBankerIndex;
            // var bIndex = MKPQZNNModel.ins.getUISeatId(index1);
            ...
            if (bIndex != null && this._zhuangNodeList[bIndex.toString()] != null) {
                ...
                this._zhuangNodeList[bIndex.toString()].zhuang_sprite.node.active = true;
    ...
    private onRandomZhuang(data: any) {
        ...
        if (data.index != null && data.bankers != null) {
            ...
            // UKPQZNNScene.ins.getMusic.playZhuang(MKPQZNNModel.ins.getPlayerSexByRealSeat(data.index));
        }
    }
    ...
    private onCallBankerResult(data) {
        ...
        if (cbAddJettonTime != null) {
            ...
            MKPQZNNModel.ins.emit(UKPQZNNHelper.QZNN_SELF_EVENT.QZNN_DJS_EVENT, djsData, true);
        }
        ...
        if (cbRandBanker == 1) {
            ...
            for (let i = 0; i < cbCallBankerUser.length; i++) {
                ...
                if (element > 0) {
                    var index = MKPQZNNModel.ins.getUISeatId(i);
                    dic[index] = element;
