 ...
 */
@ccclass
export default class VKPQZNNPlayersNode extends cc.Component {
    ...
    private addEvent() {
        // MKPQZNNModel.ins.on(UKPQZNNHelper.QZNN_SELF_EVENT.QZNN_UPDATE_ROLEINFO, this.updateMyRoleInfo, this);

        MKPQZNNModel.ins.on(UKPQZNNHelper.QZNN_SELF_EVENT.QZNN_SUB_S_GAME_START, this.onGamestart, this);
        MKPQZNNModel.ins.on(UKPQZNNHelper.QZNN_SELF_EVENT.QZNN_SUB_S_CALL_BANKER, this.onCallBanker, this);
        MKPQZNNModel.ins.on(UKPQZNNHelper.QZNN_SELF_EVENT.QZNN_SUB_S_ADD_SCORE_RESULT, this.onAddScoreResult, this);


        MKPQZNNModel.ins.on(UKPQZNNHelper.QZNN_SELF_EVENT.QZNN_XUAN_ZHUANG_END_EVENT, this.onXuanZhuangEnd, this);
        MKPQZNNModel.ins.on(UKPQZNNHelper.QZNN_SELF_EVENT.QZNN_TOTALSCORE_EVENT, this.onTotalScore, this);

        MKPQZNNModel.ins.on(UKPQZNNHelper.QZNN_SELF_EVENT.QZNN_UPDATE_PLAYERS_EVENT, this.onUpdatePlayers, this);
    ...
    private removeEvent() {
        // MKPQZNNModel.ins.off(UKPQZNNHelper.QZNN_SELF_EVENT.QZNN_UPDATE_ROLEINFO, this.updateMyRoleInfo, this);

        MKPQZNNModel.ins.off(UKPQZNNHelper.QZNN_SELF_EVENT.QZNN_SUB_S_GAME_START, this.onGamestart, this);
        MKPQZNNModel.ins.off(UKPQZNNHelper.QZNN_SELF_EVENT.QZNN_SUB_S_CALL_BANKER, this.onCallBanker, this);
        ...
        MKPQZNNModel.ins.off(UKPQZNNHelper.QZNN_SELF_EVENT.QZNN_XUAN_ZHUANG_END_EVENT, this.onXuanZhuangEnd, this);
        MKPQZNNModel.ins.off(UKPQZNNHelper.QZNN_SELF_EVENT.QZNN_TOTALSCORE_EVENT, this.onTotalScore, this);

        MKPQZNNModel.ins.off(UKPQZNNHelper.QZNN_SELF_EVENT.QZNN_UPDATE_PLAYERS_EVENT, this.onUpdatePlayers, this);
    ...
    private onResetScene(data?: any) {
        for (let i = 0; i < this._maxplayer; i++) {
            ...
            {
                var myscore = UKPQZNNScene.ins.myScore;
                if (myscore == 0) {
        ...
        this.resetPopPos();
        MKPQZNNModel.ins.updateBattlePlayer();
        //         if (element.seatId == 0) {
    ...
    setPlayerWsz(index: number, isShow: boolean): void {
        ...
        if (!MKPQZNNModel.ins.isGaming && isShow) {
            this._players[index.toString()].wsz.active = false;
        }
    }
    ...
    private onUpdatePlayers(data: any) {
        ...
        for (const key in data) {
            if (data.hasOwnProperty(key)) {
                ...
                let chairId = MKPQZNNModel.ins.getUISeatId(element.chairId);
                if (!element.isExit && element.playStatus >= 0 && chairId != null) {
                    let name = AppGame.ins.currRoomKind == ERoomKind.Club ? element.nickName : element.userId;
                    this.setPlayerNickName(chairId, name);
                    if (chairId != 0) {
    ...
    private onAddScoreResult(data) {
        ...
        //#region  test 0-3 
        // MKPQZNNModel.ins.sMeChairId = 0;
        //#endregion

        let wAddJettonUser = data.addJettonUser;
        let cbJettonMultiple = data.jettonMultiple;

        let chairId = MKPQZNNModel.ins.getUISeatId(wAddJettonUser);
    ...
    private onCallBanker(data) {
        ...
        let chairId = MKPQZNNModel.ins.getUISeatId(wCallBankerUser);

        this.setQZ_XZActive(chairId, true);
        this.setQZBSJettonMultiple(chairId, cbCallMultiple);
        this.setQPAction(chairId);

        var pos = MKPQZNNModel.ins.getPlayerSexByRealSeat(wCallBankerUser);
        if (cbCallMultiple > 0) {
            UKPQZNNScene.ins.getMusic.playQiangZhung(true, pos);
        }
        else {
            UKPQZNNScene.ins.getMusic.playQiangZhung(false, pos);
        }
    }
    ...
    private onXuanZhuangEnd(data) {
        // var bankindex = MKPQZNNModel.ins.gBankerIndex;//data.banker;
        // var index = MKPQZNNModel.ins.getUISeatId(bankindex);
        // //不抢，随机选庄后 变为抢1
