 ...
 */
@ccclass
export default class VKPQZNNXiaZhuNode extends cc.Component {
    ...
    private addEvent() {
        MKPQZNNModel.ins.on(UKPQZNNHelper.QZNN_SELF_EVENT.QZNN_RESET_SCENE, this.onResetGame, this);

        MKPQZNNModel.ins.on(UKPQZNNHelper.QZNN_SELF_EVENT.QZNN_SUB_S_ADD_SCORE_RESULT, this.onAddScoreResult, this);
        MKPQZNNModel.ins.on(UKPQZNNHelper.QZNN_SELF_EVENT.QZNN_SC_GAMESCENE_SCORE, this.onGameSceneScore, this);
        MKPQZNNModel.ins.on(UKPQZNNHelper.QZNN_SELF_EVENT.QZNN_SUB_S_SEND_CARD, this.onSendCard, this);

        MKPQZNNModel.ins.on(UKPQZNNHelper.QZNN_SELF_EVENT.QZNN_SUB_S_CALL_BANKER_RESULT, this.onCallBankerResult, this);
        MKPQZNNModel.ins.on(UKPQZNNHelper.QZNN_SELF_EVENT.QZNN_XUAN_ZHUANG_END_EVENT, this.onXuanZhuangEnd, this);

    }

    private removeEvent() {
        MKPQZNNModel.ins.off(UKPQZNNHelper.QZNN_SELF_EVENT.QZNN_RESET_SCENE, this.onResetGame, this);
        ...
        MKPQZNNModel.ins.off(UKPQZNNHelper.QZNN_SELF_EVENT.QZNN_SC_GAMESCENE_SCORE, this.onGameSceneScore, this);

        MKPQZNNModel.ins.off(UKPQZNNHelper.QZNN_SELF_EVENT.QZNN_SUB_S_CALL_BANKER_RESULT, this.onCallBankerResult, this);
    ...
    updateXiazhuBtns(zhus: number[], length1?: number, isScene?: boolean) {
        ...
        if (isScene == undefined || !isScene) {//不是重连的话，是发四个人的
            let mpl = MKPQZNNModel.ins.gMeChairId;
            // let playernum = MKPQZNNModel.ins.getBattlePlayerLength() || 4;
            ...
            //     let baplayers = MKPQZNNModel.ins.gBattlePlayer
            ...
            for (let i = 0; i < length; i++) {
                ...
                // item.setPosition(startX + startW * i * item.scaleX + 100, item.getPosition().y);
                ...
                if (mycbJettonMultiple[i] > 0) {
                    ...
                    UEventHandler.addClick(item, this.node, "VKPQZNNXiaZhuNode", "sendXiaZhuInfo", i);//zhus[i] 点击的要传什么
            ...
            for (let i = 0; i < length; i++) {
                ...
                if (zhus[i] > 0) {
                    item.getComponent(cc.Button).interactable = true;
                    label.node.active = true;
                    UEventHandler.addClick(item, this.node, "VKPQZNNXiaZhuNode", "sendXiaZhuInfo", i);//zhus[i]
    ...
    private sendXiaZhuInfo(event, customEventData) {
        ...
        MKPQZNNModel.ins.sendAddScore(customEventData);
    }
    ...
    private onAddScoreResult(data: any) {
        ...
        // let chairId = MKPQZNNModel.ins.changeToLocalPos(wAddJettonUser);

        // UDebug.Log("gMeChairId :" + MKPQZNNModel.ins.gMeChairId);
        // UDebug.Log("wAddJettonUser：" + wAddJettonUser);

        // UDebug.Log("onAddScoreResult:" + JSON.stringify(data));

        if (wAddJettonUser == MKPQZNNModel.ins.gMeChairId) {
            this.setXiaZhuNodeActive(false);
        }
    ...
    private onXuanZhuangEnd(data: any) {
        var dwBankerUser = MKPQZNNModel.ins.gBankerIndex;
