 ...
 * qznn的音乐播放器
 */
export default class UKPQZNNMusic_hy {
    ...
    playPaiXing(cardtype: number, sex: number): void {
        ...
        if (cardtype >= 0 && cardtype < 15) {
            aniName = sex == 0 ? ("audio_kpqznn_man_" + cardtype.toString()) : ("audio_kpqznn_woman_" + cardtype.toString());
        }
        ...
        //11以后再说 规则不一样
        if (aniName != null) {
            UAudioManager.ins.playGameSound(AppGame.ins.roomModel.BundleName,aniName);
        }
    }
