 ...
 */
@ccclass
export default class VKPQZNNZhuangNode_hy extends cc.Component {
    /**
     * 该节点下需用的dic
    ...
    start() {
        ...
        for (let i = 0; i < this._maxplayer; i++) {
            ...
            this._zhuangNodeList[i.toString()].zhuang_Ani.setCompleteListener((event: any) => {
                UKPQZNNScene_hy.ins.getMusic.playZhuang();
                MKPQZNNModel_hy.ins.emit(UKPQZNNHelper_hy.QZNN_SELF_EVENT.QZNN_DJS_EVENT, null, false);
            });
    ...
    setBanker(index: number) {
        MKPQZNNModel_hy.ins.sBankerIndex = index;
    }
    /**获得庄的座位索引 */
    getBankerZhuangGuang(): number {
        var index = MKPQZNNModel_hy.ins.gBankerIndex;
        var bIndex = MKPQZNNModel_hy.ins.getUISeatId(index);
    ...
    showChooseBankerAni(indexs: number[]) {
        ...
        // UDebug.Log("MKPQZNNModel_hy.ins.gBankerIndex:"+MKPQZNNModel_hy.ins.gBankerIndex );

        if (MKPQZNNModel_hy.ins.gBankerIndex == null) return;
        ...
        if (indexs.length > 1) {
            for (let i = 0; i < indexs.length; i++) {
                ...
                let rep = cc.repeat(cc.sequence(
                    ...
                    cc.fadeOut(0.05),
                    cc.callFunc(() => {
                        UKPQZNNScene_hy.ins.getMusic.playChooseZhuang();
                    })
                ), 4);
                //每个人延迟执行，并加回调
                let act = cc.sequence(cc.delayTime(i * 0.05), rep, cc.callFunc((node) => {

                    // var bIndex = MKPQZNNModel_hy.ins.getUISeatId(MKPQZNNModel_hy.ins.gBankerIndex);
                    if (bIndex == indexs[i]) {
                ...
                MKPQZNNModel_hy.ins.emit(UKPQZNNHelper_hy.QZNN_SELF_EVENT.QZNN_XUAN_ZHUANG_END_EVENT);
        ...
        {
            // var index1 = MKPQZNNModel_hy.ins.gBankerIndex;
            // var bIndex = MKPQZNNModel_hy.ins.getUISeatId(index1);
    ...
    private onRandomZhuang(data: any) {
        ...
        if (data.index != null && data.bankers != null) {
            ...
            // UKPQZNNScene_hy.ins.getMusic.playZhuang(MKPQZNNModel_hy.ins.getPlayerSexByRealSeat(data.index));
        }
    }
    ...
    private onCallBankerResult(data) {
        ...
        if (cbAddJettonTime != null) {
            ...
            MKPQZNNModel_hy.ins.emit(UKPQZNNHelper_hy.QZNN_SELF_EVENT.QZNN_DJS_EVENT, djsData, true);
        }
        ...
        if (cbRandBanker == 1) {
            ...
            for (let i = 0; i < cbCallBankerUser.length; i++) {
                ...
                if (element > 0) {
                    var index = MKPQZNNModel_hy.ins.getUISeatId(i);
                    dic[index] = element;
