 ...
 */
@ccclass
export default class VKPQZNNAniNode_hy extends cc.Component {
    /**赢家框 */
    ...
    private addEvent() {
        // this._eventTongsha = MKPQZNNModel_hy.ins.on(UKPQZNNHelper_hy.QZNN_SELF_EVENT.QZNN_TONGSHA_EVENT, this.onTongSha, this);
        // this._eventTongpei = MKPQZNNModel_hy.ins.on(UKPQZNNHelper_hy.QZNN_SELF_EVENT.QZNN_TONGPEI_EVENT, this.onTongPei, this);

        MKPQZNNModel_hy.ins.on(UKPQZNNHelper_hy.QZNN_SELF_EVENT.QZNN_SUB_S_GAME_START, this.showGameStart, this);
        ...
        MKPQZNNModel_hy.ins.on(UKPQZNNHelper_hy.QZNN_SELF_EVENT.QZNN_TOTALSCORE_EVENT, this.onTotalScore, this);//分数滚动完事件
        MKPQZNNModel_hy.ins.on(UKPQZNNHelper_hy.QZNN_SELF_EVENT.QZNN_RESET_SCENE, this.onResetScene, this);//重置场景事件
    ...
    private removeEvent() {
        MKPQZNNModel_hy.ins.off(UKPQZNNHelper_hy.QZNN_SELF_EVENT.QZNN_SUB_S_GAME_START, this.showGameStart, this);
    ...
    private onTongSha(data?: any) {
        ...
        UKPQZNNScene_hy.ins.playTongsha();
    }

    private onTongPei(data?: any) {
        this.setTongPeiAniActive(true);

        UKPQZNNScene_hy.ins.playTongpei();
    }
    ...
    private playCoinFlyAni(start: number, end: number, isMusicPlayOne: boolean = false) {
        ...
        if (!isMusicPlayOne) {
            /**播5次声音 */
            for (let j = 0; j < 5; j++) {
                UKPQZNNScene_hy.ins.getMusic.playflyCoin();
            }
        }
        ...
        if (players != null) {
            for (const key in players) {
                if (players.hasOwnProperty(key)) {
                    ...
                    if (element.playStatus == 5 || element.playStatus == 6)//上座玩家
                    {
    ...
    private onGameEnd(data: any) {
        ...
        if (dLWScore != null) {
            ...
            var banker = MKPQZNNModel_hy.ins.gBankerIndex;
            var bankerLocalPos = MKPQZNNModel_hy.ins.getUISeatId(banker);
            ...
            if (bankerLocalPos != null)//有庄家
            {
                if (cbEndType == 1) {//通杀
                    ...
                    for (let j = 0; j < dLWScore.length; j++) {
                        let player = MKPQZNNModel_hy.ins.getbattleplayerbyChairId(j);
                        if (player != null && (player.playStatus == 5 || player.playStatus == 6) && player.seatId >= 0 && player.seatId < 4) {

                            let index1 = MKPQZNNModel_hy.ins.getUISeatId(j);
                    ...
                    for (let j = 0; j < dLWScore.length; j++) {
                        ...
                        if (player != null && (player.playStatus == 5 || player.playStatus == 6) && player.seatId >= 0 && player.seatId < 4) {
                            ...
                            let index2 = MKPQZNNModel_hy.ins.getUISeatId(j);
                ...
                for (let i = 0; i < dLWScore.length; i++) {
                    ...
                    let player = MKPQZNNModel_hy.ins.getbattleplayerbyChairId(i);
                    if (player != null && (player.playStatus == 5 || player.playStatus == 6) && player.seatId >= 0 && player.seatId < 4) {
    ...
    sendFapaiCompleteEvent(index: number) {
        ...
        MKPQZNNModel_hy.ins.emit(UKPQZNNHelper_hy.QZNN_SELF_EVENT.QZNN_FAPAI_COMPLETE, index);
    ...
    private setFaPaiAni(index: number, b?: boolean) {
        ...
        if (b) {
            for (let j = 0; j < 5; j++) {
                ...
                // UKPQZNNScene_hy.ins.getMusic.playSendCard();
            }
        }
    }
    ...
    setGameStartAniActive(b: boolean) {
        ...
        if (b == true) {
            ...
            UKPQZNNScene_hy.ins.getMusic.playStart();
        }
    }
    /**庄家通杀动画 */
