 ...
 */
@ccclass
export default class VTBNNAniNode_hy extends cc.Component {
    /**赢家框 */
    ...
    start() {
        ...
        */
        ...
        this._winAni = UNodeHelper.getComponent(this.node, "win/qznnWinAni", sp.Skeleton);
        this._winAni.setCompleteListener((event) => {
            self.setWinAniActive(false);
        });
        this.setWinAniActive(false);

        this._gameStartAni = UNodeHelper.getComponent(this.node, "start/qznnStartAni", sp.Skeleton);
        this._gameStartAni.setCompleteListener((event) => {
            ...
            //     }, 1.5)
            //     MTBNNModel_hy.ins.emit(UTBNNHelper_hy.TBNN_SELF_EVENT.TBNN_SUB_S_SEND_CARD);
            // }
            self.setGameStartAniActive(false);
        });
        this.setGameStartAniActive(false);
    ...
    private addEvent() {
        // this._eventTongsha = MTBNNModel_hy.ins.on(UTBNNHelper_hy.TBNN_SELF_EVENT.TBNN_TONGSHA_EVENT, this.onTongSha, this);
        // this._eventTongpei = MTBNNModel_hy.ins.on(UTBNNHelper_hy.TBNN_SELF_EVENT.TBNN_TONGPEI_EVENT, this.onTongPei, this);

        MTBNNModel_hy.ins.on(UTBNNHelper_hy.TBNN_SELF_EVENT.TBNN_SUB_S_GAME_START, this.showGameStart, this);
        ...
        MTBNNModel_hy.ins.on(UTBNNHelper_hy.TBNN_SELF_EVENT.TBNN_TOTALSCORE_EVENT, this.onTotalScore, this);//分数滚动完事件
        MTBNNModel_hy.ins.on(UTBNNHelper_hy.TBNN_SELF_EVENT.TBNN_RESET_SCENE, this.onResetScene, this);//重置场景事件
    ...
    private removeEvent() {
        MTBNNModel_hy.ins.off(UTBNNHelper_hy.TBNN_SELF_EVENT.TBNN_SUB_S_GAME_START, this.showGameStart, this);
    ...
    private onTongSha(data?: any) {
        ...
        UTBNNScene_hy.ins.playTongsha();
    }

    private onTongPei(data?: any) {
        this.setTongPeiAniActive(true);

        UTBNNScene_hy.ins.playTongpei();
    }
    ...
    private playCoinFlyAni(start: number, end: number) {
        ...
        /**播10次声音 */
        //for (let j = 0; j < 5; j++) {
        //  UTBNNScene_hy.ins.getMusic.playflyCoin();
        //}
        this.scheduleOnce(() =>{
        ...
        if (players != null) {
            for (const key in players) {
                if (players.hasOwnProperty(key)) {
                    ...
                    if (element.playStatus == 5 || element.playStatus == 6)    
                    {
    ...
    private onGameEnd(data: any,pl:any) {
        ...
        if (dLWScore != null) {
            ...
            var banker = MTBNNModel_hy.ins.gBankerIndex;
            var bankerLocalPos = MTBNNModel_hy.ins.getUISeatId(banker);
            ...
            for (let i = 0; i < dLWScore.length; i++) {
                let player = MTBNNModel_hy.ins.getbattleplayerbyChairId(i,pl)
                if (player != null && (player.playStatus == 5 || player.playStatus == 6) && player.seatId >= 0 && player.seatId < 4) {
                    let score = dLWScore[i]
                    let sead_id = MTBNNModel_hy.ins.getUISeatId(i)
                    if (score > 0) {
                        dindex = sead_id
                    } else {
                        toPlayers.push(sead_id)
                    }

                    if (player.userId == AppGame.ins.roleModel.useId) {
                        if (score > 0) {
                            this._myScore = 1;
                        }else if (score < 0) {
                            this._myScore = -1;
            ...
            this.scheduleOnce(() => {
                for (let j = 0; j < toPlayers.length; j++) {
                    ...
                    self.playCoinFlyAni(element, dindex);
                }
            ...
            //             let player = MTBNNModel_hy.ins.getbattleplayerbyChairId(j);
            //             if (player != null && player.playStatus == 5 && player.seatId >= 0 && player.seatId < 4) {

            //                 let index1 = MTBNNModel_hy.ins.getUISeatId(j);
            ...
            //                 let index2 = MTBNNModel_hy.ins.getUISeatId(j);
            ...
            //         let player = MTBNNModel_hy.ins.getbattleplayerbyChairId(i);
            //         if (player != null && player.playStatus == 5 && player.seatId >= 0 && player.seatId < 4) {
    ...
    sendFapaiCompleteEvent(index: number) {
        ...
        MTBNNModel_hy.ins.emit(UTBNNHelper_hy.TBNN_SELF_EVENT.TBNN_FAPAI_COMPLETE, index);
    ...
    private setFaPaiAni(index: number, b?: boolean) {
        ...
        if (b) {
            for (let j = 0; j < 5; j++) {
                ...
                // UTBNNScene_hy.ins.getMusic.playSendCard();
            }
        }
    ...
    moveCardTo(node: cc.Node, playerIndex: number, cardIndex: number) {
        ...
        var desc_scalex = 0.62;
        var desc_scaley = 0.62;
        // var act_time = 0.5;
    ...
    setGameStartAniActive(b: boolean) {
        ...
        if (b == true) {
            ...
            UTBNNScene_hy.ins.getMusic.playStart();
        }
    }
    /**庄家通杀动画 */
