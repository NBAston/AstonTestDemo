 ...
 */
@ccclass
export default class VTBNNZhuangNode_hy extends cc.Component {
    /**
     * 该节点下需用的dic
    ...
    start() {
        ...
        for (let i = 0; i < this._maxplayer; i++) {
            ...
            this._zhuangNodeList[i.toString()].zhuang_Ani.setCompleteListener((event: any) => {
                UTBNNScene_hy.ins.getMusic.playZhuang();
                MTBNNModel_hy.ins.emit(UTBNNHelper_hy.TBNN_SELF_EVENT.TBNN_DJS_EVENT, null, false);
            });
    ...
    private addEvent(): void {
        MTBNNModel_hy.ins.on(UTBNNHelper_hy.TBNN_SELF_EVENT.TBZNN_SUB_S_CALL_BANKER_RESULT, this.onCallBankerResult, this);
        MTBNNModel_hy.ins.on(UTBNNHelper_hy.TBNN_SELF_EVENT.TBNN_RESET_SCENE, this.onResetScene, this);

    }
    private removeEvent(): void {
        MTBNNModel_hy.ins.off(UTBNNHelper_hy.TBNN_SELF_EVENT.TBZNN_SUB_S_CALL_BANKER_RESULT, this.onCallBankerResult, this);
    ...
    setBanker(index: number) {
        MTBNNModel_hy.ins.sBankerIndex = index;
    }
    /**获得庄的座位索引 */
    getBankerZhuangGuang(): number {
        var index = MTBNNModel_hy.ins.gBankerIndex;
        var bIndex = MTBNNModel_hy.ins.getUISeatId(index);
    ...
    showChooseBankerAni(indexs: number[]) {
        ...
        // UDebug.Log("MTBNNModel_hy.ins.gBankerIndex:"+MTBNNModel_hy.ins.gBankerIndex );

        if (MTBNNModel_hy.ins.gBankerIndex == null) return;
        ...
        if (indexs.length > 1) {
            for (let i = 0; i < indexs.length; i++) {
                ...
                let rep = cc.repeat(cc.sequence(
                    ...
                    cc.fadeOut(0.05),
                    cc.callFunc(() => {
                        UTBNNScene_hy.ins.getMusic.playChooseZhuang();
                    })
                ), 4);
                //每个人延迟执行，并加回调
                let act = cc.sequence(cc.delayTime(i * 0.05), rep, cc.callFunc((node) => {

                    // var bIndex = MTBNNModel_hy.ins.getUISeatId(MTBNNModel_hy.ins.gBankerIndex);
                    if (bIndex == indexs[i]) {
                        ...
                        MTBNNModel_hy.ins.emit(UTBNNHelper_hy.TBNN_SELF_EVENT.TBNN_XUAN_ZHUANG_END_EVENT);
        ...
        {
            // var index1 = MTBNNModel_hy.ins.gBankerIndex;
            // var bIndex = MTBNNModel_hy.ins.getUISeatId(index1);
    ...
    private onRandomZhuang(data: any) {
        ...
        if (data.index != null && data.bankers != null) {
            ...
            // UTBNNScene_hy.ins.getMusic.playZhuang(MTBNNModel_hy.ins.getPlayerSexByRealSeat(data.index));
        }
    }
    ...
    private onCallBankerResult(data) {
        ...
        if (cbAddJettonTime != null) {
            ...
            MTBNNModel_hy.ins.emit(UTBNNHelper_hy.TBNN_SELF_EVENT.TBNN_DJS_EVENT, djsData, true);
        }
        MTBNNModel_hy.ins.emit(UTBNNHelper_hy.TBNN_SELF_EVENT.TBNN_XUAN_ZHUANG_END_EVENT);
        return
        ...
        if (cbRandBanker == 1) {
            ...
            for (let i = 0; i < cbCallBankerUser.length; i++) {
                ...
                if (element > 0) {
                    var index = MTBNNModel_hy.ins.getUISeatId(i);
                    dic[index] = element;
