 ...
 */
@ccclass
export default class VTBNNResultNode_hy extends cc.Component {
    /**
     * 4个座位的 输赢分显示节点
     * @param win 正数label
     * @param lose 负数label
    ...
    onGameSceneEnd(data: any) {
        ...
        if (data.gameEnd) {
            this.onGameEnd(data.gameEnd, pl, true);
        }
    }
    ...
    private showEndScore(data: any,pl:any) {
        ...
        var players = pl;//MTBNNModel_hy.ins.gBattlePlayer;
        ...
        if (players != null) {
            for (const key in players) {
                if (players.hasOwnProperty(key)) {
                    ...
                    if ((element.playStatus == 5 || element.playStatus == 6) && dLWScore != null && dLWScore[element.chairId] != null)//上座玩家
                    {
     ...
     * @param data 
     */
    private onGameEnd(data,pl, isEndScene: boolean = false) {
    ...
    setPosIndexActive(index: number, b: boolean) {
        if(this._list && this._list[index.toString()]){
            ...
            if (iswin) {
                ...
                this._list[index].label.node.runAction(cc.sequence(cc.moveTo(1.0, this._targetPos[index]), cc.callFunc(() => {
                    ...
                    if (this._totalscore[index] != null) {
                        ...
                        MTBNNModel_hy.ins.emit(UTBNNHelper_hy.TBNN_SELF_EVENT.TBNN_TOTALSCORE_EVENT, data);
                    }
                }, this)));
