...
import { ETBNNUIzIndex } from "./UTBNNHelper_hy";
 ...
 */
@ccclass
export default class VTBNNSMNode_hy extends VBaseUI {

    start () {
        this.setZIndex(ETBNNUIzIndex.Top); //显示在最上层
        let sm_bg = UNodeHelper.find(this.node,"qznn_sm_bg");
        let btn_close = UNodeHelper.find(this.node,"qznn_sm_bg/qznn_sm_btn");

        UEventHandler.addClick(btn_close, this.node, "VTBNNSMNode_hy", "onBtnCloseClick");
    ...
    show(data:any){
        ...
        MTBNNModel_hy.ins.sIsOpenSM = true;
    }
