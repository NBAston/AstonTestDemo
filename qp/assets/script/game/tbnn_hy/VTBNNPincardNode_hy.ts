import VTBNNPinCard_hy from "./VTBNNPinCard_hy";
 ...
 */
@ccclass
export default class VTBNNPinCardNode_hy extends cc.Component {
    ...
    /**挂在5张牌的脚本 数组 */
    private _pingcardsCom: Array<VTBNNPinCard_hy> = [];
     ...
     * @param index 对应的VTBNNPinCard实例索引
     */
    private pinDic = {
        '0': {
            value: 0,
    ...
    init(): void {
        for (let i = 0; i < this.pincards.length; i++) {
            let cardCom = this.pincards[i].getComponent(VTBNNPinCard_hy);
            cardCom.setIndex = i;
        ...
        UEventHandler.addClick(this._btn_YN.node, this.node, "VTBNNPinCardNode_hy", "onBtnYNClick");
        UEventHandler.addClick(this._btn_MN.node, this.node, "VTBNNPinCardNode_hy", "onBtnMNClick");
    ...
    addEvent() {
        //监听分发的牌
        var event1 = MTBNNModel_hy.ins.on(UTBNNHelper_hy.TBNN_SELF_EVENT.TBNN_PINPAI_ADD, this.addPinCardEvent, this);
        var event2 = MTBNNModel_hy.ins.on(UTBNNHelper_hy.TBNN_SELF_EVENT.TBNN_PINPAI_DEL, this.delPinCardEvent, this);
    ...
    removeEvent() {
        ...
        //     // MTBNNModel_hy.ins.off(this._eventList.pop());
        // }

        MTBNNModel_hy.ins.off(UTBNNHelper_hy.TBNN_SELF_EVENT.TBNN_PINPAI_ADD, this.addPinCardEvent, this);
        MTBNNModel_hy.ins.off(UTBNNHelper_hy.TBNN_SELF_EVENT.TBNN_PINPAI_DEL, this.delPinCardEvent, this);
    }
    ...
    setCardValue(arr: Array<number>) {
        ...
        for (let i = 0; i < arr.length; i++) {
            const element = arr[i];
            var key = MTBNNModel_hy.ins.cardValueToSpriteKey1(element);
            // UDebug.log(key);
            this.pincards[i].spriteFrame = UTBNNScene_hy.ins.getSpriteFrame(key);
    ...
    private onBtnYNClick(event) {
        ...
        // MTBNNModel_hy.ins.emit(UTBNNHelper_hy.TBNN_SELF_EVENT.TBNN_PINCARD_DONE);

        this.hide();
    }
    /**无牛按钮点击事件 */
    private onBtnMNClick(event) {
        var isYouNiu = MTBNNModel_hy.ins.getOxCard(this._cardData);
    ...
    private getPinNum(num: number): number {
        let value = MTBNNModel_hy.ins.getCardValue(num);
        let value1 = MTBNNModel_hy.ins.getCardTrueValue(value);
        // UDebug.log(value1);
        return value1;
    }
    ...
    testGetCardType() {
        ...
        let type1 = MTBNNModel_hy.CARDTYPE_NAME[MTBNNModel_hy.ins.getCardType(card1, card1.length)];
        UDebug.log("牌型 :" + type1 + " 牌值" + card1);

        let card2 = [0x2C, 0x2C, 0x3D, 0x2B, 0x1D];//五花牛
        let type2 = MTBNNModel_hy.CARDTYPE_NAME[MTBNNModel_hy.ins.getCardType(card2, card2.length)];
        UDebug.log("牌型 :" + type2 + " 牌值" + card2);

        let card3 = [0x2B, 0x3D, 0x3D, 0x3B, 0x1A];//四花牛
        let type3 = MTBNNModel_hy.CARDTYPE_NAME[MTBNNModel_hy.ins.getCardType(card3, card3.length)];
        UDebug.log("牌型 :" + type3 + " 牌值" + card3);

        let card4 = [0x07, 0x1D, 0x0D, 0x3D, 0x2D];//炸弹
        let type4 = MTBNNModel_hy.CARDTYPE_NAME[MTBNNModel_hy.ins.getCardType(card4, card4.length)];
        UDebug.log("牌型 :" + type4 + " 牌值" + card4);

        let card5 = [0x32, 0x12, 0x11, 0x33, 0x21];//五小牛
        let type5 = MTBNNModel_hy.CARDTYPE_NAME[MTBNNModel_hy.ins.getCardType(card5, card5.length)];
