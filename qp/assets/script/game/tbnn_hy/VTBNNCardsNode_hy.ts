...
import VTBNNPincardNode_hy from "./VTBNNPincardNode_hy";
...
@ccclass
export default class VTBNNCardsNode_hy extends cc.Component {

    private _pincardNode: VTBNNPincardNode_hy = null;
    /**
     * 牌组节点下的各节点
     * @param card0-4 :牌
     * @param node 牌组的牌的父节点
    ...
    /**初始化各组件 */
    private init() {
        this._pincardNode = UNodeHelper.getComponent(this.node, "pincardsNode", VTBNNPincardNode_hy);

        this._btn_seepai = UNodeHelper.getComponent(this.node, "btn_seepai", cc.Button);
        UEventHandler.addClick(this._btn_seepai.node, this.node, "VTBNNCardsNode_hy", "sendOpenCard");
    ...
    /**添加事件 */
    private addEvent() {
        MTBNNModel_hy.ins.on(UTBNNHelper_hy.TBNN_SELF_EVENT.TBNN_FAPAI_COMPLETE, this.showCardBackByIndex, this);

        MTBNNModel_hy.ins.on(UTBNNHelper_hy.TBNN_EVENT.TBNN_GAMESCENE_SCORE, this.onGameSceneScore, this);
        MTBNNModel_hy.ins.on(UTBNNHelper_hy.TBNN_SELF_EVENT.TBNN_SC_GAMESCENE_OPEN, this.onGameSceneOpen, this);
        MTBNNModel_hy.ins.on(UTBNNHelper_hy.TBNN_SELF_EVENT.TBNN_SC_GAMESCENE_END, this.onGameSceneEnd, this);

        MTBNNModel_hy.ins.on(UTBNNHelper_hy.TBNN_SELF_EVENT.TBNN_SUB_S_OPEN_CARD_RESULT, this.onOpenCardResult, this);
        ...
        // MTBNNModel_hy.ins.on(UTBNNHelper_hy.TBNN_SELF_EVENT.TBNN_PINCARD_DONE, this.onPinCardDone, this);
    ...
    private removeEvent() {
        MTBNNModel_hy.ins.off(UTBNNHelper_hy.TBNN_SELF_EVENT.TBNN_FAPAI_COMPLETE, this.showCardBackByIndex, this);

        MTBNNModel_hy.ins.off(UTBNNHelper_hy.TBNN_EVENT.TBNN_GAMESCENE_SCORE, this.onGameSceneScore, this);
        MTBNNModel_hy.ins.off(UTBNNHelper_hy.TBNN_SELF_EVENT.TBNN_SC_GAMESCENE_OPEN, this.onGameSceneOpen, this);
        MTBNNModel_hy.ins.off(UTBNNHelper_hy.TBNN_SELF_EVENT.TBNN_SC_GAMESCENE_END, this.onGameSceneEnd, this);

        MTBNNModel_hy.ins.off(UTBNNHelper_hy.TBNN_SELF_EVENT.TBNN_SUB_S_OPEN_CARD_RESULT, this.onOpenCardResult, this);
        ...
        // MTBNNModel_hy.ins.off(UTBNNHelper_hy.TBNN_SELF_EVENT.TBNN_PINCARD_DONE, this.onPinCardDone, this);
    ...
    private onAddScoreResult(data: any) {
        // MTBNNModel_hy.ins.gameStatus = MTBNNModel_hy.TBNN_GAMESTATUS.SEND;
        MTBNNModel_hy.ins.gameStatus = MTBNNModel_hy.TBNN_GAMESTATUS.OPEN;
    }

    /**下注场景消息 */
    onGameSceneScore(data: any) {
        for (let i = 0; i < this._maxplayer; i++) {
            let player = MTBNNModel_hy.ins.getbattleplayerbySeatId(i);
    ...
    private onGameSceneOpen(data: any) {
        ...
        if (cbIsOpenCard != null) {
            if (cbIsOpenCard[MTBNNModel_hy.ins.gMeChairId] == 1) {
     ...
     * @param data 
     */
    private onOpenCardResult(data: FTbnn.NN_CMD_S_OpenCardResult) {
        // UDebug.Log("开牌结果 onOpenCardResult:" + JSON.stringify(data));
        ...
        var chairId = MTBNNModel_hy.ins.getUISeatId(wOpenCardUser);
     ...
     * @param data 
     */
    private onSendCard(data: FTbnn.NN_CMD_S_SendCard) {
        if (!data) return;
        ...
        MTBNNModel_hy.ins.emit(UTBNNHelper_hy.TBNN_SELF_EVENT.TBNN_MOVE_NEXT_CMD);
    }

    private sendOpenCard() {
        UTBNNScene_hy.ins.playClick();
        MTBNNModel_hy.ins.sendOpenCard();
    }
    ...
    private showCardBack(index: number) {
        ...
        for (let i = 0; i < 5; i++) {
            ...
            let path = "card" + i.toString();
            let key = "poker_b1";
            this._cardsList[index.toString()][path].spriteFrame = UTBNNScene_hy.ins.getSpriteFrame(key);
    ...
    private showCardBackByIndex(index: number) {
        ...
        //     //MTBNNModel_hy.ins.randCardData(this._cbCardData,this._cbCardData.length);
        ...
        //     this._btn_seepai.node.active = true;
        //     UTBNNScene_hy.ins.getMusic.playQkp();
        // }
        // else {
        this.showCardBack(index);
        // }
    ...
    private setCardsSprite(index: number, values: number[]): void {
        ...
        for (let i = 0; i < 5; i++) {
            ...
            let key = MTBNNModel_hy.ins.cardValueToSpriteKey1(values[i]);

            // UDebug.Log("key:"+JSON.stringify(key)+ "  index:"+index);

            cards[path].spriteFrame = UTBNNScene_hy.ins.getSpriteFrame(key);
        }
    }
    ...
    setSpineCardType(index: number, cardtype: number): void {
        ...
        // }
        // cardtype = 14;
        aniName = UTBNNHelper_hy.CARD_TYPE2[cardtype];
        ...
        UTBNNScene_hy.ins.getMusic.playPaiXing(cardtype, MTBNNModel_hy.ins.getPlayerSexByUISeat(index));
    ...
    showCardTypeAni1(index: number, cbCardData: number[], cbCardType?: number, cbOxCardData?: number[]) {
        ...
        // MTBNNModel_hy.ins.sortCardList(cbCardData,cbCardData.length);
        ...
        if (cbCardType != null) {
            ...
            cardtype = MTBNNModel_hy.ins.getCardType(cbCardData, cbCardData.length);
        }
        ...
        if (cbOxCardData != null && cbOxCardData.length == 5) {
            ...
            if (niuniuTemp.length == 5) {
                ...
                MTBNNModel_hy.ins.getOxCard(cbCardData);
            }

        }
        else {
            //找出牛牛牌 cbCardData 的值已排序过
            var niuniuCard = MTBNNModel_hy.ins.getOxCard(cbCardData);
    ...
    showCardTypeAni(index: number, cbCardData: number[], cbCardType?: number, cbOxCardData?: number[]) {
        ...
        if (index == 0) {//自己
            for (let i = 0; i < 5; i++) {
                ...
                { //先按原来的位置摆
                    ...
                    if (i < 3)//左3张 0 10 20    0 5 15
                    {
                        let offest = i == 0 ? 0 : offestSelf;
                        let posx = cards["pos"][path].x - offest * i;
                        let pos = new cc.Vec2(posx, cards["pos"][path].y);

                        movePos = pos;
                    }
                    else {
                        let offest = i == 4 ? 0 : offestSelf;
        ...
        for (let i = 0; i < 5; i++) {
            ...
            { //先按原来的位置摆
                ...
                {
                    // let posx = cards["pos"][path].x - offest;
                    let offest2 = i == 0 ? 0 : offest;
                    let posx = cards["pos"][path].x - offest2 * i;
                    let pos = new cc.Vec2(posx, cards["pos"][path].y);
                    ...
                    // let posx = cards["pos"][path].x + offest;
                    let offest2 = i == 4 ? 0 : offest;
                    let posx = cards["pos"][path].x + offest2;
                    let pos = new cc.Vec2(posx, cards["pos"][path].y);
                    movePos = pos;
                }
            }
