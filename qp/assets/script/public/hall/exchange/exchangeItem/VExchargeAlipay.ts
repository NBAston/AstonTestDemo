...
@ccclass
export default class VExchargeAlipay extends cc.Component {
    ...
    _ali_icon: cc.Sprite = null; // 支付宝图标
    _ali_name: cc.Label = null; // 支付宝
    _user_name: cc.Label = null; // 用户名字
    _ali_number: cc.Label = null; // 账号
    ...
    initItemInfo(index: number, offLineItem: any, manager: VExChange): void {
        ...
        this._ali_icon = UNodeHelper.find(this.node, "had_bind_page/ali_logo").getComponent(cc.Sprite);
        this._ali_name = UNodeHelper.find(this.node, "had_bind_page/ali_name").getComponent(cc.Label);
        this._user_name = UNodeHelper.find(this.node, "had_bind_page/user_name").getComponent(cc.Label);
        this._ali_number = UNodeHelper.find(this.node, "had_bind_page/ali_number").getComponent(cc.Label);
        ...
        UEventHandler.addClick(this._bindAccountBtn, this.node, "VExchargeAlipay", "onclickBindAccountBtn");
        UEventHandler.addClick(this._kefuBtn, this.node, "VExchargeAlipay", "onclickKefuBtn");
        UEventHandler.addClick(this._cancel_bind_btn, this.node, "VExchargeAlipay", "onclickCancelBindBtn");
        UEventHandler.addClick(this._all_exchange_btn, this.node, "VExchargeAlipay", "onclickAllExchangeBtn");
        UEventHandler.addClick(this._confirm_exchange_btn, this.node, "VExchargeAlipay", "onclickConfirmExchangeBtn");
        UEventHandler.addClick(this._had_bind_kefu_btn, this.node, "VExchargeAlipay", "onclickKefuBtn");
        UEventHandler.addClick(this._exchange_contact_kefu_btn, this.node, "VExchargeAlipay", "onclickKefuBtn");
        ...
        if(!UStringHelper.isEmptyString(this._userBindInfo.alipayName) && !UStringHelper.isEmptyString(this._userBindInfo.alipayAccount)) { // 已绑定支付宝
    ...
    editDidEnded(): void {
     ...
     onclickBindAccountBtn(): void {
      ...
      onclickCancelBindBtn(): void {
          // 弹框 
          AppGame.ins.showUI(ECommonUI.EXCHANGE_CANCEL_BIND_BOX, EExchareType.alipay);
      }
      ...
      refleshHadBindCardUI(): void {
          ...
          if(this._exchangeLimitInfo.exchangeMinMoneyAlipay != 0) { // 如果为0 则请求 数据
            this._lab_limit.string = "账号需保留"+this._exchangeLimitInfo.exchangeMinLeftMoney * CHARGE_SCALE+"金币，" + "兑换额度" + (this._exchangeLimitInfo.exchangeMinMoneyAlipay * CHARGE_SCALE) +"~"+(this._exchangeLimitInfo.exchangeMaxMoneyAlipay * CHARGE_SCALE);
          ...
          this._user_name.string = this._userBindInfo.alipayName;
          this._ali_number.string = UStringHelper.replaceStringFristEndNum(this._userBindInfo.alipayAccount, 4);
          this._ali_name.string = this._userBindInfo.bankCardType||"";
      }
      ...
      private confirm_cancel_bindcard(isSuccess: boolean) {
          if(isSuccess) {
              // 更换绑定
              AppGame.ins.showUI(ECommonUI.EXCHANGE_BIND_ALIPAY, 2);
          }
      }
      ...
      private confirm_exchange(isSuccess: boolean) {
          if(isSuccess) {
              // 发起兑换请求
              AppGame.ins.roleModel.requesetExchange(parseFloat(this._exchange_editbox.string), EExchareType.alipay);
